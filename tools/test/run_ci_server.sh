...
function main() {
  ...
  docker-compose -f docker-compose-test.yml build weaviate janus index db mock_api \
  ...
  surpress_on_success docker-compose -f docker-compose-test.yml up --force-recreate -d weaviate \
    janus index db genesis_fake remote_weaviate_fake remote_weaviate_fakes_with_test_schema genesis mock_api
