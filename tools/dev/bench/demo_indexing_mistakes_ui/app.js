...
//
// The target cluster URL is hardcoded. The API key is injected at server
// start time via config.js (generated by start.sh from the WCD_API_KEY
// env var). config.js is gitignored so the token never lands in the repo.
...
function apiKey() {
  return (window.WCD_API_KEY || "").trim();
}
...
async function pingServer() {
  try {
    ...
    connStatusText.textContent = apiKey()
      ...
      : `no API key - restart start.sh with WCD_API_KEY set`;
    return false;
  }
}
pingServer();
