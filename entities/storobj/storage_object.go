...
func ObjectsByDocID(bucket bucket, ids []uint64,
	additional additional.Properties, logger logrus.FieldLogger, skipMissingObjects bool,
) ([]*Object, error) {
	if len(ids) == 1 { // no need to try to run concurrently if there is just one result anyway
		return objectsByDocIDSequential(bucket, ids, additional, skipMissingObjects)
	}

	return objectsByDocIDParallel(bucket, ids, additional, logger, skipMissingObjects)
}

func objectsByDocIDParallel(bucket bucket, ids []uint64,
	addProp additional.Properties, logger logrus.FieldLogger, skipMissingObjects bool,
) ([]*Object, error) {
	...
	for chunk := 0; chunk < parallel; chunk++ {
		...
		eg.Go(func() error {
			objs, err := objectsByDocIDSequential(bucket, ids[start:end], addProp, skipMissingObjects)
			if err != nil {
				return err
			}
...
func objectsByDocIDSequential(bucket bucket, ids []uint64,
	additional additional.Properties, skipMissingObjects bool,
) ([]*Object, error) {
	if bucket == nil {
	...
	for _, id := range ids {
		...
		if skipMissingObjects && len(res) == 0 {
			continue
		}
