...
// ReadObjectsByDocID returns at most limit objects for the iterator's doc ids,
// in iteration order (limit <= 0 returns nothing). decode is called once per
// doc id: concurrently, with stored bytes that are only valid for that call, or
// afterwards with nil bytes for a doc id the bucket did not find. decode
// returning false drops the object without spending a limit slot.
