...
func Test_UserConfig(t *testing.T) {
	...
	tests := []test{
		...
		// NOTE: upper bounds (e.g. ksim > 10) are intentionally NOT rejected
		// by ParseAndValidateConfig — parsing also runs on startup/restore,
		// where persisted out-of-range classes must still load. See
		// TestParseAcceptsOutOfBoundsMuvera and TestValidateMuveraUpperBounds
		// (issue #281).
	}
