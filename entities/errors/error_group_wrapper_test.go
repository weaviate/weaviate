...
func TestErrorGroupWrapperPanics(t *testing.T) {
	dispatchers := []struct {
		name string
		// feedsWait is false for RunInline, which returns the panic to its caller.
		feedsWait bool
...
func TestRecoveredPanicKeepsAnErrorValue(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			name: "RunInline",
			run: func(egw *ErrorGroupWrapper) error {
				return egw.RunInline(func() error { panic(sentinel) }, "Books")
			},
		},
	}
	...
	t.Run("a runtime error", func(t *testing.T) {
		...
		err := NewErrorGroupWrapper(logger).RunInline(func() error {
			counts["boom"] = 1
			return nil
		})
...
func TestErrorGroupWrapperRunInline(t *testing.T) {
	...
	jobErr := errors.New("job failed")
	require.ErrorIs(t, eg.RunInline(func() error { return jobErr }), jobErr,
		"Wait never sees a RunInline error, so the return is its only route out")
