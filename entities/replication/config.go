...
type GlobalConfig struct {
	...
	// swept, REPLICA_MOVEMENT_CLEANUP_MAX_AGE. 0 disables the sweep — a zero must
	// never be read as "delete everything". A negative value is rejected, not
	// coerced: it fails startup when set via env and is refused at runtime-overrides
	// reload, leaving the previous value in place.
	...
	// REPLICA_MOVEMENT_CLEANUP_INTERVAL. 0 disables the sweep; a negative value is
	// rejected on the same terms as MaxAge above.
	...
	// CANCELLED ops, REPLICA_MOVEMENT_CLEANUP_INCLUDE_CANCELLED. Opt-in only:
