...
func ValidateFilters(f schema.Finder, filters *LocalFilter) error {
	if filters == nil {
		return errors.New("empty where")
	}
	cw := newClauseWrapper(filters.Root)
	if err := validateClause(f, cw); err != nil {
		return err
	}
	cw.updateClause()
	return nil
...
func validateClause(f schema.Finder, cw *clauseWrapper) error {
	// check if nested
	if cw.getOperands() != nil {
		var errs []error

		for i, child := range cw.getOperands() {
			if err := validateClause(f, child); err != nil {
	...
	class := f.ReadOnlyClass(className.String())
	if class == nil {
