...
type BatchDeleteResponseResults struct {
	...
	// How many objects matched the filter, counted no further than one above `limit`. At or below `limit` this is the exact number of matching objects and every one of them was handled by this call. Above `limit` it means more objects match than one call deletes, so call again.
