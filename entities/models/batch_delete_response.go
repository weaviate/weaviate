...
type BatchDeleteResponse struct {
	...
	// If true, the call reported what the filter matches and deleted nothing. The list holds at most [`QUERY_MAXIMUM_RESULTS`](https://docs.weaviate.io/deploy/configuration/env-vars#QUERY_MAXIMUM_RESULTS) IDs. Defaults to false.
...
type BatchDeleteResponseResults struct {
	...
	// How many objects matched the filter, counted up to one above `limit` and no further. A value at or below `limit` is the exact number of matching objects, and every one of them was handled by this call. A value above `limit` means more objects match than one call deletes, so call again.
