...
func (o *SchemaObjectsPropertiesAddURL) Build() (*url.URL, error) {
	...
	var _path = "/schema/objects/{className}/properties"

	className := o.ClassName
	if className != "" {
		_path = strings.Replace(_path, "{className}", className, -1)
	} else {
		return nil, errors.New("className is required on SchemaObjectsPropertiesAddURL")
	}
...
func (o *SchemaObjectsPropertiesAddURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on SchemaObjectsPropertiesAddURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on SchemaObjectsPropertiesAddURL")
	}
