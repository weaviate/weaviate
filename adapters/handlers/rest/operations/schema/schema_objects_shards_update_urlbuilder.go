...
func (o *SchemaObjectsShardsUpdateURL) Build() (*url.URL, error) {
	...
	_path := "/schema/{className}/shards/{shardName}"

	className := o.ClassName
	if className != "" {
		_path = strings.Replace(_path, "{className}", className, -1)
	} else {
		return nil, errors.New("className is required on SchemaObjectsShardsUpdateURL")
	}

	shardName := o.ShardName
	if shardName != "" {
		_path = strings.Replace(_path, "{shardName}", shardName, -1)
	} else {
		return nil, errors.New("shardName is required on SchemaObjectsShardsUpdateURL")
	}
...
func (o *SchemaObjectsShardsUpdateURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on SchemaObjectsShardsUpdateURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on SchemaObjectsShardsUpdateURL")
	}
