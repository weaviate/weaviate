...
func (h *indexesHandlers) updateIndex(params schema.SchemaObjectsIndexesUpdateParams, principal *models.Principal) middleware.Responder {
	...
	if indexTypeKnown {
		...
		for _, indexTypeForCleanup := range indexTypesForCleanup {
			err := sweep(cleanupCtx, collection, propertyName, indexTypeForCleanup)
...
// propertyName, indexType), its decoded payload, and whether it found one.
...
//
// Whether DTM would accept a cancel for the target is
// [distributedtask.TaskStatus.IsCancellable] on it, the same predicate the
// FSM guard applies, so the two cannot drift. A coordination-phase task is
// still returned as the target; the caller refuses it rather than reporting
// nothing to cancel.
//
...
func findCancelTarget(tasks []*distributedtask.Task, collection, propertyName, indexType string) (
	...
	if refusable != nil {
		return refusable, refusablePayload, true, unknownType
	}
...
// [indexesHandlers.answerCancelThatRacedTheCommit] after it, which
// re-lists first so a task that simply ended gets 202 NO_OP rather than
// a 409 naming a status it has already left.
func (h *indexesHandlers) cancelReindexTask(ctx context.Context, collection, propertyName, indexType string, principal *models.Principal) middleware.Responder {
	...
	if target == nil {
		...
		// did not exist.
		if candidate := firstUnattributableLiveTask(tasks[db.ReindexNamespace]); candidate != nil {
			fields := logrus.Fields{
				"taskID":     candidate.ID,
				"collection": collection,
				"property":   propertyName,
...
func (h *indexesHandlers) drainAndCleanupCancelledTask(
	...
	for _, it := range indexTypesToClean {
		outcome.add(it, sweep(cleanupCtx, collection, propertyName, it))
	}
...
func (o *cleanupSweepOutcome) add(indexType string, err error) {
	switch {
	case err == nil:
	case db.IsCleanupCollectionDropped(err):
		o.dropped = true
	default:
...
// firstUnattributableLiveTask finds an in-flight task whose payload names no
...
func firstUnattributableLiveTask(tasks []*distributedtask.Task) *distributedtask.Task {
	for _, task := range tasks {
		if task.Status.IsTerminal() {
			continue
		}
...
func (h *indexesHandlers) answerCancelThatRacedTheCommit(ctx context.Context, cancelErr error,
	...
	// The FSM also refuses once a task reaches a coordination phase, which it
...
// reindexTaskByID returns the task with this ID, or nil once the list no
// longer carries it.
func reindexTaskByID(tasks []*distributedtask.Task, id string) *distributedtask.Task {
	for _, task := range tasks {
		if task.ID == id {
			return task
		}
