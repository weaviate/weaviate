...
func initReindexAndDistributedTasks(
	...
	if appState.ServerConfig.Config.DistributedTasks.CompletedTaskTTL == 0 {
		...
		if appState.ServerConfig.Config.RuntimeReindexEnabled {
			warn.Warn("TTL=0 also leaves the commit-time backup overlap check nothing to read; every backup is refused before it uploads anything until DISTRIBUTED_TASKS_COMPLETED_TASK_TTL_HOURS is raised above the time a backup takes")
		}
	}
