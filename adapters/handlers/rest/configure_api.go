...
func MakeAppState(ctx, serverShutdownCtx context.Context, options *swag.CommandLineOptionsGroup) *state.State {
	...
	if appState.ServerConfig.Config.DistributedTasks.Enabled {
		...
		// Wire the cluster-wide task surface (list + unthrottled
		// progress) so processOneUnit's semantic-migration barrier
		// wait can (a) push the "reindex done" progress=1.0 signal
		// without it being dropped by ThrottledRecorder, and (b)
		// detect when all units across all nodes have finished their
		// reindex phase. This is what lets the swap run INLINE before
		// the unit is marked completed, closing the FINISHED-before-
		// schema-flip race pinned by
