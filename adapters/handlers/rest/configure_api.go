...
func MakeAppState(ctx, serverShutdownCtx context.Context, options *swag.CommandLineOptionsGroup) *state.State {
	...
	// re-register their double-write callbacks via OnAfterLsmInit during
	// shard load — BEFORE any post-restart write reaches the shard.
	// Without this, writes between shard init and the deferred swap go
	// only to the old main bucket and are silently lost.
