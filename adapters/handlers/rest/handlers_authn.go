...
func (h *authNHandlers) getOwnInfo(_ users.GetOwnInfoParams, principal *models.Principal) middleware.Responder {
	...
	if h.rbacConfig.Enabled {
		existingRoles, err := h.authzController.GetRolesForUserOrGroup(authzConv.ScopedSubjectUser(authentication.AuthType(principal.UserType), principal.Username, principal.IsGlobalOperator), authentication.AuthType(principal.UserType), false)
		if err != nil {
