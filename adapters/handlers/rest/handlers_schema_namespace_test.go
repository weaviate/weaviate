...
// The submit half of reindexusecase.ClusterService is unreachable from the
// property-mutation conflict pre-flight these tests exercise.
func (f fakeReindexTaskLister) AddDistributedTaskWithBarrier(context.Context, string, string, any, []string, bool) error {
	return nil
}

func (f fakeReindexTaskLister) AddDistributedTaskWithGroupsBarrier(context.Context, string, string, any, []distributedtask.UnitSpec, bool) error {
	return nil
}

func (f fakeReindexTaskLister) CancelDistributedTask(context.Context, string, string, uint64) error {
	return nil
}

// reindexServiceWith wraps a task lister in the usecase service the schema
// handlers delegate their conflict pre-flight to.
func reindexServiceWith(lister reindexusecase.ClusterService) *reindexusecase.Service {
	return reindexusecase.New(reindexusecase.Deps{Cluster: lister}, logrus.New())
}
...
func TestDeleteClassPropertyIndex_NamespaceConflictPreflight(t *testing.T) {
	payload, err := json.Marshal(dbreindex.ReindexTaskPayload{
		MigrationType: dbreindex.ReindexTypeChangeTokenization,
	...
	h := &schemaHandlers{
		...
		reindexService: reindexServiceWith(fakeReindexTaskLister{tasks: map[string][]*distributedtask.Task{
