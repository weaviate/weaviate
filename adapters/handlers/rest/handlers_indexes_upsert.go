...
type upsertPlan struct {
	noop           bool
	conflict       string
	migrationType  dbreindex.ReindexMigrationType
	targetTok      string
...
func reindexTasksOrFailClosed(ctx context.Context, principal *models.Principal, lister distributedtask.TaskLister, logger logrus.FieldLogger) ([]*distributedtask.Task, middleware.Responder) {
	...
	return tasks[dbreindex.ReindexNamespace], nil
}
...
func hasUnverifiableInFlightTask(reindexTasks []*distributedtask.Task) bool {
	_, _, found := reindexusecase.FirstActiveReindexTask(reindexTasks, reindexusecase.DecodeUndecodableIsHit, func(p dbreindex.ReindexTaskPayload) bool {
...
func activeSearchableTaskFor(collection, propName, tok, algorithm string, reindexTasks []*distributedtask.Task) (task *distributedtask.Task, matches bool) {
	t, p, found := reindexusecase.FirstActiveReindexTask(reindexTasks, reindexusecase.DecodeSkip, func(p dbreindex.ReindexTaskPayload) bool {
		return strings.EqualFold(p.Collection, collection) && slices.Contains(p.Properties, propName) &&
			dbreindex.TouchesSearchable(p.MigrationType)
	})
	if !found {
		return nil, false
	}
...
func requestMatchesActiveSearchable(activeType dbreindex.ReindexMigrationType, activeTargetTok, tok, algorithm string) bool {
	switch {
	case algorithm != "":
		return activeType == dbreindex.ReindexTypeChangeAlgorithm || activeType == dbreindex.ReindexTypeRebuildSearchable
	case tok != "":
		return (activeType == dbreindex.ReindexTypeChangeTokenization || activeType == dbreindex.ReindexTypeEnableSearchable) &&
			activeTargetTok == tok
	default:
		return false
	}
...
func resolveSearchableUpsert(class *models.Class, collection string, prop *models.Property, tok, algorithm string, reindexTasks []*distributedtask.Task) (upsertPlan, error) {
	...
	if algorithm != "" {
		switch reindexusecase.NormalizeSearchableAlgorithm(algorithm) {
	...
	exists := reindexusecase.SearchableIndexOn(prop)
	...
	if algorithm != "" || tok != "" {
		if task, matches := activeSearchableTaskFor(collection, prop.Name, tok, algorithm, reindexTasks); task != nil {
			...
			if algorithm != "" && exists && dbreindex.SearchablePropertyIsBlockmax(class, prop.Name, reindexTasks) {
	...
	case algorithm != "":
		...
		if dbreindex.SearchablePropertyIsBlockmax(class, prop.Name, reindexTasks) {
			return upsertPlan{noop: true}, nil
		}
		return upsertPlan{migrationType: dbreindex.ReindexTypeChangeAlgorithm}, nil

	case tok != "":
		if !exists {
			// Create the searchable index with the requested tokenization.
			if err := reindexusecase.ValidateEnableSearchableProperty(prop, tok); err != nil {
				return upsertPlan{}, err
			}
			return upsertPlan{migrationType: dbreindex.ReindexTypeEnableSearchable, targetTok: tok}, nil
		}
		if prop.Tokenization == tok {
			return upsertPlan{noop: true}, nil
		}
		// Coupled tokenization migration (rewrites searchable + filterable).
		bucketStrategy, err := reindexusecase.ValidateTokenizationChange(class, prop.Name, tok, reindexTasks)
		if err != nil {
			return upsertPlan{}, err
		}
		return upsertPlan{migrationType: dbreindex.ReindexTypeChangeTokenization, targetTok: tok, bucketStrategy: bucketStrategy}, nil

	default:
		// Empty body: ensure the index exists with its current config.
		if !exists {
			// reindexusecase.ValidateEnableSearchableProperty("") always errors — "not a text
			// type" for ineligible props, "tokenization required" for text
			// ones — so creating a searchable index always needs a body.
			return upsertPlan{}, reindexusecase.ValidateEnableSearchableProperty(prop, "")
		}
		return upsertPlan{noop: true}, nil
	}
}
...
func activeFilterableTaskFor(collection, propName, tok, currentTok string, reindexTasks []*distributedtask.Task) (task *distributedtask.Task, matches bool) {
	t, p, found := reindexusecase.FirstActiveReindexTask(reindexTasks, reindexusecase.DecodeSkip, func(p dbreindex.ReindexTaskPayload) bool {
		return strings.EqualFold(p.Collection, collection) && slices.Contains(p.Properties, propName) &&
			dbreindex.TouchesFilterable(p.MigrationType)
	})
	if !found {
		return nil, false
	}
...
func requestMatchesActiveFilterable(activeType dbreindex.ReindexMigrationType, activeTargetTok, tok, currentTok string) bool {
	switch activeType {
	case dbreindex.ReindexTypeEnableFilterable:
	...
	case dbreindex.ReindexTypeChangeTokenizationFilterable, dbreindex.ReindexTypeChangeTokenization:
...
func resolveFilterableUpsert(collection string, prop *models.Property, tok, algorithm string, reindexTasks []*distributedtask.Task) (upsertPlan, error) {
	...
	if !exists {
		...
		return upsertPlan{migrationType: dbreindex.ReindexTypeEnableFilterable}, nil
	}
	...
	if err := reindexusecase.ValidateFilterableTokenizationChange(prop, tok); err != nil {
		return upsertPlan{}, err
	}
	return upsertPlan{migrationType: dbreindex.ReindexTypeChangeTokenizationFilterable, targetTok: tok}, nil
}
...
func activeRangeableTaskFor(collection, propName string, reindexTasks []*distributedtask.Task) *distributedtask.Task {
	t, _, _ := reindexusecase.FirstActiveReindexTask(reindexTasks, reindexusecase.DecodeSkip, func(p dbreindex.ReindexTaskPayload) bool {
		if !strings.EqualFold(p.Collection, collection) || !slices.Contains(p.Properties, propName) {
			return false
		}
		matches, _ := reindexusecase.MigrationTypeTargetsIndex(p.MigrationType, "rangeable")
		return matches
	})
	return t
}
...
func resolveRangeableUpsert(class *models.Class, collection string, prop *models.Property, tok, algorithm string, reindexTasks []*distributedtask.Task) (upsertPlan, error) {
	...
	if err := reindexusecase.ValidateRangeableProperties(class, []string{prop.Name}); err != nil {
		return upsertPlan{}, err
	}
	return upsertPlan{migrationType: dbreindex.ReindexTypeEnableRangeable}, nil
}
...
func (h *indexesHandlers) resolveRebuildPlan(class *models.Class, prop *models.Property, indexType string, reindexTasks []*distributedtask.Task) (upsertPlan, error) {
	switch indexType {
	case "searchable":
		if !reindexusecase.SearchableIndexOn(prop) {
			return upsertPlan{}, errors.New(db.NoSearchableIndexError(prop.Name))
		}
		...
		return upsertPlan{migrationType: dbreindex.ReindexTypeRebuildSearchable}, nil

	case "filterable":
		if !reindexusecase.FilterableIndexOn(prop) {
			return upsertPlan{}, fmt.Errorf("property %q does not have a filterable index", prop.Name)
		}
		if err := reindexusecase.ValidateRebuildFilterableDataType(prop); err != nil {
			return upsertPlan{}, err
		}
		return upsertPlan{migrationType: dbreindex.ReindexTypeRepairFilterable}, nil

	case "rangeable":
		if err := reindexusecase.ValidateRebuildRangeableProperty(prop); err != nil {
			return upsertPlan{}, err
		}
		return upsertPlan{migrationType: dbreindex.ReindexTypeRepairRangeable}, nil
	}
	// Unreachable: normalizeIndexTypeParam already validated the token.
	return upsertPlan{}, fmt.Errorf("unsupported index type %q", indexType)
}
...
func (h *indexesHandlers) validateTenantScope(ctx context.Context, principal *models.Principal, collection string, isMT, semantic bool, tenants []string) middleware.Responder {
	...
	if err := reindexusecase.ValidateTenants(ctx, h.appState.DB, collection, tenants); err != nil {
		return jsonResponder(http.StatusBadRequest, errorResponse(principal, err.Error()))
	}
	return nil
}
...
func (h *indexesHandlers) submitReindexTask(ctx context.Context, principal *models.Principal, class *models.Class, collection, propertyName string, plan upsertPlan, tenants []string, reindexTasks []*distributedtask.Task) middleware.Responder {
	...
	unitIDs, unitToShard, unitToNode := reindexusecase.BuildUnitMaps(shardOwnership)
	...
	taskID := fmt.Sprintf("%s:%s:%s:%s", collection, migrationType, properties[0], reindexusecase.ShortRandomSuffix())
	...
	if isMT && semantic {
		unitSpecs := reindexusecase.BuildUnitSpecs(shardOwnership)
...
) dbreindex.ReindexTaskPayload {
	var originalTok string
	if mt := plan.migrationType; mt == dbreindex.ReindexTypeChangeTokenization ||
		mt == dbreindex.ReindexTypeChangeTokenizationFilterable || mt == dbreindex.ReindexTypeEnableSearchable {
		if p := findProperty(class, properties[0]); p != nil {
			originalTok = p.Tokenization
		}
	}
	return dbreindex.ReindexTaskPayload{
		MigrationType:        plan.migrationType,
		Collection:           collection,
...
func (h *indexesHandlers) cleanStalePartialStateOrFail(ctx context.Context, principal *models.Principal,
	cleaner stalePartialStateCleaner, collection, propertyName string, migrationType dbreindex.ReindexMigrationType,
) middleware.Responder {
	indexTypesForCleanup, known := reindexusecase.IndexTypesFromMigrationType(migrationType)
	if !known {
		h.appState.Logger.WithFields(logrus.Fields{
			"collection":     collection,
			"property":       propertyName,
