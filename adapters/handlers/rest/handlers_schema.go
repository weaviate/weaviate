...
func (s *schemaHandlers) deletedClassProperty(params schema.SchemaObjectsPropertiesDeleteParams,
	principal *models.Principal,
) middleware.Responder {
	ctx := restCtx.AddPrincipalToContext(params.HTTPRequest.Context(), principal)
	err := s.manager.DeleteClassProperty(ctx, principal, s.manager.ReadOnlyClass(params.ClassName), params.ClassName, params.PropertyName)
	if err != nil {
		s.metricRequestsTotal.logError(params.ClassName, err)
		switch {
		case errors.As(err, &authzerrors.Forbidden{}):
...
func setupSchemaHandlers(api *operations.WeaviateAPI, manager *schemaUC.Manager, metrics *monitoring.PrometheusMetrics, logger logrus.FieldLogger) {
	...
	api.SchemaSchemaObjectsPropertiesDeleteHandler = schema.
		SchemaObjectsPropertiesDeleteHandlerFunc(h.deletedClassProperty)
