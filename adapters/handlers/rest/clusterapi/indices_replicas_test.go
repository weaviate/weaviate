...
func TestMaintenanceModeReplicatedIndices(t *testing.T) {
	...
	indices := clusterapi.NewReplicatedIndices(nil, nil, noopAuth, func() bool { return true }, func() bool { return true })
...
func TestClusterNotReadyReplicatedIndices(t *testing.T) {
	noopAuth := clusterapi.NewNoopAuthHandler()
	// NOTE leaving shards and scaler nil for now, fill in when needed
	indices := clusterapi.NewReplicatedIndices(nil, nil, noopAuth, func() bool { return false }, func() bool { return false })
	mux := http.NewServeMux()
	mux.Handle("/replicas/indices/", indices.Indices())
	server := httptest.NewServer(mux)
