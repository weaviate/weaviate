...
func TestReplicatedIndicesWorkQueue(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			fakeReplicator.EXPECT().CommitReplication(context.Background(), mock.Anything, mock.Anything, mock.Anything).Run(func(_ context.Context, _ string, _ string, _ string) {
				<-commitBlock
