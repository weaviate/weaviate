...
func TestMaintenanceMode(t *testing.T) {
	...
	requestURL := func(suffix string) string {
		return fmt.Sprintf("%s/indices/MyClass/shards/myshard/objects%s", server.URL, suffix)
	}
	// TODO dry [(post, /_search), (post, /_find)]
	t.Run("post search returns maintenance mode status", func(t *testing.T) {
		res, err := http.Post(requestURL("/_search"), "application/json", http.NoBody)
		assert.Nil(t, err)
		assert.True(t, res.StatusCode == maintenanceModeExpectedHTTPStatus)
	})

	t.Run("post find returns maintenance mode status", func(t *testing.T) {
		res, err := http.Post(requestURL("/_find"), "application/json", http.NoBody)
		assert.Nil(t, err)
		assert.True(t, res.StatusCode == maintenanceModeExpectedHTTPStatus)
	})
}
