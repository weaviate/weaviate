...
func TestBackwardCompatibilitySearch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run("test", func(t *testing.T) {
			b126, err := payload.Marshal(tt.SearchVectors, tt.Targets, 0.7, 10, nil, nil, nil, nil, nil, additional.Properties{}, nil)
			require.Nil(t, err)

			vecs, targets, _, _, _, _, _, _, _, _, _, err := payload.Unmarshal(b126)
			require.Nil(t, err)
			...
			if tt.compatible {
				...
				vecs, targets, _, _, _, _, _, _, _, _, _, err := payload.Unmarshal(b125)
				require.Nil(t, err)
				assert.Equal(t, tt.SearchVectors, vecs)
				assert.Equal(t, tt.Targets, targets)
