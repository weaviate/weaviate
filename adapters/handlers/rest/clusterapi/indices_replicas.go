...
// subset as a binary stream. Both request and response use
// replica.CompareDigestsRecordLength (25-byte) records:
// 16-byte UUID + 8-byte UpdateTime (int64 big-endian) + 1-byte flags.
// Bit 0 of the flags byte (CompareDigestsFlagDeleted) is set in the response
// when the target has a tombstone that should win.
