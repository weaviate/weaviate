...
// buildTask builds a *distributedtask.Task with the given payload + status +
// units. Shared by the reindex handler tests in this package.
func buildTask(t *testing.T, id string, status distributedtask.TaskStatus,
	payload dbreindex.ReindexTaskPayload, units map[string]*distributedtask.Unit,
...
func TestCheckReindexAdmission(t *testing.T) {
	...
	capTasks := func(n int) []*distributedtask.Task {
		...
		for i := 0; i < n; i++ {
			...
			tasks = append(tasks, buildTask(t, "C:enable-rangeable:"+p,
				...
				dbreindex.ReindexTaskPayload{MigrationType: dbreindex.ReindexTypeEnableRangeable, Collection: collection, Properties: []string{p}}, nil))
		}
		return tasks
	}
	...
	cases := []struct {
	...
	}{
		{
			...
			migrationType: dbreindex.ReindexTypeEnableRangeable,
			props:         []string{"pZZ"},
		...
		},
		{
			...
			tasks: []*distributedtask.Task{buildTask(t, "C:change-tokenization:p:aaaa",
				distributedtask.TaskStatusStarted,
				dbreindex.ReindexTaskPayload{MigrationType: dbreindex.ReindexTypeChangeTokenization, Collection: collection, Properties: []string{"p"}}, nil)},
...
func TestReindexTasksOrFailClosed_ReturnsReindexNamespace(t *testing.T) {
	...
	tasks, resp := reindexTasksOrFailClosed(context.Background(), nil,
		fakeTaskLister{tasks: map[string][]*distributedtask.Task{dbreindex.ReindexNamespace: want}}, logger)
