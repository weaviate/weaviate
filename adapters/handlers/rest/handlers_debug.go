...
func setupDebugHandlers(appState *state.State) {
	...
	// Property cardinality stats for a whole collection, reported as the maximum
	// per-property estimate across all of the collection's shards. When values
	// are spread across shards the max is a close lower bound on the
	// collection-wide distinct count (and equals it when every distinct value
	// lands in at least one shard that holds nothing the others don't).
	// GET localhost:6060/debug/stats/cardinality/{collection}
	// The port is Weaviate's configured Go profiling port (defaults to 6060).
	http.HandleFunc("/debug/stats/cardinality/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		colName := strings.Trim(strings.TrimSpace(strings.TrimPrefix(r.URL.Path, "/debug/stats/cardinality/")), "/")
		if colName == "" || strings.Contains(colName, "/") {
			http.Error(w, "expected /debug/stats/cardinality/{collection}", http.StatusBadRequest)
			return
		}
		// Force-loading a cold shard fully initializes it (incl. vector indexes)
		// and can block for a long time on large collections, so it's opt-in.
		withLoad := r.URL.Query().Get("load") == "true"
		// For multi-tenant collections each tenant is a shard; ?tenant=<name>
		// restricts the report to that one instead of every local shard.
		...
		class := appState.SchemaManager.SchemaReader.ReadOnlyClass(colName)
		if class == nil {
			http.Error(w, "collection not found in schema", http.StatusNotFound)
			return
		}
		...
		// Probe both the filterable and searchable inverted-index buckets for
		// every top-level property; report whichever exist on any shard.
		var refs []bucketRef
		for _, prop := range class.Properties {
			refs = append(refs,
				bucketRef{prop.Name, "filterable", helpers.BucketFromPropNameLSM(prop.Name)},
				bucketRef{prop.Name, "searchable", helpers.BucketSearchableFromPropNameLSM(prop.Name)},
			)
		}
		...
		aggs := make([]agg, len(refs))

		// Enumerate shards from the sharding state, not the in-memory shard map:
		// the map omits lazy shards that haven't been loaded yet (and cold MT
		// tenants), which would otherwise report shardCount 0. GetShard below
		// loads each one on demand.
		shardNames, err := appState.SchemaManager.SchemaReader.LocalShards(colName)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		if tenant != "" {
			if !slices.Contains(shardNames, tenant) {
				http.Error(w, fmt.Sprintf("tenant/shard %q not found among local shards %v", tenant, shardNames), http.StatusNotFound)
				return
			}
			shardNames = []string{tenant}
		}

		shardErrors := map[string]string{}
		bucketsPresent := map[string][]string{} // shard -> inverted-index buckets found
		for _, name := range shardNames {
			func() {
				shard, release, err := idx.GetShard(r.Context(), name)
				if err != nil {
					shardErrors[name] = err.Error()
					return
				}
				if shard == nil {
					// GetShard does not force-load a lazy shard. Loading it fully
					// initializes the shard and can block on large collections, so
					// only do it when explicitly requested.
					if !withLoad {
						shardErrors[name] = "not loaded (pass ?load=true to force-load, may be slow)"
						return
					}
					if lerr := idx.LoadLocalShard(r.Context(), name, true); lerr != nil {
						shardErrors[name] = "load: " + lerr.Error()
						return
					}
					shard, release, err = idx.GetShard(r.Context(), name)
					if err != nil {
					...
					if shard == nil {
						shardErrors[name] = "shard nil after load"
						return
					}
				}
				defer release()
				...
				// Diagnostic: which property_* buckets actually exist on this shard.
				for _, bn := range store.BucketNames() {
					if strings.HasPrefix(bn, "property_") {
						bucketsPresent[name] = append(bucketsPresent[name], bn)
					}
				}

				for i := range refs {
					bucket := store.Bucket(refs[i].bucket)
					if bucket == nil {
						continue
					}
					a := &aggs[i]
					a.strategy = bucket.Strategy()
					est, err := bucket.GetKeysCount()
					if err != nil {
						a.lastErr = err.Error()
						continue
					}
		...
		for i := range refs {
			a := aggs[i]
			if !a.found && a.lastErr == "" {
				continue // bucket absent on every shard
			}
			ps := propStat{
				Property:  refs[i].property,
				IndexType: refs[i].indexType,
		...
		resp := map[string]any{
			...
			"propertyCount": len(class.Properties),
			"note":          "maxEstimate: per property, the largest GetKeysCount() estimate across the collection's shards (which shard is in maxShard). GetKeysCount approximates distinct keys from the inverted-index bloom filters. For searchable buckets keys are tokens (distinct terms), not distinct field values.",
			"properties":    stats,
			"elapsedMs":     float64(time.Since(start).Microseconds()) / 1000,
		}
		if len(shardErrors) > 0 {
			resp["shardErrors"] = shardErrors
		}
		// Surface the actual inverted-index buckets only when nothing matched,
		// to diagnose property/bucket-name mismatches without bloating output.
		if len(stats) == 0 {
			resp["bucketsPresent"] = bucketsPresent
		}
