...
func setupDebugHandlers(appState *state.State) {
	...
	http.HandleFunc("/debug/reindex/to-blockmax", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		colName := r.URL.Query().Get("collection")
		if colName == "" {
			http.Error(w, "collection is required", http.StatusBadRequest)
			return
		...
		task := db.NewRuntimeMapToBlockmaxTask(logger, appState.SchemaManager)
		...
		if err != nil {
			logger.WithError(err).Error("reindex/to-blockmax: ForEachShard failed")
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	http.HandleFunc("/debug/reindex/change-tokenization", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		colName := r.URL.Query().Get("collection")
		if colName == "" {
			http.Error(w, "collection is required", http.StatusBadRequest)
			return
		...
		targetTokenization := r.URL.Query().Get("tokenization")
		...
		// Find the property and validate.
		...
		if targetProp == nil {
			http.Error(w, fmt.Sprintf("property %q not found", propName), http.StatusBadRequest)
			return
		}

		dt, ok := schema.AsPrimitive(targetProp.DataType)
		if !ok || (dt != schema.DataTypeText && dt != schema.DataTypeTextArray) {
			http.Error(w, fmt.Sprintf("property %q is not a text type", propName), http.StatusBadRequest)
			return
		}

		// Validate target tokenization is a valid enum value.
		...
		if _, ok := validTokenizations[targetTokenization]; !ok {
			http.Error(w, fmt.Sprintf("invalid tokenization %q", targetTokenization), http.StatusBadRequest)
			return
		}

		if targetProp.Tokenization == targetTokenization {
			http.Error(w, fmt.Sprintf("property %q already uses tokenization %q", propName, targetTokenization), http.StatusBadRequest)
			return
		}
		...
		if bucketStrategy == "" {
			http.Error(w, fmt.Sprintf("searchable bucket not found for property %q", propName), http.StatusBadRequest)
			return
		}

		// Run searchable retokenize first, then filterable retokenize.
		// The filterable strategy performs the schema update in OnMigrationComplete.
		searchableTask := db.NewRuntimeSearchableRetokenizeTask(
			logger, propName, targetTokenization, colName, bucketStrategy, colName,
		)
		filterableTask := db.NewRuntimeFilterableRetokenizeTask(
			logger, appState.SchemaManager,
			propName, targetTokenization, colName, colName,
		)
		...
		err := idx.ForEachShard(func(shardName string, shard db.ShardLike) error {
			...
			if err := searchableTask.RunOnShard(r.Context(), shard); err != nil {
				response[shardName] = map[string]string{"status": "error", "message": "searchable: " + err.Error()}
				return nil
			}
			if err := filterableTask.RunOnShard(r.Context(), shard); err != nil {
				response[shardName] = map[string]string{"status": "error", "message": "filterable: " + err.Error()}
				return nil
			}
			response[shardName] = map[string]string{"status": "completed"}
			return nil
		...
		if err != nil {
			logger.WithError(err).Error("reindex/change-tokenization: ForEachShard failed")
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
