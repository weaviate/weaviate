...
func ExtractNearVector(source map[string]interface{}, targetVectorsFromOtherLevel []string) (searchparams.NearVector, *dto.TargetCombination, error) {
	...
	if okVec {
		if len(targetVectors) == 0 {
			args.Vectors = []types.Vector{vectorGQL}
		} else {
			args.Vectors = make([]types.Vector, len(targetVectors))
	...
	if okVecPerTarget {
		...
		if len(targetVectors) == 0 {
			...
			vectors = make([]types.Vector, 0, len(vectorPerTarget))
			...
			vectors = make([]types.Vector, len(targetVectors))
