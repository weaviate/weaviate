...
func (c *WeaviateCreator) UpsertObject(ctx context.Context, req mcp.CallToolRequest, args UpsertObjectArgs) (*UpsertObjectResp, error) {
	...
	if !c.IsWriteAccessEnabled() {
		return nil, fmt.Errorf("%w MCP write access is disabled; to enable, either set MCP_SERVER_WRITE_ACCESS_ENABLED=true (requires restart) or set mcp_server_write_access_enabled: true in the runtime overrides YAML (no restart needed)", mcpmetrics.ErrWriteDisabled)
	}
