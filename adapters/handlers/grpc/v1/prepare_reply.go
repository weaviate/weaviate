...
// Strips the caller's own namespace from the ref target class so namespaced
// callers never see "<ns>:" on an expanded cross-ref at any depth — symmetric
// with the top-level strip in extractPropertiesAnswer. (The previous code
// echoed className verbatim with a comment claiming "ref-shape stripping
// lives in a later slice" — that later slice never existed.)
