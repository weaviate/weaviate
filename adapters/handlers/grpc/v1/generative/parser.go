...
func (p *Parser) extract(req *pb.GenerativeSearch, class *models.Class) *generate.Params {
	...
	if req.Single != nil {
		...
		if len(req.Single.Queries) > 0 {
			...
			case *pb.GenerativeProvider_Friendli:
				options = p.friendli(query.GetFriendli())
				providerName = friendliParams.Name
			default:
				// do nothing
			}
...
func (p *Parser) friendli(in *pb.GenerativeFriendli) map[string]any {
	if in == nil {
		return nil
	}
	return map[string]any{
		friendliParams.Name: friendliParams.Params{
			BaseURL:     in.GetBaseUrl(),
			Model:       in.GetModel(),
			MaxTokens:   p.int64ToInt(in.MaxTokens),
			Temperature: in.Temperature,
