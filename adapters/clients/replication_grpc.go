...
func (c *grpcReplicationClient) HashTreeLevel(ctx context.Context, host, index, shard string,
	...
	// matching the REST client behavior. The server expects a discriminant of
	// size LeavesCount(level); sending the full global bitset (NodesCount(height))
	// would fail the server-side size check.
