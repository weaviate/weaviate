...
func (c *RemoteNode) GetNodeBatchStatus(ctx context.Context, hostName string) (*models.NodeBatchStatus, error) {
	p := "/nodes/batch-status"
	method := http.MethodGet
	...
	var nodeStatus models.NodeBatchStatus
	err = json.Unmarshal(body, &nodeStatus)
	if err != nil {
		return nil, enterrors.NewErrUnmarshalBody(err)
	}
