...
func TestReplicationHashTreeLevel(t *testing.T) {
	...
	t.Run("DiscriminantTooSmall", func(t *testing.T) {
		// Level 3 requires InnerNodesCount(3)+LeavesCount(3) = 7+8 = 15 bits.
		// A discriminant of size 14 must be rejected before ExtractSlice is called.
		tooSmall := hashtree.NewBitset(hashtree.InnerNodesCount(3) + hashtree.LeavesCount(3) - 1)
		c := newReplicationClient(t, &http.Client{})
		_, err := c.HashTreeLevel(context.Background(), "127.0.0.1:0", "C1", "S1", 3, tooSmall)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "discriminant size")
