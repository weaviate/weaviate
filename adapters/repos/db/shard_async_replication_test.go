...
func TestResumeMaintenanceCyclesReappliesSkippedEnable(t *testing.T) {
	...
	require.NoError(t, s.HaltForTransfer(ctx, "test:transfer", false, 0))
...
func TestHaltedForTransferDoesNotBlockOnHeldMux(t *testing.T) {
	...
	select {
	case observed := <-got:
		require.False(t, observed, "a held mux without a halt must not read as halted")
	case <-time.After(2 * time.Second):
		t.Fatal("haltedForTransfer parked on the held mux")
	}

	// Recorded under the still-held mux: the probe must see it without waiting for the unlock.
	s.haltAddOwnerLocked("test:transfer")
	close(halted)
	select {
	case observed := <-got:
		require.True(t, observed, "an actual halt must read as halted")
	case <-time.After(2 * time.Second):
		t.Fatal("haltedForTransfer parked on the held mux")
	}
	s.haltDropOwnerLocked("test:transfer")
}
