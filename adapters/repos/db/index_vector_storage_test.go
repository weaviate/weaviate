	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			index, err := NewIndex(ctx, IndexConfig{
				...
				defaultVectorConfig, vectorConfigs, mockRouter, shardResolver, mockSchema, mockSchemaReader, nil, logger, nil, nil, nil, &replication.GlobalConfig{}, nil, class, nil, scheduler, nil, nil, NewShardReindexerV3Noop(), roaringset.NewBitmapBufPoolNoop(), false)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			index, err := NewIndex(ctx, IndexConfig{
				...
				enthnsw.UserConfig{
					VectorCacheMaxObjects: 1000,
				}, vectorConfigs, mockRouter, shardResolver, mockSchema, mockSchemaReader, nil, logger, nil, nil, nil, &replication.GlobalConfig{}, nil, class, nil, scheduler, nil, nil, NewShardReindexerV3Noop(), roaringset.NewBitmapBufPoolNoop(), false)
			require.NoError(t, err)
			defer index.Shutdown(ctx)
...
func TestIndex_VectorStorageSize_ActiveVsUnloaded(t *testing.T) {
	...
	newIndex, err := NewIndex(ctx, IndexConfig{
		...
		enthnsw.UserConfig{
			VectorCacheMaxObjects: 1000,
		}, index.GetVectorIndexConfigs(), mockRouter, shardResolver, mockSchema, mockSchemaReader, nil, logger, nil, nil, nil, &replication.GlobalConfig{}, nil, class, nil, scheduler, nil, nil, NewShardReindexerV3Noop(), roaringset.NewBitmapBufPoolNoop(), false)
	require.NoError(t, err)
