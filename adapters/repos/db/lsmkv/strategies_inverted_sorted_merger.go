...
type sortedInvertedMerger struct {
	input   [][]InvertedPair
	output  []InvertedPair
	offsets []int
}

func newSortedInvertedMerger() *sortedInvertedMerger {
	return &sortedInvertedMerger{}
}

func (s *sortedInvertedMerger) do(segments [][]InvertedPair) ([]InvertedPair, error) {
...
func (s *sortedInvertedMerger) doKeepTombstonesReusable() ([]InvertedPair, error) {
	i := 0
	for {
		match, ok := s.findSegmentWithLowestKey()
		if !ok {
...
func (s *sortedInvertedMerger) init(segments [][]InvertedPair) error {
	s.input = segments
	...
	s.output = make([]InvertedPair, maxOutput)
...
func (s *sortedInvertedMerger) reset(segments [][]InvertedPair) error {
	s.input = segments
	...
	if cap(s.output) >= maxOutput {
		...
		s.output = make([]InvertedPair, maxOutput, int(float64(maxOutput)*1.25))
	}

	return nil
}

func (s *sortedInvertedMerger) findSegmentWithLowestKey() (InvertedPair, bool) {
	bestSeg := -1
	bestKey := []byte(nil)
