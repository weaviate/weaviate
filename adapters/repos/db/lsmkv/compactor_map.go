...
func newCompactorMapCollection(w io.WriteSeeker,
	...
	scratchSpacePath string, requiresSorting bool, isLeftRoot bool,
) *compactorMap {
	return &compactorMap{
		c1:                  c1,
		c2:                  c2,
...
func (c *compactorMap) writeKeys() ([]segmentindex.Key, error) {
	...
	for {
		...
		if bytes.Equal(key1, key2) {
			...
			if values, skip := c.optimizeValues(mergedEncoded); !skip {
...
func (c *compactorMap) optimizeValues(values []value) (vals []value, skip bool) {
	if !c.isLeftRoot {
		return values, false
	}
