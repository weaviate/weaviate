...
func (*Bucket) NewBucket(ctx context.Context, dir, rootDir string, logger logrus.FieldLogger,
	...
	if !b.delaySegmentLoading {
		// don't need the disk here, just start the loading
		_, err := b.getDisk()
		if err != nil {
			return nil, err
		}
...
func (b *Bucket) MapList(ctx context.Context, key []byte, cfgs ...MapListOption) ([]MapPair, error) {
	...
	disk, segmentsDisk, release, err := dsk.getCollectionAndSegments(key)
	if err != nil && !errors.Is(err, lsmkv.NotFound) {
		return nil, err
	}
...
func (b *Bucket) FlushAndSwitch() error {
	...
	if b.flushing.strategy == StrategyInverted {
		b.flushing.averagePropLength, b.flushing.propLengthCount = disk.GetAveragePropertyLength()
	}
	...
	case StrategyRoaringSetRange:
		if b.keepSegmentsInMemory {
			if err := disk.roaringSetRangeSegmentInMemory.MergeMemtable(flushing.roaringSetRange); err != nil {
...
func (b *Bucket) initAndPrecomputeNewSegment() (*segment, error) {
	...
	segment, err := disk.initAndPrecomputeNewSegment(path + ".db")
	if err != nil {
		return nil, err
	}
...
func (b *Bucket) atomicallyAddDiskSegmentAndRemoveFlushing(seg *segment) error {
	...
	if err := disk.addInitializedSegment(seg); err != nil {
		return err
	}
	b.flushing = nil
...
func (b *Bucket) CreateDiskTerm(N float64, filterDocIds helpers.AllowList, query []string, propName string, propertyBoost float32, duplicateTextBoosts []int, config schema.BM25Config, ctx context.Context) ([][]*SegmentBlockMax, map[string]uint64, func(), error) {
	...
	segmentsDisk, release := disk.getAndLockSegments()
...
func (b *Bucket) GetAveragePropertyLength() (float64, error) {
	...
	segmentAveragePropLength, segmentPropCount := disk.GetAveragePropertyLength()
...
func (b *Bucket) getDisk() (*SegmentGroup, error) {
	...
	if b.disk == nil {
		b.diskReloadLock.RUnlock()
		b.diskReloadLock.Lock()
		defer b.diskReloadLock.Unlock()
		if b.disk != nil {
			// could have been loaded by another thread
			return b.disk, nil
		}
		disk, err := b.loadSegments()
		if err != nil {
			return nil, err
		}
		b.disk = disk
