...
// reusableInvertedPairs is not thread-safe and intended for usage from a single
...
type reusableInvertedPairs struct {
	left  []InvertedPair
	right []InvertedPair
}

func newReusableInvertedPairs() *reusableInvertedPairs {
	return &reusableInvertedPairs{}
}

func (rmp *reusableInvertedPairs) ResizeLeft(size int) {
	if cap(rmp.left) >= size {
		...
		rmp.left = make([]InvertedPair, size, int(float64(size)*1.25))
	}
}

func (rmp *reusableInvertedPairs) ResizeRight(size int) {
	if cap(rmp.right) >= size {
		...
		// weaviate-chaos-engineering/apps/importer-no-vector-index test script a
		// simple 25% overhead reduced the resizing needs to almost zero.
		rmp.right = make([]InvertedPair, size, int(float64(size)*1.25))
	}
}
