...
// identityTransformers is a one-entry registry whose transformer leaves values
// untouched, enough to exercise the wiring/lifecycle without asserting bytes.
func identityTransformers() map[OpType]OpTransformerFactory {
	return map[OpType]OpTransformerFactory{
...
// TestBucket_WithEditOpTransformers_OpensAndClosesEditOps proves the option wires
...
func TestBucket_WithEditOpTransformers_OpensAndClosesEditOps(t *testing.T) {
	...
	bucket, err := NewBucketCreator().NewBucket(ctx, dir, dir, logger, nil,
		...
		WithStrategy(StrategyReplace), WithEditOpTransformers(identityTransformers()))
	require.NoError(t, err)
	require.NotNil(t, bucket.disk.editOps, "WithEditOpTransformers should wire the edit-ops sidecar")
...
// TestWithEditOpTransformers_RejectsNonReplace pins the fail-fast replace-only
// guard: wiring the option on any non-replace bucket errors at setup, across
// every other strategy.
func TestWithEditOpTransformers_RejectsNonReplace(t *testing.T) {
	for _, strategy := range []string{
		...
		t.Run(strategy, func(t *testing.T) {
			...
			_, err := NewBucketCreator().NewBucket(ctx, dir, dir, logger, nil,
				cyclemanager.NewCallbackGroupNoop(), cyclemanager.NewCallbackGroupNoop(),
				WithStrategy(strategy), WithEditOpTransformers(identityTransformers()))
			require.Error(t, err)
...
// TestBucket_NoEditOpTransformers_NoEditOps confirms a bucket without the option
// keeps no edit-ops sidecar (no per-bucket bolt file when the feature is unused).
func TestBucket_NoEditOpTransformers_NoEditOps(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	logger, _ := test.NewNullLogger()
