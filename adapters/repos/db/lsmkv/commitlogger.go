...
type memtableCommitLogger interface {
	...
	add(node *roaringset.SegmentNode) error
	getPath() string
	setPath(path string)
	size() int64
	flushBuffers() error
...
func (cl *lazyCommitLogger) add(node *roaringset.SegmentNode) error {
	err := cl.mayInitCommitLogger()
	if err != nil {
		return err
	}
...
func (cl *lazyCommitLogger) getPath() string {
	return cl.path
}

func (cl *lazyCommitLogger) setPath(path string) {
	cl.path = path
}
...
func (cl *commitLogger) getPath() string {
	return cl.path
}

// TODO: this requires further investigation, introduced to maintain current behaviour
func (cl *commitLogger) setPath(path string) {
	cl.path = path
}
