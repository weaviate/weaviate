...
// bucketKeyWriter builds a bucket of one strategy and writes distinct keys into
// it. A filterable property is a roaringset bucket and a searchable one is
// inverted or map, so a cardinality estimate reads at least two strategies for
// the same property.
type bucketKeyWriter struct {
	name    string
	opts    []BucketOption
	addKeys func(t *testing.T, b *Bucket, start, end int)
	// loadsAtOpen marks a strategy whose segment group reads segment data while
	// the bucket is being created, so lazy loading never defers past that point
	loadsAtOpen bool
}

// bucketKeyWriters omits StrategyRoaringSetRange: it holds per-bit bitmaps
// rather than a key set, so its memtable has no keys to walk and the shard
// builds it with WithUseBloomFilter(false), which GetKeysCount rejects.
var bucketKeyWriters = []bucketKeyWriter{
	{
		name: "roaringset",
		opts: []BucketOption{
	...
	},
	{
		...
		// newSegmentGroup reads every inverted segment's inverted data at open
		loadsAtOpen: true,
		addKeys: func(t *testing.T, b *Bucket, start, end int) {
			for i := start; i < end; i++ {
...
// TestBucketPropertyCardinality exercises Bucket.GetKeysCount across the bucket
// strategies an inverted-index property uses. Its keys are the distinct
// property values, so the estimates are asserted against the known distinct
// count of the inserted data.
func TestBucketPropertyCardinality(t *testing.T) {
	ctx := testCtx()

	for _, w := range bucketKeyWriters {
		t.Run(w.name, func(t *testing.T) {
			...
			t.Run("memtable-only: estimates from the unflushed memtable", func(t *testing.T) {
				...
				w.addKeys(t, b, 0, 100) // no flush: data stays in the memtable
			...
			t.Run("lazily loaded segment without a bloom filter: keys counted exactly", func(t *testing.T) {
				...
				require.Equal(t, w.loadsAtOpen, lazy.isLoaded(),
					"unless the strategy loads at open, the estimate must be what loads the segment")
