...
func (sg *SegmentGroup) replaceCompactedSegmentsBlocking(
	...
	newPath, err := sg.stripTmpExtension(newSeg.path, segmentID(leftSegment.path), segmentID(rightSegment.path))
	if err != nil {
	...
	for i, pth := range newSeg.metaPaths {
		updated, err := sg.stripTmpExtension(pth, segmentID(leftSegment.path), segmentID(rightSegment.path))
		if err != nil {
			return nil, nil, errors.Wrap(err, "strip .tmp extension of new segment")
		}
