...
func (m *Memtable) flush() (segmentPath string, rerr error) {
	...
	if m.writeSegmentInfoIntoFileName {
		// new segments are always level 0; they always carry secondary tombstones
		// because deleteObjectFromObjectsBucket now always writes both primary and
		// secondary tombstones (see shard_write_delete.go).
		tmpSegmentPath = m.path + segmentExtraInfo(0, SegmentStrategyFromString(m.strategy), true) + ".db.tmp"
	} else {
