...
func (s *FilterableToRangeableStrategy) PreReindexHook(shard *Shard, props []string) {
	...
	for _, propName := range props {
		...
		// Null/length buckets: an unindexed property has none (shard init
		// skips it) - see EnableFilterableStrategy.PreReindexHook for why
		// this must run before the backfill needs them.
