...
func singleShardState(t testing.TB) *sharding.State {
	...
	selector.EXPECT().LocalName().Return("node-1")
	selector.EXPECT().StorageCandidates().Return([]string{"node-1"})
...
func multiShardState(t *testing.T) *sharding.State {
...
func (b *MultiTenantShardingStateBuilder) Build(t *testing.T) *sharding.State {
	if len(b.tenants) <= 0 {
	...
	selector.EXPECT().StorageCandidates().Return(nodeNames)
...
func fixedMultiShardState(t *testing.T) *sharding.State {
...
`)
	...
	mockSelector := cluster.NewMockNodeSelector(t)
	mockSelector.EXPECT().LocalName().Return("node1")
	s, err := sharding.StateFromJSON(raw, mockSelector)
	if err != nil {
		panic(err)
	}
