...
func testDistributed(t *testing.T, dirName string, batch bool) {
	...
	t.Run("query individually to check if all exist using random nodes", func(t *testing.T) {
		for _, obj := range data {
			...
			ok, err := node.repo.Exists(context.Background(), distributedClass, obj.ID, nil, nil)
			require.Nil(t, err)
			assert.True(t, ok)
		}
	})
	...
	t.Run("delete a third of the data from random nodes", func(t *testing.T) {
		for i, obj := range data {
			...
			node := nodes[rand.Intn(len(nodes))]
			err := node.repo.DeleteObject(context.Background(), distributedClass, obj.ID, nil, nil)
			require.Nil(t, err)
		}
	})
	...
	t.Run("make sure 2/3 exist, 1/3 no longer exists", func(t *testing.T) {
		for i, obj := range data {
			...
			node := nodes[rand.Intn(len(nodes))]
			actual, err := node.repo.Exists(context.Background(), distributedClass, obj.ID, nil, nil)
			require.Nil(t, err)
