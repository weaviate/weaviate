	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			index, err := NewIndex(context.Background(), IndexConfig{
				...
				hnsw.NewDefaultUserConfig(nil), nil, nil, mockSchemaGetter, nil, logger, nil, nil, nil, nil, nil, class, nil, scheduler, nil, nil, NewShardReindexerV3Noop())
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			index, err := NewIndex(ctx, IndexConfig{
				...
				hnsw.NewDefaultUserConfig(nil), nil, nil, mockSchemaGetter, nil, logger, nil, nil, nil, nil, nil, class, nil, scheduler, nil, memwatch.NewDummyMonitor(), NewShardReindexerV3Noop())
			require.NoError(t, err)
