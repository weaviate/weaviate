...
type ShardReindexTaskGeneric struct {
	...
	// trackerMkdirGuard yields the guard serializing a reindex tracker's
	// init() MkdirAll against a concurrent Index.drop AND per-shard tenant
	// delete (dropShards) — see newReindexTrackerGuarded. Defaults to
	// Index.withShardRLockGuard in NewShardReindexTaskGeneric; the
	// drain-rematerialize test wraps it to interpose a barrier at the exact
	// pre-MkdirAll point that makes the delete-vs-MkdirAll race deterministic.
	// Always set — no test-only branch runs in production.
...
func NewShardReindexTaskGeneric(name string, logger logrus.FieldLogger,
	...
	t.trackerMkdirGuard = func(shard ShardLike) func(func() error) error {
		return func(fn func() error) error {
			return shard.Index().withShardRLockGuard(shard.Name(), fn)
		}
	}
	return t
}
...
func (t *ShardReindexTaskGeneric) runShardLifecycle(ctx context.Context, shard ShardLike, skipSwap bool) error {
	...
	// the guard means the index or shard is being deleted — propagate
	// unwrapped so callers treat it as a clean stop, consistent with
	// OnAfterLsmInitAsync.
...
func (t *ShardReindexTaskGeneric) onAfterLsmInitWithTracker(ctx context.Context, shard *Shard,
	...
	if err != nil {
		if errors.Is(err, context.Canceled) {
			// Guarded route: the index or shard is being deleted and the
			// tracker dir was deliberately not created. Clean stop; return
			// unwrapped so callers can errors.Is on it.
			logger.Debug("index or shard is being deleted, stopping after-LSM-init hook")
			return err
		}
		err = fmt.Errorf("creating reindex tracker: %w", err)
		return err
...
// newReindexTrackerGuarded builds a tracker whose init()-time MkdirAll runs
// under withShardRLockGuard: DTM-driven callers hold no Index lock and can
// race Index.drop or dropShards, and an unguarded MkdirAll would re-create
// the class or shard dir the delete just removed. The guard makes it
// complete-before-removal or bail with context.Canceled (a clean stop).
//
...
func (t *ShardReindexTaskGeneric) OnAfterLsmInitAsync(ctx context.Context, shard ShardLike,
	...
	// means the index or shard is being deleted; treat as a clean stop.
	rt, err := t.newReindexTrackerGuarded(shard)
	if err != nil {
		if errors.Is(err, context.Canceled) {
			logger.Debug("index or shard is being deleted, stopping reindex drain")
