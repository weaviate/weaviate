...
// makeCleanupBuilder builds a reindex.CleanupInProgressLookupBuilder reporting a fixed
// set of (collection, shard) pairs as mid-cleanup.
func makeCleanupBuilder(inProgress map[[2]string]bool) reindex.CleanupInProgressLookupBuilder {
	return func() reindex.CleanupInProgressLookup {
...
func TestAnyLiveReindexForShard_CleanupReturnsNil(t *testing.T) {
	db := &DB{}
	db.SetShardReindexActivityLookup(makeActivityBuilder(map[[2]string]bool{}))
	db.SetReindexCleanupInProgressLookup(func() reindex.CleanupInProgressLookup { return nil })
	assert.False(t, db.AnyLiveReindexForShard("MyClass", "shard1"),
