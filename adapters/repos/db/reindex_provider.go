...
func logOperatorRepairGuidanceOnPartialSwap(logger logrus.FieldLogger, payload *ReindexTaskPayload, outcome distributedtask.TaskStatus) {
	...
	for _, propName := range payload.Properties {
		...
		if len(repairCommands) == 0 {
			...
			// replicated bytes everywhere, so a field missing here is missing
			// everywhere.
...
// CANCELLED).
//
// Delegates to [ReindexRepairCall] so the terminal-cleanup guidance and the
// schema gates can never name different repairs for the same migration. Empty
// when this build cannot name one.
func repairCommandsForFailedMigration(payload *ReindexTaskPayload, propName string) []string {
	call := ReindexRepairCall(*payload, propName)
	if call == "" {
		return nil
	}
	return []string{call}
