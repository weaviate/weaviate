...
// REST cancel path.
// autoCleanupAfterTerminal returns false when the drain timed out
// ([reindexTimings.terminalCleanupDrainTimeout]) — callers must not trust
// tracker dirs read afterwards, since the worker is still writing them.
...
// gate but invisible to conflict detection. A status this build never
// declared still answers true, since [TaskStatus.IsTerminal] is
// fail-closed — the other answer deletes those dirs while a newer node is
// still migrating, and that is the one outcome nothing downstream can undo.
