...
func TestACleanupRunThatRunsOutOfTimeIsTruncatedNotFailed(t *testing.T) {
	...
	// The budget runs out inside a shard's own load, which is where a run-wide
	// deadline lands in production: the shards that need sweeping are the ones
	// that have to be hydrated, and they queue on a shared load limiter.
	monitor := &failToLoadMonitor{nthCall: 1, onNthCall: cancel}
