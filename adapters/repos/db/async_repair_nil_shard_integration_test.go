...
func withTwoNodeReplication(t *testing.T, client replica.Client) func(*Index) {
	...
	return func(idx *Index) {
		...
		sched, err := NewAsyncReplicationScheduler(context.Background(), entreplication.GlobalConfig{
			AsyncReplicationSchedulerWorkers:           runtimecfg.NewDynamicValue(1),
			AsyncReplicationSchedulerTopologyFrequency: runtimecfg.NewDynamicValue(defaultAsyncReplicationSchedulerTopologyFrequency),
			AsyncReplicationDisabled:                   runtimecfg.NewDynamicValue(false),
		}, monitoring.GetMetrics(), logger)
		require.NoError(t, err)
		sched.Start()
...
func TestAsyncRepairObjectInsertionNilShardRetry(t *testing.T) {
	...
	t.Cleanup(func() {
		_ = s.disableAsyncReplication(context.Background())
	})

	// AsyncReplicationEnabled() returns true when ReplicationFactor > 1 and
	// async replication is not globally disabled. Set ReplicationFactor = 2
	// now — after EnableAsyncReplication has initialised the hashtree with
	// the correct config — so that initShard did not call initAsyncReplication
	// with a zero-value AsyncReplicationConfig.
	func() {
