...
func (_m *MockIndexLike) CalculateUnloadedObjectsMetrics(ctx context.Context, tenantName string) (types.ObjectUsage, error) {
	...
	var r0 types.ObjectUsage
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string) (types.ObjectUsage, error)); ok {
		return rf(ctx, tenantName)
	}
	if rf, ok := ret.Get(0).(func(context.Context, string) types.ObjectUsage); ok {
		r0 = rf(ctx, tenantName)
	} else {
		r0 = ret.Get(0).(types.ObjectUsage)
	}
