...
func Test_AddingReferencesInBatches(t *testing.T) {
	...
	}, &fakeRemoteClient{}, &fakeNodeResolver{}, &fakeRemoteNodeClient{}, &fakeReplicationClient{}, nil, memwatch.NewDummyMonitor(), cluster.NewMockNodeSelector(t), types.NewMockSchemaReader(t), replicationTypes.NewMockReplicationFSMReader(t))
	require.Nil(t, err)
	...
	t.Run("add reference between them - first batch", func(t *testing.T) {
		...
		_, err = repo.AddBatchReferences(context.Background(), refs, nil, 0)
		assert.Nil(t, err)
	})
	...
	t.Run("add reference between them - second batch including errors", func(t *testing.T) {
		...
		batchRefs, err := repo.AddBatchReferences(context.Background(), refs, nil, 0)
		assert.Nil(t, err)
