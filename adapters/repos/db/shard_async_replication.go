...
const (
	...
	defaultFrequencyWhilePropagating  = 10 * time.Second
...
func (s *Shard) initAsyncReplication(config AsyncReplicationConfig, cached hashtree.AggregatedHashTree) error {
	...
	if cached != nil {
		...
		// asyncRepWg.Wait() correctly waits for it before returning.  Without
		// tracking, a rapid disable→enable cycle (e.g. during a rebuild or a
		// repair override remove→add) would spawn a new goroutine before the
		// old one exits, leading to goroutine accumulation at scale.
		s.asyncRepWg.Add(1)
	...
	enterrors.GoWrapper(func() {
		...
		for i := 0; ; i++ {
			if err := s.index.asyncReplicationScheduler.acquireInitScanSlot(ctx); err != nil {
				return // context cancelled before a slot became available
			}
			err := s.initHashtree(ctx, effectiveConfig, bucket)
			s.index.asyncReplicationScheduler.releaseInitScanSlot()
