...
func SetupClass(t require.TestingT, repo *DB, schemaGetter *fakeSchemaGetter, logger logrus.FieldLogger, k1, b float32, preset string,
) ([]string, *Migrator) {
	vFalse := false
	vTrue := true
...
func TestBM25FJourney(t *testing.T) {
	...
	t.Run("bm25f textField non-alpha", func(t *testing.T) {
		...
		resTextField, scores, err := idx.objectSearch(context.TODO(), 1000, nil, kwrTextField, nil, nil, addit, nil, "", 0, props, nil)
		require.Nil(t, err)
	...
	t.Run("Check search with two terms", func(t *testing.T) {
		...
		res, _, err := idx.objectSearch(context.TODO(), 1000, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
		require.Nil(t, err)
		// Check results in correct order
	...
	t.Run("First result has high score", func(t *testing.T) {
		...
		res, _, err := idx.objectSearch(context.TODO(), 5, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
		require.Nil(t, err)
	...
	t.Run("With autocut", func(t *testing.T) {
		...
		resNoAutoCut, noautocutscores, err := idx.objectSearch(context.TODO(), 10, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
		require.Nil(t, err)

		resAutoCut, autocutscores, err := idx.objectSearch(context.TODO(), 10, nil, kwr, nil, nil, addit, nil, "", 1, props, nil)
		require.Nil(t, err)
...
func TestBM25FSingleProp(t *testing.T) {
	...
	props, _ := SetupClass(t, repo, schemaGetter, logger, 0.5, 100, "none")
...
func TestBM25FWithFilters(t *testing.T) {
	...
	kwr := &searchparams.KeywordRanking{Type: "bm25", Properties: []string{"description"}, Query: "journey"}
	addit := additional.Properties{}
	res, _, err := idx.objectSearch(context.TODO(), 1000, filter, kwr, nil, nil, addit, nil, "", 0, props, nil)
...
func TestBM25FWithFilters_ScoreIsIdenticalWithOrWithoutFilter(t *testing.T) {
	...
	addit := additional.Properties{}
	filtered, filteredScores, err := idx.objectSearch(context.TODO(), 1000, filter, kwr, nil, nil, addit, nil, "", 0, props, nil)
	require.Nil(t, err)
	unfiltered, unfilteredScores, err := idx.objectSearch(context.TODO(), 1000, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
	require.Nil(t, err)
...
func TestBM25FCompare(t *testing.T) {
	...
	shardNames, err := idx.schemaReader.Shards("MyClass")
	require.Nil(t, err)
	...
	for _, shardName := range shardNames {
		...
		withBM25Fobjs, withBM25Fscores, err := shard.ObjectSearch(context.TODO(), 1000, nil, kwr, nil, nil, addit, props, nil)
		require.Nil(t, err)
		...
		objs, scores, err := shard.ObjectSearch(context.TODO(), 1000, nil, kwr, nil, nil, addit, props, nil)
		require.Nil(t, err)
...
func TestBM25F_ComplexDocuments(t *testing.T) {
	...
	t.Run("single term", func(t *testing.T) {
		...
		res, scores, err := idxNone.objectSearch(context.TODO(), 10, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
		require.Nil(t, err)
	...
	t.Run("Results without stopwords", func(t *testing.T) {
		...
		resNoStopwords, resNoScores, err := idxNone.objectSearch(context.TODO(), 10, nil, kwrNoStopwords, nil, nil, addit, nil, "", 0, props, nil)
		require.Nil(t, err)
		...
		resStopwords, resScores, err := idxEn.objectSearch(context.TODO(), 10, nil, kwrStopwords, nil, nil, addit, nil, "", 0, properties, nil)
		require.Nil(t, err)
		...
		resStopwordsDuplicate, duplicateScores, err := idxEn.objectSearch(context.TODO(), 10, nil, kwrStopwordsDuplicate, nil, nil, addit, nil, "", 0, properties, nil)
		require.Nil(t, err)
...
func MultiPropClass(t require.TestingT, repo *DB, schemaGetter *fakeSchemaGetter, logger logrus.FieldLogger, k1, b float32) (string, []string) {
	vFalse := false
	vTrue := true
...
func TestBM25F_SortMultiProp(t *testing.T) {
	...
	t.Run("single term", func(t *testing.T) {
		...
		res, scores, err := idx.objectSearch(context.TODO(), 1, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
		require.Nil(t, err)
	...
	t.Run("two docs to test additional explanations", func(t *testing.T) {
		...
		res, _, err := idx.objectSearch(context.TODO(), 2, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
		require.Nil(t, err)
