...
func TestBM25FJourneyBlockAnd(t *testing.T) {
	...
	for _, location := range []string{"memory", "disk"} {
		...
		t.Run("bm25f text with AND == minimum should match with len(queryTerms) "+location, func(t *testing.T) {
			...
			kwr2 := &searchparams.KeywordRanking{Type: "bm25", Properties: []string{"title", "description"}, Query: q, SearchOperator: common_filters.SearchOperatorOr, MinimumOrTokenMatch: len(strings.Split(q, " "))}
		...
		for MinimumOrTokenMatch, expectedSize := range expectedSizes {
			t.Run("bm25f text with minimum should match with 0...len(queryTerms) "+location, func(t *testing.T) {
				kwr := &searchparams.KeywordRanking{Type: "bm25", Properties: []string{"title", "description"}, Query: "This is how we get to BM25F", MinimumOrTokenMatch: MinimumOrTokenMatch}
				res, scores, err := idx.objectSearch(context.TODO(), 1000, nil, kwr, nil, nil, addit, nil, "", 0, props)
				// Print results
				t.Log("--- Start results for search with AND ---")
				for i, r := range res {
				...
				// if MinimumOrTokenMatch < 3, title and description will both match, and thus the score will be higher
				if MinimumOrTokenMatch < 3 {
