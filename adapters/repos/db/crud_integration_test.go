...
func Test_ReferenceVectorSearch(t *testing.T) {
	rand.Seed(time.Now().UnixNano())
	dirName := t.TempDir()
	logger, _ := test.NewNullLogger()
	ctx, _ := context.WithTimeout(context.Background(), time.Minute)

	paragraphClass := &models.Class{
		VectorIndexConfig:   hnsw.NewDefaultUserConfig(),
		InvertedIndexConfig: invertedConfig(),
		Class:               "Paragraph",
		Properties: []*models.Property{
	...
	articleClass := &models.Class{
		...
		Properties: []*models.Property{
			...
			{
				Name:         "hasParagraphs",
				DataType:     []string{paragraphClass.Class},
				Tokenization: "word",
			},
		},
	}
	...
	require.Nil(t,
		migrator.AddClass(ctx, paragraphClass, schemaGetter.shardState))
	require.Nil(t,
		migrator.AddClass(ctx, articleClass, schemaGetter.shardState))

	// update schema getter so it's in sync with class
	schemaGetter.schema = schema.Schema{
		Objects: &models.Schema{
			Classes: []*models.Class{paragraphClass, articleClass},
		},
	}
	...
	t.Run("expected success", func(t *testing.T) {
		para1 := &models.Object{
			...
			ID:     strfmt.UUID("d63ee5e0-e62e-4747-8e34-1eed0a7e6aa0"),
		...
		para2 := &models.Object{
			Class:  paragraphClass.Class,
			ID:     strfmt.UUID("02234ed5-40ac-42bd-bf6e-6c29b954b33c"),
		...
		article := &models.Object{
			Class: articleClass.Class,
			ID:    strfmt.UUID("e230dad7-b762-4804-a41e-0165668d4753"),
			Properties: map[string]interface{}{
				"title": "Title of the Article",
				"hasParagraphs": models.MultipleRef{
					{Beacon: newBeacon(para1.Class, para1.ID)},
					{Beacon: newBeacon(para2.Class, para2.ID)},
				},
			},
		}

		t.Run("insert test objects", func(t *testing.T) {
			err := repo.PutObject(ctx, para1, para1.Vector)
			require.Nil(t, err)

			err = repo.PutObject(ctx, para2, para2.Vector)
			require.Nil(t, err)

			err = repo.PutObject(ctx, article, nil)
			require.Nil(t, err)
		})

		t.Run("ref vec search", func(t *testing.T) {
			refProps := map[string]struct{}{"hasParagraphs": {}}
			refVecs, err := repo.ReferenceVectorSearch(ctx, article, refProps)
			assert.Nil(t, err)
			assert.Len(t, refVecs, 2)
			assert.Equal(t, refVecs, [][]float32{para1.Vector, para2.Vector})
		})
	})

	t.Run("expected failure", func(t *testing.T) {
		nonexistentRefID := newUUID()

		t.Run("nonexistent reference object", func(t *testing.T) {
			obj := &models.Object{
				Class: articleClass.Class,
				ID:    newUUID(),
				Properties: map[string]interface{}{
					"title": "Title",
					"hasParagraphs": models.MultipleRef{
						{Beacon: newBeacon(paragraphClass.Class, nonexistentRefID)},
					},
				},
			}

			t.Run("insert test objects", func(t *testing.T) {
				err := repo.PutObject(ctx, obj, nil)
				require.Nil(t, err)
			})

			t.Run("ref vec search", func(t *testing.T) {
				refProps := map[string]struct{}{"hasParagraphs": {}}
				_, err = repo.ReferenceVectorSearch(ctx, obj, refProps)
				require.NotNil(t, err)
				expetedErr := fmt.Sprintf(
					"find object with beacon \"weaviate://localhost/Paragraph/%s\"': not found",
					nonexistentRefID)
				require.EqualError(t, err, expetedErr)
			})
		})
	})
}
