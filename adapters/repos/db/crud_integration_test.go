...
func TestCRUD(t *testing.T) {
	...
	t.Run("adding a thing", func(t *testing.T) {
		...
		err := repo.PutObject(context.Background(), thing, vector, nil, nil, nil, 0)
	...
	t.Run("adding an action", func(t *testing.T) {
		...
		err := repo.PutObject(context.Background(), action, vector, nil, nil, nil, 0)
	...
	t.Run("sorting all objects", func(t *testing.T) {
		...
		for _, td := range testData {
			...
			err := repo.PutObject(context.Background(), object, vector, nil, nil, nil, 0)
			assert.Nil(t, err)
		}
		// run sorting tests
		tests := []struct {
...
func Test_PutObject_MultiTenant(t *testing.T) {
	dirName := t.TempDir()
	...
	shardState := NewMultiTenantShardingStateBuilder().
		WithNodePrefix("node").
		WithIndexName("multi-tenant-sharding state-index").
	...
	mockSchemaReader.EXPECT().ReadOnlySchema().Return(models.Schema{Classes: nil}).Maybe()
	mockSchemaReader.EXPECT().ShardReplicas(mock.Anything, mock.Anything).Return([]string{"node1", "node2"}, nil).Maybe()
	mockReplicationFSMReader := replicationTypes.NewMockReplicationFSMReader(t)
	mockReplicationFSMReader.EXPECT().FilterOneShardReplicasRead(mock.Anything, mock.Anything, mock.Anything).Return([]string{"node1"}).Maybe()
	mockReplicationFSMReader.EXPECT().FilterOneShardReplicasWrite(mock.Anything, mock.Anything, mock.Anything).Return([]string{"node1"}, nil).Maybe()
	mockNodeSelector := cluster.NewMockNodeSelector(t)
	...
	err = migrator.AddClass(context.Background(), testClass)
	require.Nil(t, err)
	...
	ok, err := repo.Exists(context.Background(), "TestClass", "5b9e2d04-0b2d-4e4b-bd5e-84e2f70a8b49", &additional.ReplicationProperties{
	...
	err = repo.PutObject(context.Background(), &models.Object{
		Additional: nil,
		Class:      "TestClass",
		ID:         "5b9e2d04-0b2d-4e4b-bd5e-84e2f70a8b49",
		Tenant:     "foo-tenant",
	}, []float32{1.1, 2.2, 3.3, 4.4}, nil, nil, &additional.ReplicationProperties{
		ConsistencyLevel: "ONE",
	}, 0)
	require.Nil(t, err)
...
func Test_ImportWithoutVector_UpdateWithVectorLater(t *testing.T) {
	...
	t.Run("import individual objects without vector", func(t *testing.T) {
		for i := 0; i < individual; i++ {
			err := repo.PutObject(context.Background(), data[i], nil, nil, nil, nil, 0) // nil vector !
			require.Nil(t, err)
		}
	})
	...
	t.Run("perform unfiltered vector search and verify there are no matches", func(t *testing.T) {
		...
		}, []string{""}, []models.Vector{randomVector(r, 7)})
		require.Nil(t, err)
	...
	t.Run("update some of the objects to add vectors", func(t *testing.T) {
		for i := range data {
			...
			err := repo.PutObject(context.Background(), data[i], data[i].Vector, nil, nil, nil, 0)
			require.Nil(t, err)
		}
	})
...
func TestVectorSearch_ByDistance(t *testing.T) {
	...
	t.Run("insert test objects", func(t *testing.T) {
		for id, props := range tests {
			err := repo.PutObject(context.Background(), &models.Object{Class: className, ID: id}, props.inputVec, nil, nil, nil, 0)
			require.Nil(t, err)
		}
	})
...
func TestCRUDWithEmptyArrays(t *testing.T) {
	...
	t.Run("empty arrays", func(t *testing.T) {
		...
		assert.Nil(t, repo.PutObject(context.Background(), obj1, []float32{1, 3, 5, 0.4}, nil, nil, nil, 0))
	...
	t.Run("empty references", func(t *testing.T) {
		...
		assert.Nil(t, repo.PutObject(context.Background(), objRef, []float32{1, 3, 5, 0.4}, nil, nil, nil, 0))
...
func TestOverwriteObjects(t *testing.T) {
	...
	t.Run("insert stale object", func(t *testing.T) {
		err := repo.PutObject(context.Background(), stale, stale.Vector, nil, nil, nil, 0)
		require.Nil(t, err)
	})
	...
	t.Run("overwrite with fresh object", func(t *testing.T) {
		...
		shd, err := idx.shardResolver.ResolveShardByObjectID(context.Background(), fresh.ID, "", 0)
		require.Nil(t, err)
...
func TestIndexDigestObjects(t *testing.T) {
	...
	t.Run("insert test objects", func(t *testing.T) {
		err := repo.PutObject(context.Background(), obj1, obj1.Vector, nil, nil, nil, 0)
		require.Nil(t, err)
		err = repo.PutObject(context.Background(), obj2, obj2.Vector, nil, nil, nil, 0)
		require.Nil(t, err)
	})

	t.Run("get digest object", func(t *testing.T) {
		idx := repo.GetIndex(schema.ClassName(class.Class))
		shd, err := idx.shardResolver.ResolveShardByObjectID(context.Background(), obj1.ID, "", 0)
		require.Nil(t, err)
...
func TestIndexDifferentVectorLength(t *testing.T) {
	...
	t.Run("Add object with nil vector", func(t *testing.T) {
		...
		require.Nil(t, repo.PutObject(context.Background(), objNil, objNil.Vector, nil, nil, nil, 0))
	...
	t.Run("Add object with non-nil vector after nil vector", func(t *testing.T) {
		...
		require.Nil(t, repo.PutObject(context.Background(), obj1, obj1.Vector, nil, nil, nil, 0))
	})
	...
	t.Run("Add object with different vector length", func(t *testing.T) {
		...
		require.NotNil(t, repo.PutObject(context.Background(), obj2, obj2.Vector, nil, nil, nil, 0))
	...
	t.Run("Add nil object after objects with vector", func(t *testing.T) {
		...
		require.Nil(t, repo.PutObject(context.Background(), obj2Nil, obj2Nil.Vector, nil, nil, nil, 0))
