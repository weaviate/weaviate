...
func TestVectorSearch_L2Squared(t *testing.T) {
	className := "L2SquaredClass"
	var class *models.Class
	...
	t.Run("create required schema", func(t *testing.T) {
		vecIdx := hnsw.NewDefaultUserConfig()
		vecIdx.Distance = hnsw.DistanceL2Squared
	...
	t.Run("assert failure when using certainty with l2-squared configured vec index",
		func(t *testing.T) {
			t.Run("with nearObject", func(t *testing.T) {
				_, err := repo.VectorClassSearch(context.Background(), traverser.GetParams{
					ClassName:  className,
					Pagination: &filters.Pagination{Limit: filters.LimitFlagSearchByDist},
					NearObject: &searchparams.NearObject{
						Certainty: 0.9,
						ID:        "b039c0ce-866c-48fb-ba2b-b423db4f1700",
					},
