...
func TestBM25FJourneyBlock(t *testing.T) {
	...
	for _, location := range []string{"memory", "disk"} {
		t.Run("bm25f journey "+location, func(t *testing.T) {
			...
			res, scores, err := idx.objectSearch(context.Background(), 1000, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
			require.Nil(t, err)
...
func TestBM25FWithFiltersBlock(t *testing.T) {
	...
	for _, location := range []string{"memory", "disk"} {
		...
		t.Run("bm25f with filter matching no docs "+location, func(t *testing.T) {
			kwr := &searchparams.KeywordRanking{Type: "bm25", Properties: []string{"description"}, Query: "journey"}
			addit := additional.Properties{}
			res, _, err := idx.objectSearch(context.TODO(), 1000, filterEmpty, kwr, nil, nil, addit, nil, "", 0, props, nil)
...
func TestBM25F_SortMultiPropBlock(t *testing.T) {
	...
	for _, location := range []string{"memory", "disk"} {
		...
		t.Run("single term "+location, func(t *testing.T) {
			...
			res, scores, err := idx.objectSearch(context.TODO(), 2, nil, kwr, nil, nil, addit, nil, "", 0, props, nil)
			require.Nil(t, err)
...
func TestBM25FWithFiltersMemtable(t *testing.T) {
	...
	for i, location := range []string{"memory", "disk"} {
		t.Run("bm25f with filter "+location, func(t *testing.T) {
			...
			addit := additional.Properties{}
			res, scores, err := idx.objectSearch(context.TODO(), 1000, filter, kwr, nil, nil, addit, nil, "", 0, props, nil)
		...
		t.Run(location, func(t *testing.T) {
			for _, tc := range tests {
				t.Run(tc.name, func(t *testing.T) {
					kwr := &searchparams.KeywordRanking{Type: "bm25", Properties: []string{"title"}, Query: "my unrelated journey", AdditionalExplanations: true}
					addit := additional.Properties{}
					res, scores, err := idx.objectSearch(context.TODO(), 1000, tc.filter, kwr, nil, nil, addit, nil, "", 0, props, nil)
