...
func TestUnloadLocalShard_ErrorClassification(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			// Our own close ended the retry, so this is the index closing rather
			// than a refusal a later attempt could clear.
		...
		{
			// The classifier does not read the close cause, so both rows run the same
			// arm. This one drives errIndexDropped, so a later per-cause branch cannot
			// add a sixth value to an enum that ships as a metric label unnoticed.
			name: "a dropped index refuses the unload the same way",
			setup: func(t *testing.T, index *Index, shardName string) (string, context.Context) {
		...
		},
		{
			...
			// Only cancellation is dropped there.
