...
func (ob *objectsBatcher) storeAdditionalStorageWithAsyncQueue(ctx context.Context) {
	...
	var vectors []common.VectorRec
	var targetVectors map[string][]common.VectorRec
	hasTargetVectors := ob.shard.hasTargetVectors()
	if hasTargetVectors {
		targetVectors = make(map[string][]common.VectorRec)
	} else {
		vectors = make([]common.VectorRec, 0, len(ob.objects))
	}
