...
func (sg *fakeMigrationSchemaGetter) TenantsShardsWithActivation(ctx context.Context, class string, tenants ...string) (map[string]string, uint64, error) {
	res, err := sg.TenantsShards(ctx, class, tenants...)
	return res, 0, err
}

func (sg *fakeMigrationSchemaGetter) TenantsShardsFromLeader(class string, tenants ...string) (map[string]string, uint64, error) {
	return sg.TenantsShardsWithActivation(context.Background(), class, tenants...)
}
