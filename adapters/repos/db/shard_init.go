...
func NewShard(ctx context.Context, promMetrics *monitoring.PrometheusMetrics,
	...
	reindexer ShardReindexerV3, implicitShardLoading bool,
) (_ *Shard, err error) {
	start := time.Now()
	...
	if err := s.initNonVector(ctx, class, implicitShardLoading); err != nil {
		return nil, errors.Wrapf(err, "init shard %q", s.ID())
	}

	if err = s.initShardVectors(ctx, implicitShardLoading); err != nil {
