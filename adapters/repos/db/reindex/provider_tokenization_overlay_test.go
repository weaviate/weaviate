...
// is the subtle one; see [reindex.MaybeClearTokenizationOverlayOnAllFailed].

// fireAllPropHooks simulates a swap loop where every prop flipped.
func fireAllPropHooks(tasks []*reindex.ShardReindexTaskGeneric, props []string) int {
	fired := 0
...
func TestMaybeWirePerPropOverlaySet_TokenizationChange_WiresAndSets(t *testing.T) {
	s := &db.Shard{}
	tasks := []*reindex.ShardReindexTaskGeneric{{}}
	...
	require.True(t, reindex.MaybeWirePerPropOverlaySet(s, payload, tasks),
...
func TestMaybeWirePerPropOverlaySet_NonTokenizationMigration_NoOp(t *testing.T) {
	for _, mt := range []reindex.ReindexMigrationType{
		...
		t.Run(string(mt), func(t *testing.T) {
			...
			require.False(t, reindex.MaybeWirePerPropOverlaySet(s, payload, tasks),
...
func TestMaybeWirePerPropOverlaySet_NilInputs_NoOp(t *testing.T) {
	...
	// shard.
	require.False(t, reindex.MaybeWirePerPropOverlaySet(nil, &reindex.ReindexTaskPayload{}, nil))
	require.False(t, reindex.MaybeWirePerPropOverlaySet(&db.Shard{}, nil, nil))
}
...
func TestMaybeWirePerPropOverlaySet_NilTaskInSlice_Skipped(t *testing.T) {
	...
	s := &db.Shard{}
	tasks := []*reindex.ShardReindexTaskGeneric{nil, {}}
	payload := &reindex.ReindexTaskPayload{
		MigrationType:      reindex.ReindexTypeChangeTokenization,
		TargetTokenization: "field",
		Properties:         []string{"name"},
...
func TestMaybeClearTokenizationOverlayOnAllFailed_AllFailed_Clears(t *testing.T) {
	...
	wasSet := reindex.MaybeWirePerPropOverlaySet(s, payload, tasks)
...
func TestMaybeClearTokenizationOverlayOnAllFailed_NilInputs_NoOp(t *testing.T) {
	...
	require.False(t, reindex.MaybeClearTokenizationOverlayOnAllFailed(&db.Shard{}, nil, true, false))
}
