...
func (sc retokenizeScenario) run(t *testing.T) map[string][]uint64 {
	...
	var task *reindex.ShardReindexTaskGeneric
	var bucketName string
	if sc.filterable {
		bucketName = helpers.BucketFromPropNameLSM(propName)
		task, _ = newFilterableRetokenizeTask(t, f.Logger(), className, propName, sc.targetTok)
	} else {
		bucketName = helpers.BucketSearchableFromPropNameLSM(propName)
		preStrategy := shard.Store().Bucket(bucketName).Strategy()
		task, _ = newSearchableRetokenizeTask(t, f.Logger(), className, propName, sc.targetTok, preStrategy)
	}
