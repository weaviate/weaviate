...
func newRangeableInMemoryTestTask(t *testing.T, logger logrus.FieldLogger, className, propName string, failing *atomic.Bool, calls *atomic.Int32,
) (*reindex.ShardReindexTaskGeneric, *testFilterableToRangeableStrategyWrapper) {
	t.Helper()
	task, wrapped := newFilterableToRangeableTask(t, logger, className, propName)
	task.SetRebuildRangeableRepFn(func(ctx context.Context, b *lsmkv.Bucket) error {
		calls.Add(1)
		if failing.Load() {
...
// newRangeableFinalizeTestShard builds a shard+index fixture with the
// rangeable in-memory knob enabled.
func newRangeableFinalizeTestShard(t *testing.T, classNamePrefix string) (context.Context, *db.Shard, *db.IndexFixture, string) {
	t.Helper()
	ctx := testCtx()
	...
	shd, _, f := testShardWithSettings(t, ctx, class, enthnsw.UserConfig{Skip: true},
		false, false, false,
		func(idx *db.Index) { idx.Config.IndexRangeableInMemory = true })
	shard := shd.(*db.Shard)
	t.Cleanup(func() { shard.Shutdown(ctx) })
	return ctx, shard, f, className
}

// putRangeableTestObjects loads numObjects test objects into shard.
func putRangeableTestObjects(t *testing.T, ctx context.Context, shard *db.Shard, className string, numObjects int) {
	t.Helper()
...
func runReindexToCompletionOrError(t *testing.T, ctx context.Context, task *reindex.ShardReindexTaskGeneric, shard *db.Shard) error {
	t.Helper()
...
func setupRangeableFinalizeDegradeFixture(t *testing.T, classNamePrefix string) (
	context.Context, *db.Shard, *db.IndexFixture, string, *reindex.ShardReindexTaskGeneric,
	*testFilterableToRangeableStrategyWrapper, *atomic.Bool, *atomic.Int32, *test.Hook,
	...
	ctx, shard, f, className := newRangeableFinalizeTestShard(t, classNamePrefix)

	// The task captures its logger by value at construction, so build it
	// with a hook-capturing logger to assert on the ERROR log this fixture
	// drives.
	...
	task, wrapped := newRangeableInMemoryTestTask(t, hookLogger, className, propName, failing, calls)
	...
	return ctx, shard, f, className, task, wrapped, failing, calls, hook
}
...
func TestRangeableFinalize_RebuildCancellation_RoutesTransient(t *testing.T) {
	propName := filterableToRangeablePropName
	ctx, shard, f, className := newRangeableFinalizeTestShard(t, "RangeableRebuildCancel_")
	...
	task2.SetRebuildRangeableRepFn(func(ctx context.Context, b *lsmkv.Bucket) error {
	...
	err := task2.RebuildRangeableInMemoryReps(cancelledCtx, f.Logger(), shard, []string{propName})
	require.Error(t, err)
...
func TestRangeableFinalize_DataWorkFailure_StillFAILED(t *testing.T) {
	propName := filterableToRangeablePropName
	ctx, shard, f, className := newRangeableFinalizeTestShard(t, "RangeableDataWorkFail_")
	putRangeableTestObjects(t, ctx, shard, className, 5)
...
func TestRangeableFinalize_MultiReplica_FailedReplicaServesCorrectDiskResults(t *testing.T) {
	...
	newReplica := func(classNamePrefix string) (context.Context, *db.Shard, *db.IndexFixture, string) {
		ctx, shard, f, className := newRangeableFinalizeTestShard(t, classNamePrefix)
		putRangeableTestObjects(t, ctx, shard, className, numObjects)
		return ctx, shard, f, className
	}
	...
	ctxA, shardA, fA, classNameA := newReplica("RangeableMultiReplicaA_")
	taskA, wrappedA := newFilterableToRangeableTask(t, fA.Logger(), classNameA, propName)
	...
	// side effect.
	ctxB, shardB, fB, classNameB := newReplica("RangeableMultiReplicaB_")
	failing := &atomic.Bool{}
	failing.Store(true)
	calls := &atomic.Int32{}
	taskB, wrappedB := newRangeableInMemoryTestTask(t, fB.Logger(), classNameB, propName, failing, calls)
	...
	taskB2, _ := newFilterableToRangeableTask(t, fB.Logger(), classNameB, propName)
	fB.SetShardReindexer(&testShardReindexer{task: taskB2})

	shdB2, err := fB.InitShard(ctxB, shardName, newFilterableToRangeableTestClass(classNameB), true, true)
	require.NoError(t, err)
	shardB2 := shdB2.(*db.Shard)
	fB.StoreShard(shardName, shdB2)
...
func TestRebuildRangeableInMemoryReps_NilBucketRoutesContextCancellation(t *testing.T) {
	propName := filterableToRangeablePropName
	ctx, shard, f, className := newRangeableFinalizeTestShard(t, "RangeableNilBucketCtxCancel_")
	putRangeableTestObjects(t, ctx, shard, className, 5)
	...
	err := task.RebuildRangeableInMemoryReps(cancelledCtx, f.Logger(), shard, []string{missingPropName})
	require.Error(t, err)
	assert.ErrorIs(t, err, context.Canceled,
...
func TestRebuildRangeableInMemoryReps_NilBucketDegradesWithoutCancellation(t *testing.T) {
	propName := filterableToRangeablePropName
	ctx, shard, f, className := newRangeableFinalizeTestShard(t, "RangeableNilBucketDegrade_")
	...
	err := task.RebuildRangeableInMemoryReps(ctx, hookLogger, shard, []string{missingPropName})
