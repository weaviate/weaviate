...
func newEnableFilterableTask(t *testing.T, f *db.IndexFixture, className, propName string) (*reindex.ShardReindexTaskGeneric, *testEnableFilterableStrategyWrapper) {
	t.Helper()
	...
	task := reindex.NewShardReindexTaskGeneric(
		"EnableFilterable", f.Logger(), wrapped,
...
func TestRecoveryConvergence_EnableFilterable_Baseline(t *testing.T) {
	...
	task, wrapped := newEnableFilterableTask(t, f, className, propName)
	require.NoError(t, task.RunReindexOnlyOnShard(ctx, shard))
	require.NoError(t, task.RunPrepareOnShard(ctx, shard))
	require.NoError(t, task.RunSwapOnShard(ctx, shard))
...
func TestRecoveryConvergence_EnableFilterable_FromEachState(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			task2, _ := newEnableFilterableTask(t, f, className, propName)
