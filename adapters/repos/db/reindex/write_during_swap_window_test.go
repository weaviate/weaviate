...
type swapWindowScenario struct {
	inject func(t *testing.T, ctx context.Context, shard *db.Shard, objs []*storobj.Object)
}
...
func (sc swapWindowScenario) run(t *testing.T) *lsmkv.Bucket {
	...
	task, _ := newFilterableToRangeableTask(t, f.Logger(), className, filterableToRangeablePropName)

	injected := false
	origSwap := task.ProcessOneSwapPropFn
	task.ProcessOneSwapPropFn = func(ctx context.Context, store *lsmkv.Store, rt reindex.ReindexTracker, propIdx int, prop string) (*lsmkv.Bucket, error) {
...
// for [reindex.ResolveDoubleWriteBucket] against the exact store state SwapBucketPointer
...
func TestResolveDoubleWriteBucket_FallsBackAfterSwap(t *testing.T) {
	...
	require.Same(t, shard.Store().Bucket(sidecarName),
		reindex.ResolveDoubleWriteBucket(shard, sidecarName, canonicalName),
	...
	require.Nil(t, reindex.ResolveDoubleWriteBucket(shard, sidecarName, ""),
