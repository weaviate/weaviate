...
func TestPropertyValuesFromInvertedCutoff(t *testing.T) {
	...
	// a searchable-only property under ingest holds unflushed writes almost
	// always; the store has no objects bucket, so falling back to the scan
	// would error instead
	t.Run("searchable-only property answers with writes still unflushed", func(t *testing.T) {
		sbName := helpers.BucketSearchableFromPropNameLSM("topic")
		require.NoError(t, store.CreateOrLoadBucket(ctx, sbName,
			lsmkv.WithStrategy(lsmkv.StrategyInverted),
			lsmkv.WithUseBloomFilter(true)))
		sb := store.Bucket(sbName)
		...
		res, err := ua.propertyValuesFromInverted(ctx, aggregation.ParamProperty{
			Name: "topic", Aggregators: topOccs, TopOccurrencesCutoff: 10,
		}, schema.DataTypeText)
		require.NoError(t, err)
		require.False(t, res.TextAggregation.CutoffExceeded)
		require.True(t, res.TextAggregation.ValuesComplete)
