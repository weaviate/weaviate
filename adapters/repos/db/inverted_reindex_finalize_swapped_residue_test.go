...
// Covers the window between swapped.mig (canonical dir already renamed to
// backup_<gen>) and markTidied, where promotion is the deferred second half
// of a committed swap, not a decision. The merged-only window is covered by
// TestFinalize_MergedResiduePromotionScoping.
...
// A committed swap must promote regardless of task liveness or schema, or
// shard init creates an empty canonical bucket and the property serves zero
// rows.
...
// Promotion must write tidied.mig before renaming, so a crash between the
// two doesn't leave a state that looks merged-only again.
...
// Promotion must leave a finalized marker in place of the removed tracker,
// or a still-running task for that generation can't tell "startup did my
// work" from "never ran" and fails the migration on correct data. Markers
// below the promoted generation must be swept in the same pass.
...
// The sweep must be narrow: only the promoted namespace's markers below the
// promoted generation are removed.
...
// A marker outlives its generation (numbers are reused, and a namespace
// with no tracker dir is never swept), so a live tracker dir must win over
// a stale marker sharing its name.
