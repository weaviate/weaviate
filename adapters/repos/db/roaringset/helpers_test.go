...
// BenchmarkBitmapFactory_GetBitmap measures GetBitmap throughput, including
// under concurrent maxId growth (simulating inserts).
...
func TestBitmapFactoryUniverse(t *testing.T) {
	...
	t.Run("the first object survives a query against the empty shard", func(t *testing.T) {
		...
		// a deny-list filter runs before anything is written; the universe is
		// empty and consumers skip unresolvable ids at resolution time (the
		// factory deliberately has no shrink operation — see the comment at
		// the factory).
		assert.Empty(t, universe(t, bmf))
