...
type Shard struct {
	...
	// self-clearing on the next write for any path that misses it), and by the
	// property-update apply that removes the very bucket this would send
	// writes to. Not by an index DELETE naming this index: the schema already
	// advertises it as disabled, so that request does no RAFT write at all.
	//
