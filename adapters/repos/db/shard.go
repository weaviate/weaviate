...
type Shard struct {
	...
	// asyncRepLastErrorByTarget tracks the timestamp of the last RPC failure per
	// target node. When time.Since(t) < cfg.frequency the target is skipped for
	// one cycle so a dead node does not block every hashbeat with a full timeout.
	asyncRepLastErrorByTarget map[string]time.Time
