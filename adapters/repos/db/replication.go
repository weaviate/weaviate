...
func (i *Index) ReplicateObject(ctx context.Context, shard, requestID string, object *storobj.Object) entrep.SimpleResponse {
	i.backupStateLock.RLock()
	defer i.backupStateLock.RUnlock()
	localShard, ok := i.Shards[shard]
	if !ok {
		return entrep.SimpleResponse{Errors: []string{fmt.Sprintf("shard not found")}}
	}
	return localShard.preparePutObject(ctx, requestID, object)
}

func (i *Index) ReplicateDeletion(ctx context.Context, shard, requestID string, uuid strfmt.UUID) entrep.SimpleResponse {
	i.backupStateLock.RLock()
	defer i.backupStateLock.RUnlock()
	localShard, ok := i.Shards[shard]
	if !ok {
		return entrep.SimpleResponse{Errors: []string{"shard not found"}}
	}
	return localShard.prepareDeleteObject(ctx, requestID, uuid)
}

func (i *Index) ReplicateObjects(ctx context.Context, shard, requestID string, objects []*storobj.Object) entrep.SimpleResponse {
	i.backupStateLock.RLock()
	defer i.backupStateLock.RUnlock()
	localShard, ok := i.Shards[shard]
	if !ok {
...
func (i *Index) CommitReplication(ctx context.Context, shard, requestID string) entrep.SimpleResponse {
	i.backupStateLock.RLock()
	defer i.backupStateLock.RUnlock()
	localShard, ok := i.Shards[shard]
	if !ok {
