...
func TestExecuteResolutionPlanIdxLoopIntegration(t *testing.T) {
	...
	t.Run("one bitmap returns MaskRootLeaf fast-path without cursor scan", func(t *testing.T) {
		...
		result, release, err := newPlanExecutor(plan, bitmapsByPath, bucket, ops, concurrency.SROAR_MERGE, nil).execute(context.Background())
		require.NoError(t, err)
		defer release()
	...
	t.Run("context already cancelled returns error", func(t *testing.T) {
		...
		_, _, err := newPlanExecutor(plan, bitmapsByPath, bucket, ops, concurrency.SROAR_MERGE, nil).execute(ctx)
		require.Error(t, err)
	})
