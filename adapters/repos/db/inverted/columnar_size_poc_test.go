...
func TestColumnarIndexPOC_ResidentSize(t *testing.T) {
	...
	numIdx, err := columnar.BuildFromBucket(numeric.bucket, uint64(numeric.numDocs+1), false)
	require.NoError(t, err)

	textF := newContainsFixture(t, n)
	textBucket := textF.store.Bucket(helpers.BucketFromPropNameLSM(benchPropName))
	textIdx, err := columnar.BuildFromBucket(textBucket, uint64(textF.numDocs+1), false)
	require.NoError(t, err)
