...
func (b *BM25Searcher) wandBlock(
	...
	MinimumOrTokenMatchByProperty := make([]int, 0, len(params.Properties))
	...
	for _, tokenization := range helpers.Tokenizations {
		...
		if len(propNames) > 0 {
			...
			for _, propName := range propNames {
				...
				MinimumOrTokenMatchByProperty = append(MinimumOrTokenMatchByProperty, MinimumOrTokenMatch)
	...
	for i, perProperty := range allResults {
		...
		for j := range perProperty {
			...
			if len(allResults[i][j]) < MinimumOrTokenMatchByProperty[i] {
				continue
			}
			...
			eg.Go(func() (err error) {
				...
				if params.SearchOperator == common_filters.SearchOperatorAnd {
					topKHeap = lsmkv.DoBlockMaxAnd(internalLimit, allResults[i][j], averagePropLength, params.AdditionalExplanations, len(termCounts[i]), MinimumOrTokenMatchByProperty[i])
				} else {
					topKHeap = lsmkv.DoBlockMaxWand(internalLimit, allResults[i][j], averagePropLength, params.AdditionalExplanations, len(termCounts[i]), MinimumOrTokenMatchByProperty[i])
				}
