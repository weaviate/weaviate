	...
	for i, perProperty := range allResults {
		...
		for j := range perProperty {
			...
			eg.Go(func() (err error) {
				...
				if params.SearchOperator == common_filters.SearchOperatorAnd {
					topKHeap = lsmkv.DoBlockMaxAnd(internalLimit, allResults[i][j], averagePropLength, params.AdditionalExplanations, len(termCounts[i]), minimumOrTokensMatchByProperty[i], ctx, b.logger)
				} else {
					topKHeap = lsmkv.DoBlockMaxWand(internalLimit, allResults[i][j], averagePropLength, params.AdditionalExplanations, len(termCounts[i]), minimumOrTokensMatchByProperty[i], ctx, b.logger)
				}
