...
func (s *Shard) CreateReplicaSnapshot(ctx context.Context, owner string, stagingRoot string) (files []string, err error) {
	if err := s.HaltForTransfer(ctx, owner, false, 0); err != nil {
		return nil, fmt.Errorf("halt for replica snapshot: %w", err)
	}
	defer func() {
		if rerr := s.resumeMaintenanceCycles(context.Background(), owner); rerr != nil && err == nil {
