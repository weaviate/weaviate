...
func (s *Shard) cleanStaleMigrationDirs(propName, indexType string) {
	reindex.CleanStaleMigrationDirsAt(s.pathLSM(), propName, indexType, s.index.logger)
}
...
func (s *Shard) CleanStalePartialReindexState(ctx context.Context, propName, indexType string) error {
	...
	if classDir, ok := reindex.ClassLevelMigrationDirForIndexType(indexType); ok {
		preservePrefixes = append(preservePrefixes, classDir)
	}
	preserveSidecars := reindex.CompletedMigrationSidecarSuffixes(s.pathLSM(), preservePrefixes)
...
// those in `preserveSidecars` (from [reindex.CompletedMigrationSidecarSuffixes]):
