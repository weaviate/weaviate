...
// The teardown half of the waiver, observed where it is observable: the gate is
// raised across the drain, so a blocked drain holds it for as long as the
// teardown is applicable. Only the rollback's shape must skip that hold.
//
// FAILED is in the table because a failed task means a worker reported a
// failure, so something ran no matter what the unit map says; the waiver is
// deliberately narrower than "terminal with nothing claimed".
func TestTeardownSkipsTheGateOnlyForACancelThatClaimedNothing(t *testing.T) {
	desc := distributedtask.TaskDescriptor{ID: "task-teardown", Version: 1}
	payload := &ReindexTaskPayload{
		Collection:    journeyCollection,
		Properties:    []string{"body"},
		MigrationType: ReindexTypeChangeTokenization,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// A handle that never closes keeps the drain blocked, which is the
			// window in which the hold is visible from outside.
			...
			if !tc.wantHold {
				...
				require.Equal(t, ReindexHoldNone, p.HoldForShard(journeyCollection, journeyShard),
					"nothing was written, so nothing may be held")
				return
			}

			require.Eventually(t,
				func() bool { return p.HoldForShard(journeyCollection, journeyShard) == ReindexHoldCleanup },
