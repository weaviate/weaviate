...
type Index struct {
	...
	namespace         string
	// lastRefusal holds the sentinel namespaceState last logged, or nil once a read
	// succeeds. objectVectorSearch's local arms and initLazyShardsInBackground reach
	// namespaceState at once, so it is atomic.
...
func (i *Index) loadLocalShardIfActive(shardName string) (monitoring.WarmupOutcome, error) {
	...
	// loop counts an error as a failed load and logs an Error line per shard.
	state, err := i.namespaceState()
	if err != nil {
		// The refusal is counted rather than dropped, so the sweep's close line
		// says how many shards it refused. namespaceState wrote the operator's line.
...
// has already committed. A namespace that refuses this caller, and a state no case
// covers, both load none and return nil rather than erroring. The schema change
// stands either way, and the shard is materialized by whatever next loads it. A
// namespace that cannot be looked up still errors.
