...
type flat struct {
	...
	rq                  compressionhelpers.RotationalQuantizer
...
func New(cfg Config, uc flatent.UserConfig, store *lsmkv.Store) (*flat, error) {
	...
	if uc.RQ.Enabled && uc.RQ.Cache {
		index.rqCache = cache.NewShardedByteLockCache(
...
func (flat *flat) getRQVector(ctx context.Context, id uint64) ([]byte, error) {
	key := flat.pool.byteSlicePool.Get(8)
	defer flat.pool.byteSlicePool.Put(key)
	binary.BigEndian.PutUint64(key.slice, id)
	bytes, err := flat.store.Bucket(flat.getCompressedBucketName()).Get(key.slice)
...
func (index *flat) Add(ctx context.Context, id uint64, vector []float32) error {
	...
	index.trackDimensionsOnce.Do(func() {
		...
		if index.isRQ() {
			index.rq = *compressionhelpers.NewRotationalQuantizer(int(size), 42, 8, index.distancerProvider)
		}
	})
	...
	if index.isRQ() {
		...
		index.storeCompressedVector(id, vectorRQ)
	}
...
func (index *flat) searchByVectorRQ(ctx context.Context, vector []float32, k int, allow helpers.AllowList) ([]uint64, []float32, error) {
	...
	if index.isRQCached() {
		if err := index.findTopVectorsCachedRQ(heap, allow, rescore, vectorRQ); err != nil {
			return nil, nil, err
		}
	} else {
		if err := index.findTopVectors(heap, allow, rescore,
...
func (index *flat) createDistanceCalcRQ(vectorRQ []byte) distanceCalc {
	return func(vecAsBytes []byte) (float32, error) {
		return index.rq.DistanceBetweenCompressedVectors(vecAsBytes, vectorRQ)
	}
}
...
func (index *flat) findTopVectorsCachedRQ(heap *priorityqueue.Queue[any],
	allow helpers.AllowList, limit int, vectorRQ []byte,
) error {
	var id uint64
	allowMax := uint64(0)
	...
	out := make([][]byte, index.rqCache.PageSize())
...
func (index *flat) PostStartup() {
	...
	if index.isRQCached() {
		...
		// however want some kind of relationship to the machine size, so
		// 2*GOMAXPROCS seems like a good default.
		it := compressionhelpers.NewParallelIterator[byte](bucket, 2*runtime.GOMAXPROCS(0),
