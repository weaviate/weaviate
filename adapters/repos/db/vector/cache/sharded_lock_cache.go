...
func (s *shardedLockCache[T]) GetMultiple(ctx context.Context, docID uint64, vecID uint64) ([]float32, error) {
	panic("not implemented")
}

func (s *shardedLockCache[T]) MultiGetMultiple(ctx context.Context, docIDs []uint64, vecIDs []uint64) ([][]float32, []error) {
	panic("not implemented")
}

func (s *shardedLockCache[T]) PreloadMultiple(docID uint64, vecID uint64, vec []float32) {
	panic("not implemented")
}

func (s *shardedLockCache[T]) PrefetchMultiple(docID uint64, vecID uint64) {
	panic("not implemented")
}
...
func NewShardedMultiFloat32LockCache(vecForID common.VectorForID[float32], multipleVecForID common.MultipleVectorForID[float32], maxSize int,
	logger logrus.FieldLogger, normalizeOnRead bool, deletionInterval time.Duration,
	allocChecker memwatch.AllocChecker,
) Cache[float32] {
	var vecForIDValue common.VectorForID[float32] = nil
