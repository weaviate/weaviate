...
// locked situation
//GW
//GWfunc (h *hnsw) growIndexToAccomodateNode(id uint64, logger logrus.FieldLogger) error {
func (h *gemini) growIndexToAccomodateNode(id uint64, logger logrus.FieldLogger) error {
//GW
	before := time.Now()
...
func growIndexToAccomodateNode(index []*vertex, id uint64,
	...
	//GW logger.WithField("action", "hnsw_grow_index").
	logger.WithField("action", "gemini_grow_index").
    //GW
		WithField("took", took).
