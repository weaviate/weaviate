...
//GW
//GW func newVectorCachePrefiller(cache cache, index *hnsw,
func newVectorCachePrefiller(cache cache, index *gemini,
//GW
	logger logrus.FieldLogger,
...
func (pf *vectorCachePrefiller) logLevel(level, count int, before time.Time) {
	pf.logger.WithFields(logrus.Fields{
        //GW
		//GW"action":     "hnsw_vector_cache_prefill_level",
		"action":     "gemini_vector_cache_prefill_level",
        //GW
		"hnsw_level": level,
		"count":      count,
...
func (pf *vectorCachePrefiller) logTotal(count, limit int, before time.Time) {
	pf.logger.WithFields(logrus.Fields{
        //GW
		//GW"action":   "hnsw_vector_cache_prefill",
		"action":   "gemini_vector_cache_prefill",
        //GW
		"limit":    limit,
		"count":    count,
		"took":     time.Since(before),
