...
import (
    ...
    //GW
	//GW ent "github.com/weaviate/weaviate/entities/vectorindex/hnsw"
	ent "github.com/weaviate/weaviate/entities/vectorindex/gemini"
    //GW
)

// Dump to stdout for debugging purposes
//GW func (h *hnsw) Dump(labels ...string) {
func (h *gemini) Dump(labels ...string) {
//GW
	if len(labels) > 0 {
		fmt.Printf("--------------------------------------------------\n")
		fmt.Printf("--  %s\n", strings.Join(labels, ", "))
...
//GW func (h *hnsw) DumpJSON(labels ...string) {
func (h *gemini) DumpJSON(labels ...string) {
//GW
	dump := JSONDump{
		Labels:              labels,
		ID:                  h.id,
...
//GW func NewFromJSONDump(dumpBytes []byte, vecForID VectorForID) (*hnsw, error) {
func NewFromJSONDump(dumpBytes []byte, vecForID VectorForID) (*gemini, error) {
//GW
	var dump JSONDump
...
//GW func NewFromJSONDumpMap(dumpBytes []byte, vecForID VectorForID) (*hnsw, error) {
func NewFromJSONDumpMap(dumpBytes []byte, vecForID VectorForID) (*gemini, error) {
//GW
...
//GW func (h *hnsw) ValidateLinkIntegrity() {
func (h *gemini) ValidateLinkIntegrity() {
//GW
	h.RLock()
	defer h.RUnlock()
