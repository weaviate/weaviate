...
//GW func (h *hnsw) PauseMaintenance(ctx context.Context) error {
func (h *gemini) PauseMaintenance(ctx context.Context) error {
//GW
...
//GW func (h *hnsw) SwitchCommitLogs(ctx context.Context) error {
func (h *gemini) SwitchCommitLogs(ctx context.Context) error {
//GW
...
//GW func (h *hnsw) ListFiles(ctx context.Context) ([]string, error) {
func (h *gemini) ListFiles(ctx context.Context) ([]string, error) {
//GW
	var (
		//GW logRoot = filepath.Join(h.commitLog.RootPath(), fmt.Sprintf("%s.hnsw.commitlog.d", h.commitLog.ID()))
		logRoot = filepath.Join(h.commitLog.RootPath(), fmt.Sprintf("%s.gemini.commitlog.d", h.commitLog.ID()))
        //GW
	...
	if err != nil {
		//GW return nil, errors.Errorf("failed to list files for hnsw commitlog: %s", err)
		return nil, errors.Errorf("failed to list files for gemini commitlog: %s", err)
        //GW
	}
...
//GW func (h *hnsw) ResumeMaintenance(ctx context.Context) error {
func (h *gemini) ResumeMaintenance(ctx context.Context) error {
//GW
	h.tombstoneCleanupCycle.Start()
	h.commitLog.Start()
	return nil
