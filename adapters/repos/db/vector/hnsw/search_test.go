...
func TestDistance(t *testing.T) {
	...
	t.Run("test SearchByVector", func(t *testing.T) {
		for _, vec := range vectors {
			_, dists, err := vectorIndex.SearchByVector(context.TODO(), vec, 10, nil)
			require.Nil(t, err)
			for _, d := range dists {
				assert.True(t, d >= 0)
			}
	...
	t.Run("test SearchByVectorDistance", func(t *testing.T) {
		for _, vec := range vectors {
			_, dists, err := vectorIndex.SearchByVectorDistance(context.TODO(), vec, 0.5, 10, nil)
			require.Nil(t, err)
			for _, d := range dists {
				assert.True(t, d >= 0)
			}
