...
func TestMultiVectorScoringWithViewUsesConfiguredDistanceProvider(t *testing.T) {
	vectors := [][][]float32{
		{{2, 2}},
		{{1, 0}},
	}
	index := newTestMultiVectorDistancerIndex(t, "multi-vector-cosine-view", distancer.NewCosineDistanceProvider(), vectors)
	index.TempMultiVectorForIDWithViewThunk = func(
		ctx context.Context, id uint64, container *common.VectorSlice, view common.BucketView,
	) ([][]float32, error) {
		return [][]float32{append([]float32(nil), vectors[id][0]...)}, nil
	}

	distance, err := index.computeScoreWithView(
		context.TODO(), [][]float32{{1, 0}}, 0, &common.VectorSlice{}, &noopBucketView{},
	)
...
func TestAcornPercentage(t *testing.T) {
	...
	t.Run("check acorn params on different filter percentags", func(t *testing.T) {
		vectorIndex.configuredFilterStrategy.Store(int32(SWEEPING))
		allowList := helpers.NewAllowList(1, 2, 3)
		useAcorn := vectorIndex.acornEnabled(allowList)
		assert.False(t, useAcorn)

		vectorIndex.configuredFilterStrategy.Store(int32(ACORN))
