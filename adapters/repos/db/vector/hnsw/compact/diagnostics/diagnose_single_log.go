...
func main() {
	...
	sfw, err := compact.NewSafeFileWriter(testPath, compact.DefaultBufferSize)
	if err != nil {
	...
	sortedWalReader := compact.NewWALCommitReader(sortedReader, logger)
	sortedMemReader := compact.NewInMemoryReader(sortedWalReader, logger)
	...
	walReader3 := compact.NewWALCommitReader(logReader3, logger)
	for {
	...
	sortedWalReader3 := compact.NewWALCommitReader(sortedReader3, logger)
	for {
