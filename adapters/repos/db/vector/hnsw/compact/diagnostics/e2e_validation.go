...
func main() {
	...
	merger, err := compact.NewNWayMerger(iterators, logger)
	if err != nil {
		panic(err)
	}
	...
	mergedSfw, err := compact.NewSafeFileWriter(mergedPath, compact.DefaultBufferSize)
	if err != nil {
		panic(fmt.Errorf("create safe file writer for %s: %w", mergedPath, err))
	}
	mergedWriter := compact.NewWALWriter(mergedSfw.Writer())
	...
	mergedWalReader := compact.NewWALCommitReader(mergedReader, logger)
	mergedMemReader := compact.NewInMemoryReader(mergedWalReader, logger)
	...
	snapshotIterators := make([]compact.IteratorLike, len(sortedFiles))
	...
	snapshotMerger, err := compact.NewNWayMerger(snapshotIterators, logger)
	if err != nil {
		panic(err)
	}
	...
	snapshotSfw, err := compact.NewSafeFileWriter(snapshotPath, compact.DefaultBufferSize)
	if err != nil {
		panic(fmt.Errorf("create safe file writer for %s: %w", snapshotPath, err))
	}
	snapshotWriter := compact.NewSnapshotWriter(snapshotSfw.Writer())
...
func writeCommit(w *compact.WALWriter, c compact.Commit) error {
	switch commit := c.(type) {
	case *compact.AddNodeCommit:
		return w.WriteAddNode(commit.ID, commit.Level)
	case *compact.SetEntryPointMaxLevelCommit:
	...
	case *compact.ClearLinksCommit:
		return w.WriteClearLinks(commit.ID)
	case *compact.ClearLinksAtLevelCommit:
	...
	case *compact.ResetIndexCommit:
		return w.WriteResetIndex()
	case *compact.AddPQCommit:
		return w.WriteAddPQ(commit.Data)
	case *compact.AddSQCommit:
		return w.WriteAddSQ(commit.Data)
	case *compact.AddRQCommit:
		return w.WriteAddRQ(commit.Data)
	case *compact.AddBRQCommit:
		return w.WriteAddBRQ(commit.Data)
	case *compact.AddMuveraCommit:
