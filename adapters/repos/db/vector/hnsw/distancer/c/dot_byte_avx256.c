...
void dot_byte_256(unsigned char *a, unsigned char *b, unsigned int *res,
                  long *len) {
  int n = *len;
  ...
  for (i = 0; i + 31 < n; i += 32) {
    ...
    __m256i a_low = _mm256_bslli_epi128(
        vec_a, 1); // left 1 byte = low to high in each 16-bit element
