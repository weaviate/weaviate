...
// locked situation
func (h *hnsw) growIndexToAccomodateNode(id int64, logger logrus.FieldLogger) error {
...
func growIndexToAccomodateNode(index []*vertex, id int64,
	logger logrus.FieldLogger) ([]*vertex, error) {
	before := time.Now()
	previousSize := int64(len(index))
	if id < previousSize {
