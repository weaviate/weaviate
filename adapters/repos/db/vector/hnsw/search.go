...
func (h *hnsw) searchLayerByVectorWithDistancer(ctx context.Context,
	queryVector []float32,
	entrypoints *priorityqueue.Queue[any], ef int, level, callerId int,
	...
	for candidates.Len() > 0 {
		...
		for _, neighborID := range connectionsReusable {
			...
			if h.compressed.Load() {
				...
				distance, err = h.distanceToFloatNode(floatDistancer, callerId, neighborID)
			}
			if err != nil {
			...
			if distance < worstResultDistance || results.Len() < ef {
				...
				if h.compressed.Load() {
					...
					h.cache.Prefetch(callerId, candidates.Top().ID)
				}
...
func (h *hnsw) currentWorstResultDistanceToFloat(results *priorityqueue.Queue[any],
	...
	if results.Len() > 0 {
		...
		d, err := h.distanceToFloatNode(distancer, callerId, id)
		if err != nil {
			var e storobj.ErrNotFound
			if errors.As(err, &e) {
...
func (h *hnsw) distanceFromBytesToFloatNode(concreteDistancer compressionhelpers.CompressorDistancer, nodeID uint64) (float32, error) {
	...
	vec, err := h.TempVectorForIDThunk(context.Background(), -1, nodeID, slice)
	if err != nil {
		var e storobj.ErrNotFound
		if errors.As(err, &e) {
...
func (h *hnsw) distanceToFloatNode(distancer distancer.Distancer, callerId int, nodeID uint64) (float32, error) {
	candidateVec, err := h.vectorForID(context.Background(), callerId, nodeID)
	if err != nil {
		return 0, err
	}
...
func (h *hnsw) knnSearchByVector(ctx context.Context, searchVec []float32, k int,
	...
	for level := maxLayer; level >= 1; level-- {
		...
		res, err := h.searchLayerByVectorWithDistancer(ctx, searchVec, eps, 1, level, callerId, nil, compressorDistancer)
		if err != nil {
	...
	res, err := h.searchLayerByVectorWithDistancer(ctx, searchVec, eps, ef, 0, callerId, allowList, compressorDistancer)
	if err != nil {
...
func (h *hnsw) QueryVectorDistancer(queryVector []float32) common.QueryVectorDistancer {
	...
	if h.compressed.Load() {
		...
		f := func(nodeID uint64) (float32, error) {
			...
			callerId := h.cache.GetFreeCallerId()
			defer h.cache.ReturnCallerId(callerId)
			return h.distanceToFloatNode(distancer, callerId, nodeID)
		}
