...
func Test_NoRace_ManySmallCommitlogs(t *testing.T) {
	...
	t.Run("verify that the loaded index is unmodified", func(t *testing.T) {
		for i, node := range index.nodes {
			if node == nil {
				assert.Nil(t, oldNodes[i])
				continue
			}
			assert.Equal(t, len(oldNodes[i].connections), len(node.connections))
			for level, conns := range node.connections {
				assert.Equal(t, len(oldNodes[i].connections[level]), len(conns))
				for slot, x := range conns {
					assert.Equal(t, oldNodes[i].connections[level][slot], x)
				}
			}
		}
	})
