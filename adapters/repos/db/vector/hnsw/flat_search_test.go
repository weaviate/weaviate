...
func Test_NoRaceCompressionRecallWithDeletes(t *testing.T) {
	// This test is like Test_NoRaceCompressionRecall, but it also deletes some
	// vectors,so we have tombstones. They are deleted from both the index and
	...
	for _, concurrency := range []int{1, 2, 4, 8} {
		t.Run(fmt.Sprintf("concurrency %d", concurrency), func(t *testing.T) {
			...
			uc.EF = 256
			truths := make([][]uint64, queries_size)
			// delete every 3rd vector before building the ground truths
