...
`)
	...
	vectors := readSiftFloat(siftFile, 100)

	for n := 0; n < 5; n++ { // increase if you don't want to reread SIFT for every run
		...
		for j := 0; j < 10; j++ {
			...
			for i := 0; i < numGoroutines; i++ {
				wg.Add(1)
				goroutineIndex := i * numGoroutines
				go func() {
					defer wg.Done()
					for j := 0; j < 10; j++ {
