...
func TestRecallDoesNotDrop(t *testing.T) {
	...
	index.Delete(delete_indices...)
	control := make([][]uint64, queries_size)
	for i, v := range queries {
		control[i], _, _ = index.SearchByVector(v, k, nil)
	}
	index.Compress(dimensions, 256, false, int(ssdhelpers.UseKMeansEncoder), int(ssdhelpers.LogNormalEncoderDistribution))
	//index.CleanUpTombstonedNodes(neverStop)
	recall := float32(0)
	for i, v := range queries {
		res, _, err := index.SearchByVector(v, k, nil)
		assert.Nil(t, err)
		recall += float32(testinghelpers.MatchesInLists(control[i], res))
	}
	recall = recall / float32(queries_size*k)
	fmt.Println(recall)
