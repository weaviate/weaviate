...
/*
type noopCondensor struct{}

func (c *noopCondensor) Do(_ string) error {
	return nil
}
*/
...
func NewCommitLogger(rootPath, name string, logger logrus.FieldLogger,
	...
	l := &hnswCommitLogger{
		rootPath:  rootPath,
		id:        name,
		condensor: NewMemoryCondensor(logger), //&noopCondensor{},
		logger:    logger,
...
func getCommitFileNames(rootPath, name string, createdAfter int64) ([]string, error) {
	...
	if createdAfter > 0 {
		files, err = removeOldCommitFiles(dir, files, createdAfter)
		if err != nil {
...
func getLastSnapshot(rootPath, name string) (snap *DeserializationResult, ts int64, err error) {
	dir := commitLogDirectory(rootPath, name)
	err = os.MkdirAll(dir, os.ModePerm)
	if err != nil {
	...
	for i := len(files) - 1; i >= 0; i-- {
		...
		if snap == nil {
			snapFile, err := os.Open(path)
			if err != nil {
				return nil, 0, errors.Wrapf(err, "open snapshot file %q", path)
			}
			...
			if err != nil {
				return nil, 0, errors.Wrapf(err, "read snapshot file %q", path)
			}
...
func removeOldCommitFiles(dirPath string,
