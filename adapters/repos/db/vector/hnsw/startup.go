...
func (h *hnsw) init(cfg Config) error {
	h.pools = newPools(h.maximumConnectionsLayerZero, h.visitedListCollisionRate)
...
func (h *hnsw) restoreFromDisk(cl CommitLogger) error {
	...
	h.pools.visitedLists = newVisitedPool(len(h.nodes)+512, h.visitedListCollisionRate)
