...
func (s *BRQSettings) Description() string {
	return fmt.Sprintf("BRQ(%d,%d)", s.QueryBits, s.DataBits)
}
...
func NewBRQNeighborProvider(data [][]float32, settings BRQSettings, distance distancer.Provider) *BRQNeighborProvider {
	d := len(data[0])
	quantizer := compressionhelpers.NewBinaryRotationalQuantizer(d, settings.DataBits, settings.QueryBits, 42, distance)
...
func BenchmarkQuantizationRecall(b *testing.B) {
	...
	algorithms := []QuantizationSettings{
		...
		// &RQSettings{Bits: 8},
		&BQSettings{},
		&BRQSettings{DataBits: 1, QueryBits: 1},
		&BRQSettings{DataBits: 1, QueryBits: 2},
		&BRQSettings{DataBits: 2, QueryBits: 2},
		// &PQSettings{Centroids: 256, SegmentLength: 8, BitsPerEntry: 1, TrainingSize: 10_000},
		// &PQSettings{Centroids: 256, SegmentLength: 4, BitsPerEntry: 2, TrainingSize: 10_000},
	}
