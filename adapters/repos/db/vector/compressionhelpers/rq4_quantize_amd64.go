...
// rq4MinMaxSumAVX2 runs the fused min/max/sum kernel on the largest
// multiple-of-16 prefix and folds in the tail with Go. See rq4QuantCorrAVX2
// for why the tail only runs in tests.
func rq4MinMaxSumAVX2(xs []float32) (float32, float32, float32) {
	n := len(xs) &^ 15
	if n == 0 {
...
// rq4QuantCorrAVX2 runs the fused quantize+correlate kernel on the largest
...
func rq4QuantCorrAVX2(ci []int32, xs []float32, invStep, offset float32) (float32, int32, int32) {
	n := len(xs) &^ 15
	var sumXC float32
