...
func TestFilteredSearchReplicaDedupe(t *testing.T) {
	...
	// The returned ids alone pin the invariant: ResultSet.Insert does not
	// deduplicate by id, so a replica scored twice (v lives in both
	// postings) would appear twice in the result. w1 and w2 each live in
	// exactly one posting, so an exact [vID, w2ID, w1ID] — v once, in
	// distance order (v ~0.0004, w2 0.64, w1 14.44) — proves both postings
	// were scanned AND v's replica was collapsed by the posting-scan
	// visited set.
