...
func NewPool(initialSize int, listSetSize int, maxStorage int) *Pool {
	...
	p := &Pool{
		...
		listSets:    make([]visited.ListSet, initialSize), // make enough room
		maxStorage:  maxStorage,
	}

	for i := 0; i < initialSize; i++ {
		p.listSets[i] = visited.NewList(listSetSize)
	}
...
// return.
func (p *Pool) Borrow() visited.ListSet {
	p.Lock()
