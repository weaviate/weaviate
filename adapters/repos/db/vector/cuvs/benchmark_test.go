...
var datasets = map[string]string{
	"sift-128-euclidean": "/home/ajit/datasets/sift-128-euclidean.hdf5",
}
...
func BenchmarkCuvsSift128Euclidean(b *testing.B) {
	runbooks := []string{
		"datasets/sift-128-euclidean/simple_runbook.yaml",
	}
...
Ex: go test -v -benchmem -bench ^BenchmarkHnswNeurips23$ -download`, step.Dataset)
					...
					for i := 0; i < b.N; i++ {
						index := createEmptyCuvsIndexForTests(b, idVectorSize(len(vectors[0])))
