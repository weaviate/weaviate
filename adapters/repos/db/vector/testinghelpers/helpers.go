...
// with real (non-noop) objects/nonObjects cyclemanager groups so background
// compaction/flush callbacks behave like production rather than a dummy
// store's no-ops.
...
// NewTestObjectsBucket is NewTestObjectsStore, returning just the objects
// bucket for callers that don't need the store itself.
func NewTestObjectsBucket(t testing.TB) *lsmkv.Bucket {
	t.Helper()
	return NewTestObjectsStore(t).Bucket(helpers.ObjectsBucketLSM)
}

// PutTestObject stores an object marshalled exactly as the write path does, so a
