...
func Test_NoRaceQuantizedVectorCompressor(t *testing.T) {
	t.Run("loading and deleting data works", func(t *testing.T) {
		compressor, err := ssdhelpers.NewBQCompressor(distancer.NewCosineDistanceProvider(), 1e12, nil, testinghelpers.NewDummyStore(t))
		assert.Nil(t, err)
	...
	t.Run("distance are right when using BQDistancer", func(t *testing.T) {
		...
		defer compressor.ReturnDistancer(distancer)
