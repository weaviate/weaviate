...
const (
	...
	defaultAsyncReplicationSchedulerTopologyFrequency = 30 * time.Second
	maxMaxWorkers                                     = 20
)
...
type AsyncReplicationScheduler struct {
	...
	// initScanSem gates how many hashtree init-scan goroutines run their
	// full on-disk object scan concurrently. On restart with many tenants,
	// each shard without a persisted hashtree spawns one goroutine; without
	// this limit 10k concurrent scans would thrash disk and spike RSS.
	initScanSem *semaphore.Weighted
	...
	// topologyFrequency controls how often the topology watcher
	// checks for newly-alive replica nodes. Polled on each tick so changes
	// take effect without a restart.
	topologyFrequency *configRuntime.DynamicValue[time.Duration]
	...
	// for topologyFrequency so the dispatcher wakes up promptly
...
func NewAsyncReplicationScheduler(
	...
	topologyFreq := replicationCfg.AsyncReplicationSchedulerTopologyFrequency
	if topologyFreq == nil {
		topologyFreq = configRuntime.NewDynamicValue(defaultAsyncReplicationSchedulerTopologyFrequency)
	}
	...
	s := &AsyncReplicationScheduler{
		...
		topologyFrequency:        topologyFreq,
		asyncReplicationDisabled: asyncReplicationDisabled,
		initScanSem:              semaphore.NewWeighted(maxMaxWorkers),
		metrics:                  m,
		logger:                   logger,
	}
	heap.Init(&s.h)
...
// acquireInitScanSlot blocks until a slot in the init-scan semaphore is
// available or ctx is cancelled. Returns an error if ctx is done first.
func (sched *AsyncReplicationScheduler) acquireInitScanSlot(ctx context.Context) error {
	return sched.initScanSem.Acquire(ctx, 1)
}

// releaseInitScanSlot returns a slot to the init-scan semaphore.
func (sched *AsyncReplicationScheduler) releaseInitScanSlot() {
	sched.initScanSem.Release(1)
}

// Close shuts down the scheduler and waits for all goroutines to exit.
// Registered shards should be deregistered before calling Close, but
// concurrent Register/Deregister calls that escape via ctx.Done() are safe.
...
// Returns 24h if the heap is empty. Returns topologyFrequency if
...
func (sched *AsyncReplicationScheduler) dispatchDueLocked() {
	...
	for len(sched.h) > 0 && !sched.h[0].nextRunAt.After(now) {
		entry := sched.h[0]
		if entry.inFlight {
			// Should never happen: an inFlight entry must not be in the heap.
			// Pop and reset so the shard is not permanently orphaned: with the
			// old "pop and continue" the entry would stay in sched.entries with
			// inFlight=true forever — no result would arrive, the shard would
			// never be re-enqueued, and Deregister+asyncRepWg.Wait() could
			// deadlock if asyncRepWg.Add had already been called for this entry.
			// Resetting inFlight and re-pushing lets the shard resume normally.
			...
			if sched.logger != nil {
				sched.logger.
					...
					Error("dispatchDueLocked: inFlight entry found in heap — this is a bug; resetting and re-queuing")
			}
			continue
		}
...
func (sched *AsyncReplicationScheduler) nextInterval(cfg AsyncReplicationConfig, entry *asyncSchedulerEntry, result asyncSchedulerResult) time.Duration {
	if result.err != nil {
		if entry.shard.asyncRepCtx != nil && errors.Is(result.err, entry.shard.asyncRepCtx.Err()) {
...
// topologyWatcher checks once per topologyFrequency whether any
...
func (sched *AsyncReplicationScheduler) topologyWatcher() {
	timer := time.NewTimer(sched.topologyFreq())
	defer timer.Stop()
	...
	for {
		...
		case <-timer.C:
			...
			timer.Reset(sched.topologyFreq())
		}
	}
}

// topologyFreq returns the configured topology-check interval,
// clamping to defaultAsyncReplicationSchedulerTopologyFrequency when the runtime-config value
// is zero or negative. This prevents a busy-spin if the value is set to 0 via
// a runtime override.
func (sched *AsyncReplicationScheduler) topologyFreq() time.Duration {
	if d := sched.topologyFrequency.Get(); d > 0 {
		return d
	}
	return defaultAsyncReplicationSchedulerTopologyFrequency
}
...
func (sched *AsyncReplicationScheduler) checkTopology() {
	...
	for _, s := range shards {
		// allAliveHostnames and getLastComparedHosts are safe to call without
		// the scheduler lock: they only read shard fields that are structurally
		// immutable after init (s.index, s.index.router) or guarded by their
		// own per-shard mutex (lastComparedHosts).
		...
		// Re-acquire the lock only at the point of effect. A shard deregistered
		// between the snapshot and here must not be reprioritised or have its
		// baseline updated; reprioritizeNowLocked already guards via entries[s],
		// but the setLastComparedNodes call must also be skipped.
		sched.mu.Lock()
		_, stillRegistered := sched.entries[s]
		if stillRegistered {
			if hasNew {
...
func (sched *AsyncReplicationScheduler) adjustWorkers(n int) {
	...
	if delta > 0 {
		...
		// under-provisioned. This is safe because adjustWorkers is serialised
		// by workersMu, so no concurrent shrink can be adding new tokens.
		for {
			select {
			case <-sched.scaleDownCh:
			default:
