...
func (s *Shard) CompareDigests(ctx context.Context, sourceDigests []types.RepairResponse) ([]types.RepairResponse, error) {
	...
	for _, d := range sourceDigests {
		...
		if err != nil {
			if stderrors.Is(err, entlsmkv.Deleted) {
				...
				if stderrors.As(err, &errDeleted) && !errDeleted.DeletionTime().IsZero() {
				...
				case models.ReplicationConfigDeletionStrategyNoAutomatedResolution:
					...
					// neither propagates nor deletes — leave the conflict for manual handling.
					continue
			...
			if stderrors.Is(err, entlsmkv.NotFound) {
		...
		if v == nil {
			// Safety guard: treat unexpected nil as absent.
			result = append(result, types.RepairResponse{ID: d.ID, UpdateTime: 0})
			continue
		}
		...
		if d.UpdateTime > localTime {
			// Source has a strictly newer version — this shard is stale.
