...
func (t *ShardInvertedReindexTask_SpecifiedIndex) ObjectsIterator(shard ShardLike) objectsIterator {
	...
	return func(ctx context.Context, fn func(object *storobj.Object) error, className string) error {
		cursor := objectsBucket.Cursor()
		defer cursor.Close()

		i := 0
		for k, v := cursor.First(); k != nil; k, v = cursor.Next() {
			obj, err := storobj.FromBinaryOptionalWithClassName(v, shard.Index().Config.ClassName.String(), additional.Properties{}, propsExtraction)
			if err != nil {
