...
func (s *Shard) DeleteObjectBatch(ctx context.Context, uuids []strfmt.UUID, dryRun bool) objects.BatchSimpleObjects {
	...
	for _, uuid := range uuids {
		lock, ok := s.batchDeletePatchLocks[uuid]
		if !ok {
			newLock := sync.Mutex{}
			newLock.Lock()
			defer newLock.Unlock()
			s.batchDeletePatchLocks[uuid] = &newLock
			defer delete(s.batchDeletePatchLocks, uuid)
		} else {
			lock.Lock()
			defer lock.Unlock()
		}
...
func (s *Shard) FindUUIDs(ctx context.Context, filters *filters.LocalFilter) ([]strfmt.UUID, error) {
	...
	for _, doc := range docs {
		...
		if err != nil {
			...
			s.index.logger.WithField("op", "shard.find_uuids").WithField("docID", doc).WithError(err).Info("could not find UUID for docID")
			continue
		}
		uuids[currIdx] = uuid
		currIdx++
