...
func (l *LazyLoadShard) SetAsyncReplicationEnabled(ctx context.Context, config *models.ReplicationAsyncConfig, enabled bool) error {
	if err := l.Load(ctx); err != nil {
		return err
	}
	return l.shard.SetAsyncReplicationEnabled(ctx, config, enabled)
}
