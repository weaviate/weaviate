...
func TestNullArrayClass(t *testing.T) {
	...
	for _, name := range names {
		t.Run("add nil object via "+name, func(t *testing.T) {
			...
			if name == names[0] {
				assert.Nil(t, repo.PutObject(context.Background(), arrayObjNil, []float32{1}, nil, nil, 0, nil))
				assert.Nil(t, repo.PutObject(context.Background(), arrayObjEmpty, []float32{1}, nil, nil, 0, nil))
