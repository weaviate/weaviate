...
func (i *Index) usageForCollection(ctx context.Context, shardConcurrency int, exactObjectCount bool, vectorConfig map[string]models.VectorConfig) (*types.CollectionUsage, error) {
	...
	//
	// After collecting all local shards from the sharding state, we process them with a bounded number of concurrent
	// readers. The concurrency is capped to limit disk pressure on big clusters (this replaces the previous per-shard
	// jitter sleep). Each shard locks itself individually against changes in the _local_ state (i.e. loading/unloading)
...
// usageForShard computes usage for a single local shard, locking it against local-state changes
// (loading/unloading) for the duration of the calculation. A nil *ShardUsage with a nil error means
// the shard should be skipped (transitional states like FREEZING/OFFLOADING, which are handled
// elsewhere). It is safe to call concurrently for distinct shards.
