...
type Finder struct {
	...
	digestsPool sync.Pool // pooled []Digest scratch buffers for CollectShardDifferences
}
...
type ShardDiffSkipState struct {
	...
	HadWork bool
	// InCooldown skips the target entirely for one cycle after an RPC failure,
	// preventing a dead node from blocking every hashbeat with a full timeout.
	InCooldown bool
}
...
//
// In both cases the full tree walk (up to Height additional RPCs) is avoided.
func (f *Finder) CollectShardDifferences(ctx context.Context,
...
) (diffReader *ShardDifferenceReader, failedTargets []string, err error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("%w : class %q shard %q", err, f.class, shardName)
	}

	// Obtain a scratch buffer from the pool. The buffer is shared across all
	// replica attempts within this call (the closure is called sequentially, not
	// concurrently) and returned to the pool when CollectShardDifferences returns.
	// ht.Level writes only digests[0..n-1] (selected by the discriminant) and
	// LevelDiff reads the same positions, so stale content beyond index n is never
	// accessed — no zeroing needed before reuse.
	leavesCount := hashtree.LeavesCount(ht.Height())
	var digestsBufP *[]hashtree.Digest
	if v := f.digestsPool.Get(); v != nil {
		digestsBufP = v.(*[]hashtree.Digest)
		if len(*digestsBufP) < leavesCount {
			*digestsBufP = make([]hashtree.Digest, leavesCount)
		}
	} else {
		buf := make([]hashtree.Digest, leavesCount)
		digestsBufP = &buf
	}
	digests := *digestsBufP
	defer f.digestsPool.Put(digestsBufP)
	...
	collectDiffForTargetNode := func(targetNodeAddress, targetNodeName string) (*ShardDifferenceReader, error) {
		...
		for l := 0; l <= ht.Height(); l++ {
			...
			if l == 0 && n > 0 {
				...
				// Height level RPCs when a skip condition is met.
	...
	for i, targetNodeAddress := range replicasHostAddrs {
		...
		// Skip targets in cooldown (failed last cycle) so a dead node does not
		// block the entire hashbeat iteration with a full diffTimeoutPerNode wait.
		if prior, ok := skipStateByTarget[targetNodeName]; ok && prior.InCooldown {
			continue
		}
		...
		if err != nil {
			...
			if !errors.Is(err, ErrNoDiffFound) {
				failedTargets = append(failedTargets, targetNodeName)
				ec.Add(err)
			}
			continue
		}

		return diffReader, failedTargets, nil
	}

	err = ec.ToError()
	if err != nil {
		return nil, failedTargets, err
	}
	...
	if hadUnchanged {
		return &ShardDifferenceReader{}, failedTargets, ErrHashtreeRootUnchanged
	}

	return &ShardDifferenceReader{}, failedTargets, ErrNoDiffFound
}
