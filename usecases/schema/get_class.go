...
import (
	...
	configv2 "github.com/weaviate/weaviate/usecases/config/v2"
)
...
type ClassGetter struct {
	...
	schemaRetrievalStrategy *configv2.FeatureFlag[int]
}
...
func NewClassGetter(schemaRetrievalStrategy config.SchemaRetrievalStrategy, schemaParser *Parser, schemaManager SchemaManager, schemaReader SchemaReader, logger logrus.FieldLogger) *ClassGetter {
	return &ClassGetter{
		...
		schemaManager:           schemaManager,
		logger:                  logger,
		schemaRetrievalStrategy: configv2.NewFeatureFlag[int]("collection-retrieval-strategy", int(schemaRetrievalStrategy)).WithEnvDefault("COLLECTION_STRATEGY_RETRIEVAL").WithLogOnChange(logger),
	}
}
...
func (cg *ClassGetter) getClasses(names []string) (map[string]versioned.Class, error) {
	...
	case config.LocalOnly:
		...
		// This should never happen, but in case the strategy gets expanded and we miss adding a new case we cover on default case
	default:
		return cg.getClassesLeaderOnly(names)
	}
}
