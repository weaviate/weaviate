...
type cronsRegistration[T comparable] struct {
	...
	// jobLogger is logger with the job field. Derived once, so the loop and
	// the registrant cannot end up naming the same job two ways.
...
func newCronsRegistration[T comparable](cfg cronsRegistrationConfig[T]) (*cronsRegistration[T], error) {
	...
	// Nothing else in this package reads it, and an empty key prefix-matches
	// every runtime config field name for the cluster that does.
...
// start launches the registration loop and reports whether it did. A job its
// enable gate turns off gets no goroutine, so wait returns at once. tickGate
// is consulted per tick. A job every node runs passes cron.RunOnEveryNode.
func (c *cronsRegistration[T]) start(cr *gocron.Cron, tickGate func() bool,
	tick func(context.Context),
) (started bool, err error) {
	// Caught here because none of them fails loudly later: a nil gate or
	// tick panics on every tick, where initGoCron's Recover chain swallows
	// it, and a nil channel parks the loop with no output.
	if tickGate == nil {
...
func (c *cronsRegistration[T]) loop(cr *gocron.Cron, tickGate func() bool,
	...
	for {
		...
		case value := <-c.valueCh:
			...
			// than going quiet for a full run.
			if !c.runMu.TryLock() {
			...
			if err != nil {
				c.jobLogger.WithField("schedule", spec).Errorf("cron job not added: %v", err)
				continue
			}
