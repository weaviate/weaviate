...
func (m *Provider) UpdateVector(ctx context.Context, object *models.Object,
	...
	if !ok {
		return fmt.Errorf("vector index config (%T) is not of type HNSW, "+
			"but objects manager is restricted to HNSW", idxCfg)
	}

	if vectorizerName == config.VectorizerModuleNone {
		err = validateVectorPresent(object, hnswConfig, logger)
		if err != nil {
			return err
		}
