...
func (m *Manager) authorize(ctx context.Context, principal *models.Principal, verb string, skipAudit bool, resources ...string) error {
	...
	for _, resource := range uniqueResources {
		// Note: Namespace access is now granted via explicit namespace-admin-{ns} roles
		// which are auto-assigned when creating namespace-bound users.
		// This ensures proper RBAC with audit trails.
