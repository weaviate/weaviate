...
func TestCoordinator_RestoreShutdownAbortsParticipants(t *testing.T) {
	...
	fc.client.On("Abort", any, nodes[1], abortReq).Return(nil).Once()
	fc.backend.On("HomeDir", any, any, backupID).Return("bucket/" + backupID)
	// Restore first reads the global descriptor to see whether a cancellation is
	// already in progress; there is none here.
	fc.backend.On("GetObject", any, backupID, GlobalRestoreFile).Return(nil, backup.ErrNotFound{})
...
func TestRestorer_ShutdownCancelsLocalRestore(t *testing.T) {
	...
	metadata := backup.BackupDescriptor{
		...
		Classes: []backup.ClassDescriptor{{
			...
			Chunks:        map[int32][]string{1: {"dir1/file1"}},
	...
	// The chunk download hands us the ctx the restore actually runs on and then
	// blocks on it. If that ctx is not wired to shutdownCtx the call blocks
	// forever and waitForCompletion times out.
	restoreCtxCh := make(chan context.Context, 1)
	backend.On("Read", mock.Anything, nodeHome, chunkKey(cls, 1), mock.Anything).
		Run(func(args mock.Arguments) {
			callCtx := args.Get(0).(context.Context)
			select {
			case restoreCtxCh <- callCtx:
			default:
			}
			<-callCtx.Done()
		}).
		Return(int64(0), context.Canceled).Maybe()
	...
	select {
	case restoreCtx = <-restoreCtxCh:
	case <-time.After(2 * time.Second):
		t.Fatal("restore goroutine did not reach the chunk download in time")
	}
	require.NoError(t, restoreCtx.Err(), "restore ctx must be alive while the restore runs")

	// Fire shutdown while the restore is mid-download.
	m.Shutdown()

	require.ErrorIs(t, restoreCtx.Err(), context.Canceled,
		"Handler.Shutdown must cancel the ctx the local restore runs on")

	st := m.restorer.waitForCompletion(req.Backend, req.ID, 40, 50)
	assert.NotEqual(t, backup.Transferring, st.Status,
		"restore must terminate after Handler.Shutdown propagates ctx cancellation")
	assert.NotEmpty(t, st.Err, "the aborted restore must report why it stopped")
}
