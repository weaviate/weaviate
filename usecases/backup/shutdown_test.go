...
func TestRestorer_ShutdownCancelsLocalRestore(t *testing.T) {
	...
	// The chunk download hands us the ctx the restore runs on and blocks on it;
	// shutdown should propagate cancellation.
	...
	case <-time.After(2 * time.Second):
		t.Fatal("restore did not reach the chunk download in time")
	}
	...
	assert.NotEqual(t, backup.Transferring, st.Status,
		"restore must terminate once shutdown cancels its ctx")
