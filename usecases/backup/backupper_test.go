...
func TestBackupOnStatus(t *testing.T) {
	...
	t.Run("GetBackupProvider", func(t *testing.T) {
		...
		m := createManager(t, nil, nil, ErrAny)
	...
	t.Run("MetadataNotFound", func(t *testing.T) {
		...
		m := createManager(t, nil, backend, nil)
		got := m.OnStatus(ctx, &req)
...
func TestManagerCoordinatedBackup(t *testing.T) {
	...
	t.Run("BackendUnregistered", func(t *testing.T) {
		...
		bm := createManager(t, nil, nil, backendError)
	...
	t.Run("ClassNotBackupable", func(t *testing.T) {
		...
		bm := createManager(t, sourcer, backend, nil)
...
func createManager(t *testing.T, sourcer Sourcer, backend modulecapabilities.BackupBackend, backendErr error) *Handler {
	...
	return NewHandler(logger, mocks.NewMockAuthorizer(), "node1", sourcer, backends, fakeRbacBackupWrapper{}, fakeRbacBackupWrapper{})
}
