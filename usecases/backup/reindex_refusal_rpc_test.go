...
func TestReindexRefusalSurvivesTheCanCommitRPC(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantKind: CanCommitErrReindexStateUnknown,
		},
		{
			name: "genuine reindex, sentinel wrapped in the text",
			refusal: fmt.Errorf("%w: collection %q has an active runtime-reindex task in DTM",
		...
		},
		{
			...
			refusal: errors.Join(
				fmt.Errorf("%w: one", backup.ErrBackupBlockedByInFlightReindex),
				fmt.Errorf("%w: two", backup.ErrBackupBlockedByInFlightReindex),
			),
			wantKind: CanCommitErrInFlightReindex,
		},
	}
...
func TestReindexRefusalRebuildKeepsTheParticipantsFirstLine(t *testing.T) {
	...
	t.Run("genuine keeps the prefix byte for byte", func(t *testing.T) {
		...
		require.Equal(t, genuine, rebuilt.Error(),
			"already self-describing, so the coordinator adds nothing")
		require.Equal(t, 1, strings.Count(rebuilt.Error(), sentinel),
	...
	t.Run("unknown state is not prefixed with a claim it denies", func(t *testing.T) {
		rebuilt := canCommitErrFromResponse(&CanCommitResponse{
			Err: unknown, ErrKind: CanCommitErrReindexStateUnknown,
		})
