...
const MaxChunkSize = 1024 * 1024 * 100 // 1 GB
...
func (z *zip) WriteShard(ctx context.Context, sd *entBackup.ShardDescriptor, firstChunkForShard bool, preCompressionSize *atomic.Int64) (int64, *SplitFile, error) {
	var written int64
	...
	if firstChunkForShard {
		for _, x := range [3]struct {
			...
			n, err := z.writeOne(ctx, info, x.absPath, bytes.NewReader(x.data))
			if err != nil {
				return written, nil, err
			}
			written += n
	...
	n, sizeExceededInfo, err := z.WriteRegulars(ctx, sd, preCompressionSize)
	written += n

	return written, sizeExceededInfo, err
}

func (z *zip) WriteRegulars(ctx context.Context, sd *entBackup.ShardDescriptor, preCompressionSize *atomic.Int64) (int64, *SplitFile, error) {
	// Process files in sd.Files and remove them as we go (pop from front).
	firstFile := true
	written := int64(0)
	for len(sd.Files) > 0 {
		relPath := sd.Files[0]
		sd.Files = sd.Files[1:] // remove processed element from slice
...
func (z *zip) WriteSplitFile(ctx context.Context, splitFile *SplitFile, preCompressionSize *atomic.Int64) (*SplitFile, error) {
	amountToWrite := min(splitFile.FileInfo.Size()-splitFile.AlreadyWritten, MaxChunkSize)
