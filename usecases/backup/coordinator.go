...
func canCommitErrFromResponse(resp *CanCommitResponse, classes []string) error {
	...
	case CanCommitErrCreateReindexUndetermined:
		return backupUndeterminedByParticipant()
	default:
		...
		g.Go(func() error {
			...
			if err != nil {
				if isReindexRefusal(err) {
					...
					// node, except the configuration refusal, which is one node's TTL
					// setting. Equal ranks keep the first answer, so two nodes reporting
