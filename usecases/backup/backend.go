				...
				eg.Go(func() error {
					...
					for shard := range sender {
						...
						for {
							...
							shards, preCompressionSize, err := u.compress(ctx, desc.Name, chunk, shard, filesInShard, firstChunk, overrideBucket, overridePath, sourcePath)
							if err != nil {
								return err
							}
...
func (u *uploader) compress(ctx context.Context,
	...
	zip, reader, err := NewZip(sourcePath, u.Level, chunkTargetSize, minIndividualFileSize)
	if err != nil {
