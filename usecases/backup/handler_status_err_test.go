...
// opWithID matches the operation the uploader minted for id. The Sourcer takes a
// backup.Op, and only the uploader knows the fence it minted, so the id is the part
// a caller can pin.
func opWithID(id string) interface{} {
	return mock.MatchedBy(func(op backup.Op) bool { return op.ID == id })
}
...
func TestHandlerOnStatusServesTheReasonAFailedUploadPublished(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sourcer := &fakeSourcer{}
			sourcer.On("BackupDescriptors", mock.Anything, opWithID(backupID), []string{class}, mock.Anything).
				Return((<-chan backup.ClassDescriptor)(descriptors))
			sourcer.On("ReleaseBackup", mock.Anything, opWithID(backupID), class).Return(nil)
...
func TestHandlerOnStatusServesTheReasonAfterTheCreateGoroutineExits(t *testing.T) {
	...
	sourcer.On("BackupDescriptors", mock.Anything, opWithID(backupID), mock.Anything, mock.Anything).
		Return((<-chan backup.ClassDescriptor)(descriptors))
	sourcer.On("ReleaseBackup", mock.Anything, opWithID(backupID), mock.Anything).Return(nil)
