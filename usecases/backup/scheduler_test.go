...
func TestSchedulerBackupStatus(t *testing.T) {
	...
	t.Run("ActiveState", func(t *testing.T) {
		...
		st, err := s.BackupStatus(ctx, nil, backendName, id, "", "", nil)
		assert.Nil(t, err)
	...
	t.Run("GetBackupProvider", func(t *testing.T) {
		...
		_, err := fs.scheduler().BackupStatus(ctx, nil, backendName, id, "", "", nil)
		assert.NotNil(t, err)
	})
	...
	t.Run("ReadFromMetadata", func(t *testing.T) {
		...
		fs.backend.On("GetObject", ctx, id, GlobalBackupFile).Return(bytes, nil)
		fs.backend.On("HomeDir", mock.Anything, mock.Anything, mock.Anything).Return(path)
		got, err := fs.scheduler().BackupStatus(ctx, nil, backendName, id, "", "", nil)
		assert.Nil(t, err)
...
func TestSchedulerRestorationStatus(t *testing.T) {
	...
	t.Run("ActiveState", func(t *testing.T) {
		...
		st, err := s.RestorationStatus(ctx, nil, backendName, id, "", "", nil)
		assert.Nil(t, err)
		assert.Equal(t, want, st)
	})
	...
	t.Run("GetBackupProvider", func(t *testing.T) {
		fs := newFakeScheduler(nil)
		fs.backendErr = ErrAny
		_, err := fs.scheduler().RestorationStatus(ctx, nil, backendName, id, "", "", nil)
		assert.NotNil(t, err)
	})
	...
	t.Run("ReadFromMetadata", func(t *testing.T) {
		...
		fs.backend.On("HomeDir", mock.Anything, mock.Anything, mock.Anything).Return(path)
		got, err := fs.scheduler().RestorationStatus(ctx, nil, backendName, id, "", "", nil)
		assert.Nil(t, err)
		assert.Equal(t, want, got)
	})
}
...
func TestCancellingBackup(t *testing.T) {
	...
	t.Run("ValidateEmptyID-Cancellation", func(t *testing.T) {
		assert.NotNil(t, scheduler.Cancel(ctx, nil, backendName, "", "", "", nil))
	})

	t.Run("ValidateID", func(t *testing.T) {
		assert.NotNil(t, scheduler.Cancel(ctx, nil, backendName, "A*:", "", "", nil))
	})
	...
	t.Run("CancellingSucceeded", func(t *testing.T) {
		...
		err = fakeScheduler.scheduler().Cancel(ctx, nil, backendName, "abc", "", "", nil)
		assert.NotNil(t, err)
