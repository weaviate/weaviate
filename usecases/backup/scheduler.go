...
func NewScheduler(
	...
	// A probe that cannot see its Scheduler answers "not busy" for a whole backup
	// this node is coordinating, so a missing one must stop the node at startup.
	if activity == nil {
...
func (s *Scheduler) Restore(ctx context.Context, pr *models.Principal,
	...
	if err != nil {
		if errors.Is(err, errMetaNotFound) {
			...
			} else if s.authorizer.Authorize(ctx, pr, authorization.CREATE, authorization.Backups()...) == nil {
...
func (s *Scheduler) validateBackupRequest(ctx context.Context, store coordStore, req *BackupRequest) (selections backupSelections, err error) {
	...
	if err := s.backupper.selector.Backupable(ctx, classes); err != nil {
		// Deferred to canCommit, which re-raises it before a byte is written: this
		// runs before authorization, so answering leaks a setting to a caller owed 403.
