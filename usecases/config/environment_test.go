...
func TestFromEnv_QueryBatchedContainsEnabled(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "a config-file true survives an unset variable", env: "", seed: &seededOn, want: true},
		{name: "the variable wins over a config-file value", env: "true", seed: &seededOff, want: true},
		{name: "the variable turns off what a config-file true turned on", env: "false", seed: &seededOn, want: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
