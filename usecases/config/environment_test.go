...
func TestEnvironmentAsyncReplicationSchedulerWorkers(t *testing.T) {
	cases := []struct {
		name        string
		value       []string
		expected    int
		...
		{"not given", []string{}, min(runtime.GOMAXPROCS(0), DefaultAsyncReplicationSchedulerWorkers), false},
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			if len(tt.value) == 1 {
				t.Setenv("ASYNC_REPLICATION_SCHEDULER_WORKERS", tt.value[0])
			}
			conf := Config{}
			err := FromEnv(&conf)
			if tt.expectedErr {
				...
				require.NoError(t, err)
				require.Equal(t, tt.expected, conf.Replication.AsyncReplicationSchedulerWorkers.Get())
			}
		})
	}
}

func TestEnvironmentAsyncReplicationSchedulerTopologyFrequency(t *testing.T) {
	cases := []struct {
		name        string
		value       []string
		expected    time.Duration
		...
		{"not given", []string{}, DefaultAsyncReplicationSchedulerTopologyFrequency, false},
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			if len(tt.value) == 1 {
				t.Setenv("ASYNC_REPLICATION_SCHEDULER_TOPOLOGY_FREQUENCY", tt.value[0])
			}
			conf := Config{}
			err := FromEnv(&conf)
			if tt.expectedErr {
				...
				require.NoError(t, err)
				require.Equal(t, tt.expected, conf.Replication.AsyncReplicationSchedulerTopologyFrequency.Get())
			}
		})
	}
}
