...
func FromEnv(config *Config) error {
	...
	if err := parsePositiveInt(
		...
		min(runtime.GOMAXPROCS(0), DefaultAsyncReplicationSchedulerWorkers),
	); err != nil {
		return err
	}

	if err := parsePositiveDuration(
		"ASYNC_REPLICATION_SCHEDULER_TOPOLOGY_FREQUENCY",
		func(val time.Duration) {
			config.Replication.AsyncReplicationSchedulerTopologyFrequency = configRuntime.NewDynamicValue(val)
		},
		DefaultAsyncReplicationSchedulerTopologyFrequency,
	); err != nil {
		return err
	}
