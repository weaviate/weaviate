...
const objectTableName = "object_data_wide"
const sep = "||"
...
func (f *Cassandra) Init() error {
	...
	for _, class := range f.schema.ThingSchema.Schema.Classes {
		for _, prop := range class.Properties {
			...
			columns = fmt.Sprintf(`%s %s text,`, columns, prop.Name)
	...
	err := f.client.Query(fmt.Sprintf(`
		CREATE TABLE IF NOT EXISTS `+objectTableName+` (
			...
			`+RootColumn+` boolean,
			`+TokenColumn+` text,
			PRIMARY KEY ((`+IDColumn+`, `+UUIDColumn+`), `+TimeStampColumn+`)
		)`, columns)).Exec()
	...
	// Create all indexes
	indexes := []string{ClassColumn, UUIDColumn, RootColumn}
...
func (f *Cassandra) AddThing(thing *models.Thing, UUID strfmt.UUID) error {
	...
	i := 0
	for i < dataCount {
		val1 := rand.Intn(100000000)
		val2 := rand.Intn(100000000)

		randuuid, _ := gocql.RandomUUID()
		query := f.client.Query(
			...
			false,
			connutils.NowUnix()+int64(val1),
			fmt.Sprintf("Value %d", val1),
			fmt.Sprintf("Value %d", val2),
		)
