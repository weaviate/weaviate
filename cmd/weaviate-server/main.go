...
func main() {
	...
	if if_gen_metrics() {
		genMetrics(prometheus.DefaultRegisterer)
		return
	}
...
func if_gen_metrics() bool {
	if len(os.Args) > 1 {
		if os.Args[1] == "gen-metrics" {
			return true
		}
	}
	return false
...
func genMetrics(reg prometheus.Registerer) {
	...
	for _, v := range mf {
		fmt.Println(v.GetName(), v.GetHelp())
	}
}
