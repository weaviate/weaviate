...
import (
	...
	_ "github.com/weaviate/weaviate/usecases/monitoring"
)
...
func main() {
	...
	if len(os.Args) > 1 && os.Args[1] == "gen-metric" {
		genMetricsDoc(prometheus.DefaultRegisterer)
		return
	}
...
func genMetricsDoc(reg prometheus.Registerer) {
	r := reg.(*prometheus.Registry)
	mf, err := r.Gather()
	if err != nil {
		panic(err)
	}
	for _, f := range mf {
		fmt.Println(f.GetName(), f.GetHelp(), f.GetType(), f.GetUnit())
	}
}
