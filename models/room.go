...
type Room struct {
	...
	// associated label
	AssociatedLabel []*AssociatedLabel `json:"associatedLabel"`
...
func (m *Room) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateAssociatedLabel(formats); err != nil {
		// prop
		res = append(res, err)
	}
...
func (m *Room) validateAssociatedLabel(formats strfmt.Registry) error {

	if swag.IsZero(m.AssociatedLabel) { // not required
		return nil
	}

	for i := 0; i < len(m.AssociatedLabel); i++ {

		if swag.IsZero(m.AssociatedLabel[i]) { // not required
			continue
		}

		if m.AssociatedLabel[i] != nil {

			if err := m.AssociatedLabel[i].Validate(formats); err != nil {
				return err
			}
		}
...
const (
	...
	roomRoomTypeOther           string = "other"
