...
## Phase 4 — gRPC surface + copier client (single-log primitives)

**Scope:** wire activation, snapshot, tailing (with optional LSN cap), finalize, and teardown over the wire. Built on a per-tailer LSN cap so the consumer can drain a phase boundary without sealing the log — this avoids the FINALIZING→DEHYDRATING write-loss window that a one-shot `Finalize` would otherwise introduce. After this phase, the change-capture system is fully functional but nothing in `cluster/replication/consumer.go` invokes it.

**Modified files (Phase 1/2 additions inside this PR):**

- `cluster/replication/changelog/tailer.go` — add `untilLSN` field on `Tailer` (inclusive upper bound on emitted LSNs); new constructor `(*ChangeLog).NewTailerWithCap(fromLSN, untilLSN uint64)`; `Tailer.Next` returns `io.EOF` when `lastLSN >= untilLSN`. `untilLSN=0` emits no entries — required for the quiet-shard case. `NewTailer` wraps with `math.MaxUint64` for unbounded behavior. Capped EOF and finalized EOF are independent: whichever fires first ends the stream cleanly.
- `adapters/repos/db/shard_changelog.go` — add `Shard.SnapshotChangeLogLSN(opID)`. Mirrors `FinalizeChangeLog` (briefly takes `quiesceMux.Lock` for the upper-bound guarantee) but does not seal the log.
- `adapters/repos/db/shard.go` and `shard_lazyloader.go` — extend `ShardLike` interface to include `SnapshotChangeLogLSN`.

**Modified files (Phase 4 proper):**

- `adapters/handlers/rest/clusterapi/grpc/protocol/file_replication.proto`:
  - RPCs: `StartChangeCapture`, `GetChangeLog` (server streaming, with new `until_lsn` field), `SnapshotChangeLogLSN`, `FinalizeChangeLog`, `StopChangeCapture`.
  - Messages: `StartChangeCaptureRequest/Response`, `GetChangeLogRequest` (with `until_lsn uint64` — inclusive upper bound; 0 emits no entries), `ChangeLogStreamEntry`, `SnapshotChangeLogLSNRequest/Response`, `FinalizeChangeLogRequest/Response`, `StopChangeCaptureRequest/Response`.
...
- `adapters/handlers/rest/clusterapi/grpc/file_replication_service.go`:
  - Five handlers. `GetChangeLog` plumbs `req.UntilLsn` to `IncomingGetChangeLog`. Auth is inherited from the existing basic-auth interceptor at `server.go:107-173` — no changes there.
- `adapters/repos/db/replication.go`:
  - Add `IncomingStartChangeCapture`, `IncomingGetChangeLog(ctx, shardName, opID, untilLSN)`, `IncomingSnapshotChangeLogLSN`, `IncomingFinalizeChangeLog`, `IncomingStopChangeCapture`. These delegate to the Phase 2 `Shard` methods.
...
- `cluster/replication/copier/copier_changelog.go`:
  - 5 client primitives: `StartChangeCapture`, `TailAndApply(ctx, ..., untilLSN uint64)`, `SnapshotChangeLogLSN`, `FinalizeChangeLog`, `StopChangeCapture`.
  - `TailAndApply` opens the `GetChangeLog` stream with `until_lsn=untilLSN`, decodes each `ChangeLogStreamEntry`, batches (batch size 128), and flushes via `Index.OverwriteObjectsFromChangeLog` (Phase 3). It blocks until the server stream hits `io.EOF` — cap-driven (`lastSentLSN >= untilLSN`) or Finalize-driven (`lastSentLSN >= finalLSN` once sealed), whichever fires first — or until ctx is cancelled / a fatal error occurs. `untilLSN=0` is a no-op (drains nothing).
- `cluster/replication/copier/types/replica_copier.go` — extend `ReplicaCopier` interface with `SnapshotChangeLogLSN` and the `untilLSN` parameter on `TailAndApply`.

**Tests:**

- Tailer cap unit tests in `cluster/replication/changelog/log_test.go`: stops at cap; blocks until cap is reached; respects finalize-EOF if it fires first; cap=0 behaves like the existing finalize-EOF path.
- Shard `SnapshotChangeLogLSN` integration tests in `adapters/repos/db/shard_changelog_integration_test.go`: reflects all committed appends and stays writable; unknown opID surfaces `errNoSuchChangeLog`; after Finalize returns the same LSN.
- Handler-level unit tests in `file_replication_service_test.go` covering happy path + unknown-index + index-error for `SnapshotChangeLogLSN` plus a `GetChangeLog_PlumbsUntilLsn` case.
- Bufconn E2E in `cluster/replication/copier/copier_changelog_bufconn_test.go`:
  - `TestChangeCapture_SingleLogMovementFlow` exercises Snapshot → cap'd drain → more writes → Finalize → un-cap'd drain.
  - `TestChangeCapture_CappedDrainStopsAtCap` confirms a cap'd drain stops at the cap on a wide-open log (does not wait for Finalize).
  - `TestChangeCapture_CancelMidTail` (existing) carries over for the leak-guard.

**Merge gate:**
- `make grpc` regenerates cleanly; `git diff generated/` has no hand edits.
- `make mocks` regenerates cleanly.
- `go test -race -count 1 ./cluster/replication/... ./adapters/handlers/rest/clusterapi/grpc/... ./adapters/repos/db/...` passes.
- `go test -tags integrationTest -race -count 1 ./adapters/repos/db/...` passes (Shard `SnapshotChangeLogLSN` cases).
...
**Scope:** the behavioural change. Tees are already live from Phase 2 but never activated in production; this phase flips on activation by rewriting the three movement state handlers around the single-log Phase 4 primitives and uncommenting the acceptance test.

A single op-id (`op.Op.ID`) names the change-capture log for the entire movement lifecycle — HYDRATING, FINALIZING, and (for MOVE) DEHYDRATING. The log is sealed by `FinalizeChangeLog` only at the very end of the flow. Phase boundaries that need a synchronous drain converge via `SnapshotChangeLogLSN` + `TailAndApply(... untilLSN: N)` against a still-writable log.

**Modified files:**

- `cluster/replication/consumer.go`:
  - `processHydratingOp`: call `replicaCopier.StartChangeCapture(...)` before `CopyReplicaFiles`. On any error after a successful Start, defer `StopChangeCapture(...)` so a HYDRATING retry begins from a clean slate.
  - `processFinalizingOp`: schema-version wait + `LoadLocalShard` + `replicaExists` lookup (unchanged) → `SnapshotChangeLogLSN(opID) → N` → `TailAndApply(..., opID, untilLSN: N)` (synchronous cap'd drain) → RAFT-add target if not already replica → switch on transfer type. For COPY: `FinalizeChangeLog(opID) → finalLSN` → `TailAndApply(..., opID, untilLSN: finalLSN)` → `StopChangeCapture(opID)` → `c.sync` → READY. For MOVE: return DEHYDRATING (log stays alive, no Finalize, no Stop). The Snapshot-before-RAFT-add ordering closes the read-staleness window: the target catches up to `snap` before joining the sharding state, so a `consistency=ONE` coordinator read after RAFT-add never sees state older than the FINALIZING phase boundary.
  - `processDehydratingOp`: schema-version wait + `ShardReplicas` check (unchanged) → `DeleteReplicaFromShard` → `FinalizeChangeLog(opID) → finalLSN` → `TailAndApply(..., opID, untilLSN: finalLSN)` → `StopChangeCapture(opID)` → `c.sync` → READY.
  - **`untilLSN: 0` is not a wildcard.** `cluster/replication/changelog/tailer.go:78` returns `io.EOF` immediately when `lastLSN >= untilLSN`, so a tailer constructed with `untilLSN=0` emits no entries. The post-Finalize drain must pass the `finalLSN` returned by `FinalizeChangeLog` (or `math.MaxUint64`); passing 0 silently drops every entry between Snapshot and Finalize.
  - Delete `startAsyncReplication` / `waitForAsyncReplication` / `stopAsyncReplication` / `handleAsyncReplErr` and the `asyncReplicationMinimumWait` field. Drop the constructor parameter from `NewCopyOpConsumer` and from the 12 call sites in `cluster/service.go`, `consumer_test.go`, and `producer_consumer_test.go`. (The lower-level `replicaCopier` async-replication methods stay — they're used by the non-movement async-replication path.)
  - **Cleanup on error paths:** vary by handler. HYDRATING errors call `StopChangeCapture(opID)` (lose pending entries; next retry starts fresh). FINALIZING and DEHYDRATING errors do NOT call Stop — the source log must persist so the next attempt can re-tail. The drain goroutine is cancelled via `cancelDrain()` and waited on via `<-drainDone` so the gRPC stream tears down cleanly. This policy preserves the "every committed write delivered at least once" invariant under retries.
...
- **Phase 5 revert** restores the old hashtree catchup. Because Phase 5 deletes the now-unused `startAsyncReplication` / `waitForAsyncReplication` / `stopAsyncReplication` / `handleAsyncReplErr` helpers and the `asyncReplicationMinimumWait` field+parameter, a Phase 5 revert restores those alongside the call sites in `consumer.go`, `cluster/service.go`, and the consumer test files. `TestReplicaMovementTenantParallelWrites` would need to be re-commented. This is the only phase with behavioural revert cost. Phase 2 tees stay in `main` after a Phase 5 revert — harmless, since no production caller activates a log.
...
- The single-log design depends on three Phase 1/2 idempotency properties, all verified in the code: `ChangeLog.Finalize` returns the same `finalLSN` on a second call (`log.go:144-146`); `Tailer.Next` against an already-finalized log emits all entries before EOF (`tailer.go:54-99`); `Shard.StopChangeCapture` is a no-op for an unknown opID (`shard_changelog.go:71-78`). The retry policy in Phase 5's cleanup-on-error rules relies on these.
