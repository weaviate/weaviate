...
- `cluster/replication/consumer.go`:
  ...
  - `processFinalizingOp`: schema-version wait + `LoadLocalShard` + `replicaExists` lookup (unchanged) → RAFT-add target if not already replica → `SnapshotChangeLogLSN(opID) → N` → `TailAndApply(..., opID, untilLSN: N)` (synchronous) → switch on transfer type. For COPY: `FinalizeChangeLog(opID)` → `TailAndApply(..., opID, untilLSN: 0)` → `StopChangeCapture(opID)` → `c.sync` → READY. For MOVE: return DEHYDRATING (log stays alive, no Finalize, no Stop).
  - `processDehydratingOp`: schema-version wait + `ShardReplicas` check (unchanged) → `DeleteReplicaFromShard` → `FinalizeChangeLog(opID)` → `TailAndApply(..., opID, untilLSN: 0)` → `StopChangeCapture(opID)` → `c.sync` → READY.
