...
ListThings() | - | - | Get list of things based on key-uuid, sorted by most recent | Search on class, search on property value (wildcard). | owner = uuid | multiple where’s for searching | X | timestamp | 1
...
ListAction() | Get list of actions based on key-uuid, sorted by most recent | Search on class, search on property value (wildcard). | properties.things.object.cref = ‘uuid’ | multiple where’s for searching | X | timestamp | 1
