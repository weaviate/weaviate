...
func TestVectorizingObjects(t *testing.T) {
	...
	tests := []testCase{
		{
			...
			octoAIModel:         "thenlper/gte-large",
			expectedOctoAIModel: "thenlper/gte-large",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, config.Model(), test.expectedOctoAIModel)
		})
	}
}
