...
func TestBuildUrlFn(t *testing.T) {
    t.Run("buildMorphUrl returns default Morph endpoint", func(t *testing.T) {
        config := ent.VectorizationConfig{BaseURL: "https://api.morphllm.com"}
        url, err := buildMorphUrl(config.BaseURL)
        assert.Nil(t, err)
        assert.Equal(t, "https://api.morphllm.com/v1/embeddings", url)
    })

    t.Run("buildMorphUrl uses provided BaseURL", func(t *testing.T) {
        config := ent.VectorizationConfig{BaseURL: "https://foobar.some.proxy"}
        url, err := buildMorphUrl(config.BaseURL)
        assert.Nil(t, err)
...
func TestClient(t *testing.T) {
	t.Run("when all is fine", func(t *testing.T) {
        ...
        c.buildUrlFn = func(baseURL string) (string, error) {
        ...
        res, rl, _, err := c.Vectorize(context.Background(), []string{"This is my text"}, fakeClassConfig{classConfig: map[string]interface{}{"Type": "text", "Model": "morph-embedding-v3"}})
	...
	t.Run("when the server returns an error", func(t *testing.T) {
		...
		require.NotNil(t, err)
        assert.EqualError(t, err, "connection to: Morph API failed with status: 500 error: nope, not gonna happen")
	})
	...
	t.Run("when the server returns an error with request id", func(t *testing.T) {
		...
		require.NotNil(t, err)
        assert.EqualError(t, err, "connection to: Morph API failed with status: 500 request-id: some-request-id error: nope, not gonna happen")
	})

    t.Run("when Morph key is passed using X-Morph-Api-Key header", func(t *testing.T) {
		server := httptest.NewServer(&fakeHandler{t: t})
		defer server.Close()
        c := New("", 0, nullLogger())
        c.buildUrlFn = func(baseURL string) (string, error) {
		...
		ctxWithValue := context.WithValue(context.Background(),
            "X-Morph-Api-Key", []string{"some-key"})
        ...
        res, _, _, err := c.Vectorize(ctxWithValue, []string{"This is my text"},
            fakeClassConfig{classConfig: map[string]interface{}{"Type": "text", "Model": "morph-embedding-v3"}})

		require.Nil(t, err)
		assert.Equal(t, expected, res)
	})

    t.Run("when Morph key is empty", func(t *testing.T) {
		server := httptest.NewServer(&fakeHandler{t: t})
		defer server.Close()
        c := New("", 0, nullLogger())
        c.buildUrlFn = func(baseURL string) (string, error) {
        ...
        assert.EqualError(t, err, "API Key: no api key found "+
            "neither in request header: X-Morph-Api-Key "+
            "nor in environment variable under MORPH_APIKEY")
	})

    t.Run("when X-Morph-Api-Key header is passed but empty", func(t *testing.T) {
		server := httptest.NewServer(&fakeHandler{t: t})
		defer server.Close()
        c := New("", 0, nullLogger())
        c.buildUrlFn = func(baseURL string) (string, error) {
    ...
    t.Run("when X-Morph-BaseURL header is passed", func(t *testing.T) {
        c := New("", 0, nullLogger())
        ...
        ctxWithValue := context.WithValue(context.Background(),
            "X-Morph-Baseurl", []string{"http://base-url-passed-in-header.com"})
    ...
    // Azure configuration is not applicable for Morph

    t.Run("pass rate limit headers requests", func(t *testing.T) {
        c := New("", 0, nullLogger())

        ctxWithValue := context.WithValue(context.Background(),
            "X-Morph-Ratelimit-RequestPM-Embedding", []string{"50"})
    ...
    t.Run("pass rate limit headers tokens", func(t *testing.T) {
        ...
        ctxWithValue := context.WithValue(context.Background(), "X-Morph-Ratelimit-TokenPM-Embedding", []string{"60"})
...
func TestGetApiKeyFromContext(t *testing.T) {
	t.Run("value from context", func(t *testing.T) {
        ctx := context.WithValue(context.Background(), "X-Morph-Api-Key", []string{"key-from-ctx"})
        c := New("", 0, nullLogger())
...
func TestGetEmbeddingsRequest(t *testing.T) {
    c := New("", 0, nullLogger())
    dim := int64(42)
    req := c.getEmbeddingsRequest([]string{"foo"}, "model", &dim)
    assert.Equal(t, []string{"foo"}, req.Input)
    assert.Equal(t, "model", req.Model)
    assert.Equal(t, &dim, req.Dimensions)
}

// No header selection logic for Morph; always Authorization: Bearer
...
func TestGetErrorFormat(t *testing.T) {
    c := New("", 0, nullLogger())
    err := c.getError(403, "abc-123", &openAIApiError{Message: "denied"})
