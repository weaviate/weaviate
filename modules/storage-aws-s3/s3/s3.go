...
func (s *s3) RestoreSnapshot(ctx context.Context, className, snapshotID string) ([]byte, *snapshots.Snapshot, error) {
	bucketName := s.config.BucketName()
	bucketExists, err := s.client.BucketExists(ctx, bucketName)
	if !bucketExists {
		return nil, nil, errors.Wrap(err, "backup bucket does not exist")
	}
	if err != nil {
		return nil, nil, errors.Wrap(err, "can't connect to bucket")
	}
	...
	for _, srcRelPath := range snapshot.Files {
		if err := ctx.Err(); err != nil {
			return nil, nil, errors.Wrapf(err, "restore snapshot aborted")
		}
		...
		if err != nil {
			return nil, nil, errors.Wrapf(err, "Unable to restore file %s, system might be in a corrupted state", filePath)
		}
	}
