	...
	tests := []testCase{
		{
			...
			upstageModel:         "solar-embedding-1-large-passage",
			expectedupstageModel: "solar-embedding-1-large-passage",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			ic := &fakeClassConfig{
				...
				upstageModel:           test.upstageModel,
			...
			assert.Equal(t, config.Model(), test.expectedupstageModel)
		})
	}
}
