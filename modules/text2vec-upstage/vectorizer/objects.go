...
const (
	MaxObjectsPerBatch = 2048 // Info from upstage (The array can contain up to 100 strings, and the total number of tokens in the array(tokens per each request) should be less than 204,800.  ie. 204800/100=2048)
...
func New(client text2vecbase.BatchClient, logger logrus.FieldLogger) *text2vecbase.BatchVectorizer {
	...
	// There is a limit of 4000 tokens per batch, however we cannot easily precompute how many tokens an object will.
	// Therefore, we use a dummy value and let the upstage API fail in case of a too large input object.
