...
const (
	DefaultRPM = 100 * 60 // from https://upstage.ai/embeddings/
...
type embedding struct {
	...
	Error  *upstageApiError `json:"error,omitempty"`
}
...
func New(upstageApiKey string, timeout time.Duration, logger logrus.FieldLogger) *vectorizer {
	return &vectorizer{
...
func (v *vectorizer) vectorize(ctx context.Context, input []string, model string, config ent.VectorizationConfig) (*modulecomponents.VectorizationResult, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "join upstage API host and path")
	}
...
func (v *vectorizer) getError(statusCode int, resBodyError *upstageApiError) error {
	endpoint := "upstage API"
	if resBodyError != nil {
...
func (v *vectorizer) getApiKey(ctx context.Context) (string, error) {
	...
	apiKey = "X-upstage-Api-Key"
	envVar = "upstage_APIKEY"
	if len(v.upstageApiKey) > 0 {
		return v.upstageApiKey, nil
	}
...
func (v *vectorizer) GetVectorizerRateLimit(ctx context.Context, cfg moduletools.ClassConfig) *modulecomponents.RateLimits {
	rpm, _ := modulecomponents.GetRateLimitFromContext(ctx, "upstage", DefaultRPM, 0)
