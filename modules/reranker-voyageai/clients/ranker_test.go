...
func (f *testRankHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if len(f.batchedResults) > 0 {
		...
		f.response.Data = f.batchedResults[index]
	}
...
func TestRank_client_getVoyageAIUrl(t *testing.T) {
	...
	url, err := c.getVoyageAIUrl(ctx, "https://api.voyageai.com/v1")
	assert.NoError(t, err)
	assert.Equal(t, "https://api.voyageai.com/v1/rerank", url)

	ctxWithBaseURL := context.WithValue(ctx, "X-Voyageai-Baseurl", []string{"https://base-url-from-ctx.com"})
	url, err = c.getVoyageAIUrl(ctxWithBaseURL, "https://api.voyageai.com/v1")
	assert.NoError(t, err)
	assert.Equal(t, "https://base-url-from-ctx.com/rerank", url)
}
