...
func TestClassSettings(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:            "defaults",
			cfg:             fakeClassConfig{},
			expectedBaseURL: "https://api.meta.ai",
			expectedModel:   "muse-spark-1.2",
		},
		{
			name: "all properties set",
			cfg: fakeClassConfig{
				"baseURL":          "https://proxy.meta.ai",
			...
			expectedBaseURL:         "https://proxy.meta.ai",
			expectedModel:           "muse-spark-1.1",
			...
			expectedReasoningEffort: ptr("xhigh"),
		},
		{
			name:        "nil config",
			cfg:         nil,
		...
		{
			name:        "unknown reasoningEffort",
			cfg:         fakeClassConfig{"reasoningEffort": "extreme"},
			expectedErr: "wrong reasoningEffort configuration",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expectedReasoningEffort, settings.ReasoningEffort())
		})
	}
}
...
func TestValidateReasoningEffort(t *testing.T) {
	tests := []struct {
		...
		{name: "unset", effort: nil},
		{name: "none", effort: ptr("none")},
		{name: "minimal", effort: ptr("minimal")},
		{name: "low", effort: ptr("low")},
		{name: "medium", effort: ptr("medium")},
		{name: "high", effort: ptr("high")},
		{name: "xhigh", effort: ptr("xhigh")},
		{name: "wrong case", effort: ptr("XHigh"), expectedErr: true},
		{name: "empty", effort: ptr(""), expectedErr: true},
		{name: "unknown", effort: ptr("extreme"), expectedErr: true},
	}
