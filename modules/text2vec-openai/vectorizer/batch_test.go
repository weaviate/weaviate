...
func TestBatch(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "multiple vectorizer batches", objects: []*models.Object{
			{Class: "Car", Properties: map[string]interface{}{"test": "rate 25"}}, // set limit so next 3 objects are one batch
		...
		{name: "deadline", deadline: 100 * time.Millisecond, objects: []*models.Object{
			{Class: "Car", Properties: map[string]interface{}{"test": "rate 15"}}, // set limit
	...
	for _, tt := range cases {
		t.Run("", func(t *testing.T) {
			...
			_, errs := v.ObjectBatch(context.Background(), []*models.Object{
				{Class: "Car", Properties: map[string]interface{}{"test": "rate 11"}}, // first request, set rate down so the next two items can be sent
				{Class: "Car", Properties: map[string]interface{}{"test": "wait 90"}}, // second batch, use up batch time to trigger waiting for refresh
				{Class: "Car", Properties: map[string]interface{}{"test": "rate 11"}}, // set next rate so the next object is too long. Depending on the total batch time it either sleeps or not
