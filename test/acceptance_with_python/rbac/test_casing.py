...
def test_rbac_refs(request: SubRequest, admin_client, custom_client, to_upper: bool):
    ...
    admin_client.roles.create(
        name=name,
        permissions=[
            RBAC.permissions.collection_config(collection=name, read_config=True),
...
def test_role_name_case_sensitivity(request: SubRequest, admin_client):
    ...
    admin_client.roles.create(
        name=l_name, permissions=RBAC.permissions.collection_config(collection="lower", read_config=True)
    )

    admin_client.roles.create(
        name=u_name, permissions=RBAC.permissions.collection_config(collection="upper", read_config=True)
    )
