...
function main() {
  ...
  run_flake_hunt_multinode_cancel=false
  ...
  while [[ "$#" -gt 0 ]]; do
      case $1 in
          ...
          --flake-hunt-multinode-cancel|-fhmc) run_all_tests=false; run_flake_hunt_multinode_cancel=true;;
          ...
          --help|-h) printf '%s\n' \
              ...
              "--flake-hunt-multinode-cancel | -fhmc"\
  ...
  if $run_flake_hunt_multinode_cancel; then
    echo "running flake-hunt: TestMultiNode_CancelClearsAcrossReplicas (10×)"
    run_flake_hunt_multinode_cancel
  fi
  echo "Done!"
}
...
# QA flake-hunt: drive TestMultiNode_CancelClearsAcrossReplicas 10×.
# Used from the qa/flake-hunt-multinode-cancel branch only — there the entire
# CI matrix is reduced to 30 shards all running this. 30 × 10 = 300 total runs,
# enough to estimate the flake rate that surfaced once on PR #11327.
# Per-iteration pass/fail goes to stdout so the shard summary lands in the
# job log; exits non-zero only if all 10 fail.
function run_flake_hunt_multinode_cancel() {
  build_weaviate_test_image
  echo_green "flake-hunt: TestMultiNode_CancelClearsAcrossReplicas (10 iterations)"
  local pass=0 fail=0
  for i in $(seq 1 10); do
    echo
    echo "=== iter $i/10 — $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
    if go test -v -count=1 -timeout=15m \
        -run '^TestMultiNode_CancelClearsAcrossReplicas$' \
  ...
  # Don't fail the shard on individual flakes — we want all 10 iterations to
  # run so the per-shard summary captures the rate. Only fail if all 10 fail
  # (which would itself indicate a deterministic regression, not a flake).
  [[ $pass -gt 0 ]]
}
