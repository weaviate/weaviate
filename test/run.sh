...
function main() {
  ...
  run_acceptance_replica_replication_fast_movement_tests=false
  ...
  while [[ "$#" -gt 0 ]]; do
      case $1 in
          ...
          --acceptance-only-replica-replication-fast-movement|-aorrfm) run_all_tests=false; run_acceptance_replica_replication_fast_movement_tests=true ;;
...
function run_acceptance_tests() {
  ...
  if $run_acceptance_replica_replication_fast_movement_tests || $run_acceptance_tests || $run_all_tests; then
  echo "running acceptance replica replication replication fast movement"
    run_acceptance_replica_replication_fast_movement_tests "$@"
  fi
...
function run_acceptance_replica_replication_fast_movement_tests() {
  for pkg in $(go list ./.../ | grep 'test/acceptance/replication/replica_replication/fast_movement$'); do
    if ! go test -timeout=30m -count 1 -race "$pkg"; then
      echo "Test for $pkg failed" >&2
      return 1
    fi
