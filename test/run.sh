...
function main() {
  ...
  run_acceptance_replica_replication_long_rf1_tests=false
  run_acceptance_replica_replication_long_rf2_tests=false
  ...
  while [[ "$#" -gt 0 ]]; do
      case $1 in
          ...
          --acceptance-only-replica-replication-long-rf1|-aorrl1) run_all_tests=false; run_acceptance_replica_replication_long_rf1_tests=true ;;
          --acceptance-only-replica-replication-long-rf2|-aorrl2) run_all_tests=false; run_acceptance_replica_replication_long_rf2_tests=true ;;
          ...
          --help|-h) printf '%s\n' \
              ...
              "--acceptance-only-replica-replication-long-rf1| -aorrl1"\
              "--acceptance-only-replica-replication-long-rf2| -aorrl2"\
  ...
  if $run_acceptance_tests  || $run_acceptance_only_fast || $run_acceptance_only_authz || $run_acceptance_go_client || $run_acceptance_graphql_tests || $run_acceptance_replication_tests || $run_acceptance_replica_replication_fast_tests || $run_acceptance_replica_replication_slow_tests || $run_acceptance_replica_replication_long_rf1_tests || $run_acceptance_replica_replication_long_rf2_tests || $run_acceptance_async_replication_tests || $run_acceptance_only_python || $run_all_tests || $run_benchmark || $run_acceptance_go_client_only_fast || $run_acceptance_go_client_named_vectors_single_node || $run_acceptance_go_client_named_vectors_cluster || $only_acceptance || $run_acceptance_objects
  then
    ...
    if $run_acceptance_tests || $run_acceptance_only_fast || $run_acceptance_only_authz || $run_acceptance_go_client || $run_acceptance_graphql_tests || $run_acceptance_replication_tests || $run_acceptance_replica_replication_fast_tests || $run_acceptance_replica_replication_slow_tests || $run_acceptance_replica_replication_long_rf1_tests || $run_acceptance_replica_replication_long_rf2_tests || $run_acceptance_async_replication_tests || $run_acceptance_go_client_only_fast || $run_acceptance_go_client_named_vectors_single_node || $run_acceptance_go_client_named_vectors_cluster || $run_all_tests || $only_acceptance || $run_acceptance_objects
    then
...
function run_acceptance_tests() {
  ...
  if $run_acceptance_replica_replication_long_rf1_tests || $run_acceptance_tests || $run_all_tests; then
  echo "running acceptance replica replication replication long"
    run_acceptance_replica_replication_long_rf1_tests "$@"
  fi
  if $run_acceptance_replica_replication_long_rf2_tests || $run_acceptance_tests || $run_all_tests; then
  echo "running acceptance replica replication replication long"
    run_acceptance_replica_replication_long_rf2_tests "$@"
  fi
...
function run_acceptance_replica_replication_long_rf1_tests() {
  for pkg in $(go list ./.../ | grep 'test/acceptance/replication/replica_replication/long_rf1'); do
    if ! go test -v -timeout=30m -count 1 -race "$pkg"; then
      echo "Test for $pkg failed" >&2
      return 1
    fi
  done
...
function run_acceptance_replica_replication_long_rf2_tests() {
  for pkg in $(go list ./.../ | grep 'test/acceptance/replication/replica_replication/long_rf2'); do
    if ! go test -v -timeout=30m -count 1 -race "$pkg"; then
      echo "Test for $pkg failed" >&2
      return 1
    fi
