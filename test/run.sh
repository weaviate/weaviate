...
function run_go_client_group() {
  ...
  for pattern in "${package_paths[@]}"; do
    for pkg in $(go list ./... | grep -v 'acceptance_tests_with_client/named_vectors_tests' | grep "${pattern}$"); do
      echo_green "Running $pkg"
      # Explicit timeout: these packages run race-instrumented and the heaviest
      # of them needs well over Go's 10m default, which every other acceptance
      # runner in this file already overrides.
