...
func testExplore(t *testing.T, client *client.Weaviate) {
	className := "L2Squared_Class_2"
	defer deleteObjectClass(t, client, className)

	t.Run("create second L2 class", func(t *testing.T) {
		createObjectClass(t, client, &models.Class{
			Class:      className,
			Vectorizer: "none",
	...
	t.Run("run Explore with certainty arg and expect failure", func(t *testing.T) {
		res := ErrorGraphQLWithClient(t, nil, `
		{
