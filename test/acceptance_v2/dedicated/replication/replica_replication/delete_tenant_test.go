	...
	for _, state := range stateToDeleteIn {
		t.Run(fmt.Sprintf("delete tenant when op is %s", state), func(t *testing.T) {
			...
			t.Run(fmt.Sprintf("create replication operation for %s", tenant1), func(t *testing.T) {
				created, err := client.Replication.Replicate(replication.NewReplicateParams().WithBody(getMTRequest(t, client, paragraphClass.Class, tenant1)), nil)
				require.Nil(t, err)
			...
			t.Run(fmt.Sprintf("create replication operation for %s", tenant2), func(t *testing.T) {
				created, err := client.Replication.Replicate(replication.NewReplicateParams().WithBody(getMTRequest(t, client, paragraphClass.Class, tenant2)), nil)
				require.Nil(t, err)
				require.NotNil(t, created)
				require.NotNil(t, created.Payload)
				require.NotNil(t, created.Payload.ID)
			...
			if state != api.REGISTERED {
				t.Run(fmt.Sprintf("wait until op for %s is in %s state", tenant1, state), func(t *testing.T) {
					assert.EventuallyWithT(t, func(ct *assert.CollectT) {
						details, err := client.Replication.ReplicationDetails(replication.NewReplicationDetailsParams().WithID(id1), nil)
						require.Nil(ct, err)
			...
			t.Run(fmt.Sprintf("delete %s", tenant1), func(t *testing.T) {
				helper.DeleteTenantsWithClient(t, client, paragraphClass.Class, []string{tenant1})
			})

			t.Run("wait for replication operation to be deleted", func(t *testing.T) {
				assert.EventuallyWithT(t, func(ct *assert.CollectT) {
					_, err := client.Replication.ReplicationDetails(replication.NewReplicationDetailsParams().WithID(id1), nil)
			...
			t.Run(fmt.Sprintf("ensure that the replication operation for %s is still there", tenant2), func(t *testing.T) {
				details, err := client.Replication.ReplicationDetails(replication.NewReplicationDetailsParams().WithID(id2), nil)
				require.Nil(t, err)
			...
			t.Run(fmt.Sprintf("delete %s", tenant2), func(t *testing.T) {
				helper.DeleteTenantsWithClient(t, client, paragraphClass.Class, []string{tenant2})
			})

			t.Run("wait for replication operation to be deleted", func(t *testing.T) {
				assert.EventuallyWithT(t, func(ct *assert.CollectT) {
					_, err := client.Replication.ReplicationDetails(replication.NewReplicationDetailsParams().WithID(id2), nil)
					require.NotNil(ct, err)
					assert.IsType(ct, replication.NewReplicationDetailsNotFound(), err)
				}, 30*time.Second, 1*time.Second, "replication operation should be deleted")
			})
...
func getMTRequest(t *testing.T, client *client.Weaviate, className, tenant string) *models.ReplicationReplicateReplicaRequest {
	verbosity := verbosity.OutputVerbose
	nodes, err := client.Nodes.NodesGetClass(nodes.NewNodesGetClassParams().WithOutput(&verbosity).WithClassName(className), nil)
	require.Nil(t, err)
