...
func TestReplicaMovementHappyPath(t *testing.T) {
	...
	t.Run("start replica replication to node3 for paragraph", func(t *testing.T) {
		...
		for i, node := range body.Payload.Nodes {
			...
			for _, shard := range node.Shards {
				...
				sourceNode = i
				resp, err := client.Replication.Replicate(
	...
	t.Run("waiting for replication to finish", func(t *testing.T) {
		assert.EventuallyWithT(t, func(ct *assert.CollectT) {
			client = helper.ClientFromURI(t, compose.GetWeaviate().URI())
			details, err := client.Replication.ReplicationDetails(
	...
	t.Run("assert data is available for paragraph on node3 with consistency level one", func(t *testing.T) {
		assert.EventuallyWithT(t, func(ct *assert.CollectT) {
			for _, objId := range paragraphIDs {
				exists, err := common.ObjectExistsCL(t, compose.ContainerURI(2), paragraphClass.Class, objId, types.ConsistencyLevelOne)
				assert.Nil(ct, err)
				assert.True(ct, exists)

				resp, err := common.GetObjectCL(t, compose.ContainerURI(2), paragraphClass.Class, objId, types.ConsistencyLevelOne)
				assert.Nil(ct, err)
...
func TestReplicaMovementTenantHappyPath(t *testing.T) {
	...
	t.Run("start replica replication to node3 for paragraph", func(t *testing.T) {
		...
		for i, node := range body.Payload.Nodes {
			...
			targetNodeURI = compose.ContainerURI(i)
			break
		}
...
func TestReplicaMovementOneWriteExtraSlowFileCopy(t *testing.T) {
			...
			for _, shard := range node.Shards {
				...
				enterrors.GoWrapper(func() {
					...
					for i := 0; i < numParagraphsInsertedWhileStarting; i++ {
						...
						// Insert the object individually
						common.CreateObjects(t, compose.ContainerURI(sourceNode), []*models.Object{obj})
					}
				}, logger)
	...
	t.Run("assert correct number of objects on node3", func(t *testing.T) {
		numObjectsFound := common.CountObjects(t, compose.ContainerURI(2), paragraphClass.Class)
