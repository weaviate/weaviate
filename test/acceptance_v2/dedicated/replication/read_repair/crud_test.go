...
func TestImmediateReplicaCRUD(t *testing.T) {
	t.Parallel()
	...
	client := helper.ClientFromURI(t, compose.ContainerURI(1))
...
func TestEventualReplicaCRUD(t *testing.T) {
	t.Parallel()
	...
	t.Run("configure classes to replicate to node 2 and 3", func(t *testing.T) {
		...
		ac := helper.GetClassWithClient(t, client, "Article")
		ac.ReplicationConfig = &models.ReplicationConfig{
			Factor: 3,
		}
		helper.UpdateClassWithClient(t, client, ac)

		pc := helper.GetClassWithClient(t, client, "Paragraph")
		pc.ReplicationConfig = &models.ReplicationConfig{
			Factor: 3,
		}
		helper.UpdateClassWithClient(t, client, pc)
	})
	...
	t.Run("assert any future writes are replicated", func(t *testing.T) {
		...
		t.Run("configure classes to decrease replication factor should fail", func(t *testing.T) {
			...
			resp, err := client.Schema.SchemaObjectsUpdate(params, nil)
