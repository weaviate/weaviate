...
// ---------------------------------------------------------------------------
// Group finalization acceptance tests
// ---------------------------------------------------------------------------

// TestGroupFinalization_IndependentGroups verifies that OnGroupCompleted fires
// per-group as each group's sub-units all complete, even while the overall task
// is still STARTED. Two groups with 3 sub-units each, all succeed.
func TestGroupFinalization_IndependentGroups(t *testing.T) {
	...
	// Verify group-level finalization markers exist under {taskID}/{groupID}/
	...
	// Verify OnTaskCompleted fired
	awaitTaskCompletedOnAnyNode(t, compose, taskID)
}

// TestGroupFinalization_OneGroupFails verifies that when one group's sub-unit
// fails (task → FAILED), OnGroupCompleted still fires for both groups and
// OnTaskCompleted fires with FAILED status.
func TestGroupFinalization_OneGroupFails(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()
	...
	addTaskJSON(t, debugURI, addTaskRequest{
		...
		SubUnits:      subUnits,
		SubUnitGroups: subUnitGroups,
		FailSubUnit:   "g2-su1", // group-B fails
	})
	...
	// OnTaskCompleted should fire (with FAILED status)
	awaitTaskCompletedOnAnyNode(t, compose, taskID)
}

// TestGroupFinalization_ManyGroups verifies group finalization at scale:
// 20 groups with 2 sub-units each, MaxConcurrency=4.
// Uses synthetic sub-units (no collection), so scale is kept moderate to avoid
// excessive Raft contention from 3 nodes competing for all sub-units.
func TestGroupFinalization_ManyGroups(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()
...
// TestGroupFinalization_DefaultGroup verifies that tasks without explicit groups
// (default group "") preserve old behavior: OnGroupCompleted fires once with
// groupID="" when all sub-units reach terminal state, and finalization markers
// appear directly under /tmp/dtm-finalize/{taskID}/ (no group subdirectory).
func TestGroupFinalization_DefaultGroup(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()
	...
	// No SubUnitGroups → default group ""
	addTaskJSON(t, debugURI, addTaskRequest{
		ID:       taskID,
		SubUnits: subUnits,
	})
	...
	// Default group: markers appear directly under /tmp/dtm-finalize/{taskID}/
	// (same as pre-group behavior)
