...
func TestAuthzRolesForUsers(t *testing.T) {
	...
	t.Run("get roles for non existing user", func(t *testing.T) {
		_, err := helper.Client(t).Authz.GetRolesForUser(authz.NewGetRolesForUserParams().WithID("notExists").WithUserType(string(models.UserAndGroupTypeInputDb)), helper.CreateAuth(adminKey))
...
func TestReadUserPermissions(t *testing.T) {
	...
	t.Run("user cannot return roles for other user", func(t *testing.T) {
		_, err := helper.Client(t).Authz.GetRolesForUser(authz.NewGetRolesForUserParams().WithID(secondUser).WithUserType(string(models.UserAndGroupTypeInputDb)), helper.CreateAuth(customKey))
		require.Error(t, err)
