...
func TestAuthZBackupsManageJourney(t *testing.T) {
	...
	t.Run("create and assign a role that can read (for backup create) and create (for backup restore) clsA metadata and data", func(t *testing.T) {
...
func TestAuthZBackupsCollectionJourney(t *testing.T) {
	adminUser := "existing-user"
	adminKey := "existing-key"
	adminRole := "admin"
	...
	t.Run("create and assign a role that can manage all backups", func(t *testing.T) {
		helper.CreateRole(t, adminKey, &models.Role{
			Name: String(testRoleName),
			Permissions: []*models.Permission{
				{Action: String(authorization.ManageBackups)},
			},
		})
		helper.AssignRoleToUser(t, adminKey, testRoleName, customUser)
	})

	t.Run("fail to create a backup due to missing read_data/read_schema action on clsA", func(t *testing.T) {
		_, err := helper.CreateBackupWithAuthz(t, helper.DefaultBackupConfig(), clsA.Class, backend, backupID, helper.CreateAuth(customKey))
		require.NotNil(t, err)
		parsed, forbidden := err.(*backups.BackupsCreateForbidden)
		require.True(t, forbidden)
	...
	t.Run("add read_schema on clsA to role", func(t *testing.T) {
		helper.AddPermissions(t, adminKey, testRoleName, helper.NewCollectionPermission().WithAction(authorization.ReadSchema).WithCollection(clsA.Class).Permission())
	})

	t.Run("fail to create a backup due to missing read_data action on clsA", func(t *testing.T) {
		_, err := helper.CreateBackupWithAuthz(t, helper.DefaultBackupConfig(), clsA.Class, backend, backupID, helper.CreateAuth(customKey))
		require.NotNil(t, err)
		parsed, forbidden := err.(*backups.BackupsCreateForbidden)
		require.True(t, forbidden)
	...
	t.Run("add read_data on clsA to role", func(t *testing.T) {
		helper.AddPermissions(t, adminKey, testRoleName, helper.NewCollectionPermission().WithAction(authorization.ReadData).WithCollection(clsA.Class).Permission())
	})
	...
	t.Run("fail to restore a backup due to missing create_data/create_schema actions on clsA", func(t *testing.T) {
		_, err := helper.RestoreBackupWithAuthz(t, helper.DefaultRestoreConfig(), clsA.Class, backend, backupID, map[string]string{}, helper.CreateAuth(customKey))
		require.NotNil(t, err)
		parsed, forbidden := err.(*backups.BackupsRestoreForbidden)
		require.True(t, forbidden)
	...
	t.Run("add create_schema on clsA to role", func(t *testing.T) {
		helper.AddPermissions(t, adminKey, testRoleName, helper.NewCollectionPermission().WithAction(authorization.CreateSchema).WithCollection(clsA.Class).Permission())
	})

	t.Run("fail to restore a backup due to missing create_data action on clsA", func(t *testing.T) {
		_, err := helper.RestoreBackupWithAuthz(t, helper.DefaultRestoreConfig(), clsA.Class, backend, backupID, map[string]string{}, helper.CreateAuth(customKey))
		require.NotNil(t, err)
		parsed, forbidden := err.(*backups.BackupsRestoreForbidden)
		require.True(t, forbidden)
	...
	t.Run("add create_data on clsA to role", func(t *testing.T) {
		helper.AddPermissions(t, adminKey, testRoleName, helper.NewCollectionPermission().WithAction(authorization.CreateData).WithCollection(clsA.Class).Permission())
	})
