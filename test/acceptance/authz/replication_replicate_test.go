...
func TestAuthzReplicationReplicate(t *testing.T) {
	...
	t.Run("Cancel a replication of a shard with permissions", func(t *testing.T) {
		require.EventuallyWithT(t, func(ct *assert.CollectT) {
			...
			require.IsNotType(ct, replication.NewCancelReplicationForbidden(), err, "request should be permissioned but got different error type than expected: %v", err)
	...
	t.Run("Delete a replication of a shard with permissions", func(t *testing.T) {
		require.EventuallyWithT(t, func(ct *assert.CollectT) {
			...
			require.IsNotType(ct, replication.NewDeleteReplicationForbidden(), err, "request should be permissioned but got different error type than expected: %v", err)
