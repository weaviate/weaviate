...
func TestUpsertTool_InsertMultipleObjects(t *testing.T) {
	...
	// Verify first object
	uuid0 := strfmt.UUID(resp.Results[0].ID)
	obj0, _ := helper.GetObjectAuth(t, cls.Class, uuid0, apiKey)
	assert.Equal(t, "First Article Content", obj0.Properties.(map[string]interface{})["contents"])

	// Verify second object
	uuid1 := strfmt.UUID(resp.Results[1].ID)
	obj1, _ := helper.GetObjectAuth(t, cls.Class, uuid1, apiKey)
	assert.Equal(t, "Second Article Content", obj1.Properties.(map[string]interface{})["contents"])

	// Verify third object
	uuid2 := strfmt.UUID(resp.Results[2].ID)
	obj2, _ := helper.GetObjectAuth(t, cls.Class, uuid2, apiKey)
	assert.Equal(t, "Third Article Content", obj2.Properties.(map[string]interface{})["contents"])
}
...
func TestUpsertTool_UpdateMultipleObjects(t *testing.T) {
	...
	// Verify UUIDs match
	assert.Equal(t, uuid1, resp.Results[0].ID)
	assert.Equal(t, uuid2, resp.Results[1].ID)
	assert.Equal(t, uuid3, resp.Results[2].ID)

	// Verify first object was updated
	updatedObj1, err := helper.GetObjectAuth(t, cls.Class, created1.ID, apiKey)
	require.Nil(t, err)
	assert.Equal(t, "Updated Content 1", updatedObj1.Properties.(map[string]interface{})["contents"])
	assert.Equal(t, "Updated Title 1", updatedObj1.Properties.(map[string]interface{})["title"])

	// Verify second object was updated
	updatedObj2, err := helper.GetObjectAuth(t, cls.Class, created2.ID, apiKey)
	require.Nil(t, err)
	assert.Equal(t, "Updated Content 2", updatedObj2.Properties.(map[string]interface{})["contents"])
	assert.Equal(t, "Updated Title 2", updatedObj2.Properties.(map[string]interface{})["title"])

	// Verify third object was updated
	updatedObj3, err := helper.GetObjectAuth(t, cls.Class, created3.ID, apiKey)
	require.Nil(t, err)
	assert.Equal(t, "Updated Content 3", updatedObj3.Properties.(map[string]interface{})["contents"])
	assert.Equal(t, "Updated Title 3", updatedObj3.Properties.(map[string]interface{})["title"])
}
