...
// TestDropVectorIndex_Lifecycle drives the full drop lifecycle end to end:
// insert -> flush -> drop -> cleanup drains -> the VectorConfig entry is
// removed -> the name is re-creatable with a fresh index (even with a new
// dimensionality). Runs on its own compose so segment flushing is prompt
// (PERSISTENCE_MEMTABLES_FLUSH_DIRTY_AFTER_SECONDS=1) and the cleanup has real
// segments to rewrite.
func TestDropVectorIndex_Lifecycle(t *testing.T) {
