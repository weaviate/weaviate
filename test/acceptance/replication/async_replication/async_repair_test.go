...
func (suite *AsyncReplicationTestSuite) SetupSuite() {
	...
	suite.suiteCtx, suite.suiteCtxCancel = context.WithTimeout(context.Background(), 15*time.Minute)
...
func (suite *AsyncReplicationTestSuite) TearDownSuite() {
	...
	if suite.suiteCtxCancel != nil {
		suite.suiteCtxCancel()
	}
}
...
func (suite *AsyncReplicationTestSuite) SetupTest() {
	...
	require.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		assert.NoError(ct, clientErr)
		assert.NotNil(ct, body.Payload)
		resp := body.Payload
		assert.Len(ct, resp.Nodes, 3)
		for _, n := range resp.Nodes {
			assert.NotNil(ct, n.Status)
