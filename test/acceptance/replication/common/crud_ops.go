...
func ObjectExistsCL(t *testing.T, host, class string, id strfmt.UUID, cl types.ConsistencyLevel) (bool, error) {
	...
	var notFoundErr *objects.ObjectsClassHeadNotFound
...
func CountObjects(t *testing.T, host, class string) int64 {
	...
	aggRaw, ok := resp.Payload.Data["Aggregate"]
	require.True(t, ok, "missing 'Aggregate' key in GraphQL response data")
	aggMap, ok := aggRaw.(map[string]interface{})
	require.True(t, ok, "'Aggregate' is not map[string]interface{}")
	classRaw, ok := aggMap[class]
	require.True(t, ok, "missing class %q in Aggregate map", class)
	agg, ok := classRaw.([]interface{})
	require.True(t, ok, "class %q value is not []interface{}", class)
	require.Len(t, agg, 1)
	item, ok := agg[0].(map[string]interface{})
	require.True(t, ok, "aggregate item is not map[string]interface{}")
	metaRaw, ok := item["meta"]
	require.True(t, ok, "missing 'meta' key in aggregate item")
	meta, ok := metaRaw.(map[string]interface{})
	require.True(t, ok, "'meta' is not map[string]interface{}")
	countNum, ok := meta["count"].(json.Number)
	require.True(t, ok, "'count' is not json.Number")
