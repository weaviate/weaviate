...
func (suite *ReplicationTestSuiteEndpoints) TestReplicationReplicateCancelOpsEndpoints() {
	t := suite.T()
	...
	// Create COPY replication operation and wait until it is READY
	...
	t.Run("fail to cancel replication operation due to conflict with READY state", func(t *testing.T) {
...
func TestReplicationReplicateConflictsMOVE(t *testing.T) {
	...
	require.EventuallyWithT(t, func(ct *assert.CollectT) {
		status, err := helper.Client(t).Replication.ReplicationDetails(replication.NewReplicationDetailsParams().WithID(*created.Payload.ID), nil)
		require.Nil(t, err)
		require.Equal(ct, "DEHYDRATING", status.Payload.Status.State)
	}, 60*time.Second, 100*time.Millisecond, "Replication operation should be in DEHYDRATING state")

	t.Run("fail to cancel replication operation due to conflict with DEHYDRATING state", func(t *testing.T) {
		_, err := helper.Client(t).Replication.CancelReplication(replication.NewCancelReplicationParams().WithID(*created.Payload.ID), nil)
		require.NotNil(t, err)
		require.IsType(t, &replication.CancelReplicationConflict{}, err)
	})

	t.Run("fail to delete replication operation due to conflict with DEHYDRATING state", func(t *testing.T) {
		_, err := helper.Client(t).Replication.DeleteReplication(replication.NewDeleteReplicationParams().WithID(*created.Payload.ID), nil)
		require.NotNil(t, err)
