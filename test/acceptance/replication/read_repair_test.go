...
func readRepairNamedVectors(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()
	...
	helper.SetupClient(compose.ContainerURI(1))
	bookClass := books.ClassNamedContextionaryVectorizer()
	booksObjs := books.Objects()

	t.Run("CreateSchema", func(t *testing.T) {
		bookClass.ReplicationConfig = &models.ReplicationConfig{
			Factor: 3,
		}
		helper.CreateClass(t, bookClass)
	})

	time.Sleep(time.Second) // remove once eventual consistency has been addressed

	t.Run("InsertBooks/Node-1", func(t *testing.T) {
		createObjects(t, compose.ContainerURI(1), booksObjs)
	})
