...
func TestRootType(t *testing.T) {
	...
	assert.Equal(t, response.AssertKey(t, "__schema").AssertKey(t, "queryType").AssertKey(t, "name").AssertString(t), "WeaviateObj", "Root type should be the same as expected.")
}
...
func TestRootQueryType(t *testing.T) {
	...
	assert.Equal(t, response.AssertKey(t, "__type").AssertKey(t, "kind").AssertString(t), "OBJECT", "Root query type object kind should be same as expected.")
	assert.Equal(t, response.AssertKey(t, "__type").AssertKey(t, "name").AssertString(t), "WeaviateObj", "Root query type name should be same as expected.")
	assert.Equal(t, response.AssertKey(t, "__type").AssertKey(t, "description").AssertString(t), "Location of the root query", "Root query type description should be same as expected.")
}
...
func TestSchemaTypes(t *testing.T) {
	...
	for _, expected_type := range expected_schema.AssertKey(t, "data").AssertKey(t, "__schema").AssertKey(t, "types").AssertSlice(t) {
		...
		assert.Equal(t, response.AssertKey(t, "__type").AssertKey(t, "kind").AssertString(t), expected_type.AssertKey(t, "kind").AssertString(t), "The object's kind type should be the same as expected.")
		assert.Equal(t, response.AssertKey(t, "__type").AssertKey(t, "description").AssertString(t), expected_type.AssertKey(t, "description").AssertString(t), "The object's description should be the same as expected.")
			...
			for _, expected_possible_type := range expected_type.AssertKey(t, fields).AssertSlice(t) {
				...
				for _, actual_possible_type := range response.AssertKey(t, "__type").AssertKey(t, "fields").AssertSlice(t) {
					if expected_possible_type.AssertKey(t, "name").AssertString(t) == actual_possible_type.AssertKey(t, "name").AssertString(t) {
						...
						// assert field type
						// hasName := actual_possible_type.AssertKey(t,"name")
						// hasOfType := actual_possible_type.AssertKey(t, "ofType")

						assert.Equal(t, actual_possible_type.AssertKey(t, "name").AssertString(t), expected_possible_type.AssertKey(t, "type").AssertKey(t, "name").AssertString(t), "The field type's name should be the same as expected.")

						for true {
							assert.Equal(t, actual_possible_type.AssertKey(t, "ofType").AssertKey(t, "name").AssertString(t), expected_possible_type.AssertKey(t, "ofType").AssertKey(t, "name").AssertString(t), "The field type's name should be the same as expected.")
							actual_possible_type := actual_possible_type.AssertKey(t, "ofType")
							name := actual_possible_type.AssertKey(t, "name").AssertString(t)
							// if name != nil {
							// 	break
							// }
						}
		...
		if fields == "fields" || fields == "inputFields" {
			for _, expected_field := range expected_type.AssertKey(t, fields).AssertSlice(t) {
				...
				for _, actual_field := range response.AssertKey(t, "__type").AssertKey(t, "fields").AssertSlice(t) {
					if expected_field.AssertKey(t, "name").AssertString(t) == actual_field.AssertKey(t, "name").AssertString(t) {
						...
						assert.Equal(t, name, expected_field.AssertKey(t, "type").AssertKey(t, "name").AssertString(t), "The field type's name should be the same as expected.")
					}
					for true {
						assert.Equal(t, actual_field_type.AssertKey(t, "ofType").AssertKey(t, "name").AssertString(t), expected_field.AssertKey(t, "ofType").AssertKey(t, "name").AssertString(t), "The field type's name should be the same as expected.")
