...
// EVERY sample while the completed task is still in DTM, not just
// eventually.
func testPostDeleteNoSyntheticEntry(t *testing.T, restURI string) {
	...
	// DTM long after that, which is what the bleed fed on.
	...
	// for longer than the removed override's bound (2× scheduler tick,
	// clamped ≥3s).
