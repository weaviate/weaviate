...
// countDeletemeSegments counts files ending in ".db.deleteme".
func countDeletemeSegments(ctx context.Context, c testcontainers.Container, col, shard, bucket string) int {
	...
	count := 0
	for _, f := range files {
		if strings.HasSuffix(f, ".db.deleteme") {
			count++
		}
	}
	return count
...
// totalSegmentFileCount counts all *.db and *.db.deleteme files.
...
// reExplicitLevel matches e.g. ".l3.s2.db" or ".l3.s2.db.deleteme"
...
// countHighLevelSegments counts live (non-deleteme) segments with level ≥ 1.
func countHighLevelSegments(ctx context.Context, c testcontainers.Container, col, shard, bucket string) int {
	files := listBucketFiles(ctx, c, col, shard, bucket)
	count := 0
	for _, f := range files {
		if !strings.HasSuffix(f, ".db") || strings.HasSuffix(f, ".db.deleteme") {
...
// releaseView sends DELETE to the debug endpoint to release a held consistent view.
func releaseView(t *testing.T, debugHost, col, shard, bucket string) {
	t.Helper()
	url := fmt.Sprintf("http://%s/debug/consistent-view/%s/shards/%s/buckets/%s",
		debugHost, col, shard, bucket)
	...
	body, _ := io.ReadAll(resp.Body)
	require.Equal(t, http.StatusOK, resp.StatusCode,
		"releaseView failed: status=%d body=%s", resp.StatusCode, body)
}
