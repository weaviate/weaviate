...
func TestGenerativeContextualAI_SingleNode(t *testing.T) {
	apiKey := os.Getenv("CONTEXTUALAI_APIKEY")
	if apiKey == "" {
		t.Skip("skipping, CONTEXTUALAI_APIKEY environment variable not present")
	}
	...
	t.Run("tests", testGenerativeContextualAI(endpointREST, endpointGRPC))
}
...
func composeModules(apiKey string) (composeModules *docker.Compose) {
	composeModules = docker.New().
		...
		WithGenerativeContextualAI(apiKey)
