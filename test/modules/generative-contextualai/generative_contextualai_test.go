...
func testGenerativeContextualAI(rest, grpc string) func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				class.ModuleConfig = map[string]any{
					"generative-contextualai": map[string]any{
				...
				t.Run("create a tweet with params", func(t *testing.T) {
					params := "contextualai:{temperature:0.1}"
				...
				t.Run("create a tweet with params using grpc", func(t *testing.T) {
					params := &pb.GenerativeProvider_Contextualai{
					...
					planets.CreateTweetTestWithParamsGRPC(t, class.Class, &pb.GenerativeProvider{
						ReturnMetadata: false, // no metadata for contextualai
						Kind:           params,
					})
				})
