...
func TestText2VecDigitalOcean(t *testing.T) {
	apiKey := os.Getenv("DIGITALOCEAN_APIKEY")
	if apiKey == "" {
		t.Skip("skipping, DIGITALOCEAN_APIKEY environment variable not present")
	}
	...
	t.Run("tex2vec-digitalocean", testText2VecDigitalOcean(endpoint))
}
...
func composeModules(apiKey string) (composeModules *docker.Compose) {
	composeModules = docker.New().
		WithText2VecDigitalOcean(apiKey).
