...
func TestConsumerCopyIntegratingState(t *testing.T) {
	...
	mockReplicaCopier.EXPECT().
		...
		Times(1)
	mockReplicaCopier.EXPECT().
		TailAndApply(mock.Anything, "node1", "TestCollection", "shard1", mock.Anything, mock.Anything).
		Return(uint64(9), nil)
	mockReplicaCopier.EXPECT().
		FinalizeChangeLog(mock.Anything, "node1", "TestCollection", "shard1", mock.Anything).
		Return(uint64(9), nil).
		Times(1)
	mockReplicaCopier.EXPECT().
...
func TestConsumerSelfRecoveryFinalizingPromotesUnderLoadPolicy(t *testing.T) {
	logger, _ := logrustest.NewNullLogger()
	mockFSMUpdater := types.NewMockFSMUpdater(t)
	mockReplicaCopier := types.NewMockReplicaCopier(t)
	parser := fakes.NewMockParser()
	...
	mockReplicaCopier.EXPECT().
		...
		Times(1)
	mockReplicaCopier.EXPECT().
		PromoteRecoveredShard(mock.Anything, "TestCollection", "tenant1").
		Return(nil).
		Times(1)
	mockFSMUpdater.EXPECT().
		ReplicationUpdateReplicaOpStatus(mock.Anything, opId, api.INTEGRATING).
		Return(errors.New("stop after finalizing")).
		Times(1)
	mockFSMUpdater.EXPECT().
		ReplicationRegisterError(mock.Anything, opId, mock.Anything).
		Return(nil).
