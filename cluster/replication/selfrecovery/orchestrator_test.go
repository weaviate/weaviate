...
func newOrchestratorForTest(t *testing.T, raft RaftEntryPoint, schemaR SchemaReader, ns *stubNodeSelector,
	...
	t.Cleanup(func() {
		...
		if err := o.Close(ctx); err != nil {
			t.Errorf("closing orchestrator in cleanup: %v", err)
		}
	})
	return o
}
...
func TestShufflePeers_NotDeterministic(t *testing.T) {
	const N = 20
	base := make([]string, 10)
	for i := range base {
		base[i] = fmt.Sprintf("node-%d", i)
	}
	...
	require.True(t, differs, "shufflePeers produced identical orderings across %d trials — entropy regression?", N+1)
}
