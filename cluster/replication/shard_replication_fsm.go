...
func (s ShardReplicationOpStatus) ShouldConsumeOps() bool {
	return (!s.Canceling && s.GetCurrentState() != api.CANCELLED && s.GetCurrentState() != api.READY) || ((s.GetCurrentState() == api.CANCELLED || s.GetCurrentState() == api.READY) && s.ShouldCancelOrDelete())
}
