...
func (s *SchemaManager) UpdateClass(cmd *command.ApplyRequest, nodeID string, schemaOnly bool, enableSchemaCallback bool) error {
	...
	update := func(meta *metaClass) error {
		...
		// Gate at apply time so the verdict is RAFT-deterministic. The check
		// only applies when the caller actually targets VectorConfig: a masked
		// update that leaves VectorConfig alone drops nothing, and comparing
		// against its nil VectorConfig would read as "every entry dropped".
		if s.mutationGuard != nil && shouldUpdate(command.ClassFieldVectorConfig) {
