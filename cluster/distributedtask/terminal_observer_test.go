...
func TestFailedApplyRaisesTheTerminalObserver(t *testing.T) {
	...
	tests := []struct {
		...
		// unitsStopFirst marks the paths whose units are already stopped by the
		// time the failure lands, so their stamps predate it.
		unitsStopFirst bool
	}{
		{
	...
	// failing apply, so every timestamp already on the task is far in the past
	// when the failure lands: a dispatch that judges by any timestamp silently
	// drops these.
			...
			for _, tc := range tests {
				t.Run(tc.name, func(t *testing.T) {
					...
					tc.drive(t, h, func() {
						...
						require.True(t, staged[ns][0].FinishedAt.IsZero(),
							"sanity: a task that has not gone terminal carries no finish time")
						for _, u := range staged[ns][0].Units {
							require.Equal(t, tc.unitsStopFirst, !u.FinishedAt.IsZero(),
								"sanity: the units are stopped exactly on the paths staged that way")
						}
					...
					if aged {
						require.Equal(t, h.clock.Now().UnixMilli(), tasks[ns][0].FinishedAt.UnixMilli(),
							"sanity: the failing apply stamps FinishedAt at the failure, long after the units stopped")
					}
