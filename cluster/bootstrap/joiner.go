...
func NewJoiner(cl client, localNodeID string, localRaftAddr string, voter bool) *Joiner {
	return &Joiner{
		nodeClient:    cl,
...
func (j *Joiner) Do(ctx context.Context, lg *logrus.Logger, remoteNodes []string) (string, error) {
	...
	for _, addr := range remoteNodes {
		resp, err = j.nodeClient.Join(ctx, addr, req)
		if err == nil {
			return addr, nil
		}
		...
		if leader := resp.GetLeader(); st.Code() == codes.NotFound && leader != "" {
			_, err = j.nodeClient.Join(ctx, leader, req)
			if err == nil {
				return leader, nil
			}
		}
