...
// client is the interface we expect to be able to talk to the other nodes to either Join or Notify them
...
func NewBootstrapper(cl client, raftID string, raftAddr string, voter bool, r resolver.NodeToAddress, isStoreReady func() bool) *Bootstrapper {
	return &Bootstrapper{
		nodeClient:    cl,
		addrResolver:  r,
...
func (b *Bootstrapper) Do(ctx context.Context, serverPortMap map[string]int, lg *logrus.Logger, stop chan struct{}) error {
	...
	for {
		...
		case <-ticker.C:
			...
			joiner := NewJoiner(b.nodeClient, b.localNodeID, b.localRaftAddr, b.voter)
...
func (b *Bootstrapper) notify(ctx context.Context, remoteNodes []string) (err error) {
	...
	for _, addr := range remoteNodes {
		...
		_, err = b.nodeClient.Notify(ctx, addr, req)
		if err != nil {
			return err
		}
	}
