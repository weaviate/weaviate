...
func TestServerNewError(t *testing.T) {
	...
	t.Run("Empty server address", func(t *testing.T) {
		srv := NewServer(members, executor, querierManager, "", logger, raftGrpcMessageMaxSize, false)
		assert.NotNil(t, srv.Open())
	})

	t.Run("Invalid IP", func(t *testing.T) {
		srv := NewServer(members, executor, querierManager, "abc", logger, raftGrpcMessageMaxSize, false)
		netErr := &net.OpError{}
	...
	tests := []struct {
	...
	}{
		{
			...
			testFunc: func(t *testing.T, leaderAddr string, members *MockMembers, executor *MockExecutor) {
				...
				server := NewServer(members, executor, querierManager, leaderAddr, logger, raftGrpcMessageMaxSize, false)
