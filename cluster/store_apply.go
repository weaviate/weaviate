...
func (st *Store) Apply(l *raft.Log) interface{} {
	...
	case api.ApplyRequest_TYPE_DISTRIBUTED_TASK_ADD:
		f = func() {
			ret.Error = st.distributedTaskManager.AddTask(&cmd, l.Index)
		}
	case api.ApplyRequest_TYPE_DISTRIBUTED_TASK_RECORD_NODE_COMPLETED:
		f = func() {
			ret.Error = st.distributedTaskManager.RecordNodeCompletion(&cmd, st.numberOfNodesInTheCluster())
		}
	case api.ApplyRequest_TYPE_DISTRIBUTED_TASK_CANCEL:
		f = func() {
			ret.Error = st.distributedTaskManager.CancelTask(&cmd)
		}
	case api.ApplyRequest_TYPE_DISTRIBUTED_TASK_CLEAN_UP:
		f = func() {
			ret.Error = st.distributedTaskManager.CleanUpTask(&cmd)
		}
	default:
